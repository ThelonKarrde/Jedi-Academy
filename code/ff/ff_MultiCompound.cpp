...
qboolean MultiCompound::Add(Set &effect)
{
	...
	for (Set::iterator itSet = effect.begin(); itSet != effect.end(); itSet++)
	{
		result &= Add(*itSet);
	}
...
qboolean MultiCompound::Start()
{
	...
	for (Set::iterator itSet = mSet.begin(); itSet != mSet.end(); itSet++)
	{
		result &= (*itSet)->Start();
	}

	return qboolean(result && mSet.size() != 0);
}
...
qboolean MultiCompound::operator==(MultiCompound &compound)
{
	...
	{
		for (Set::iterator itSet = mSet.begin(), itOther = other.begin(); itSet != mSet.end()

...
FFSet::FFSet(FFConfigParser &ConfigParser, CImmDevice *Device)
	: mParser(ConfigParser), mDevice(Device), mInclude(), mIncludePath()
{
...
FFSet::~FFSet()
{
	for (TInclude::iterator itInclude = mInclude.begin(); itInclude != mInclude.end(); itInclude++)
	{
		for (TProject::iterator itProject = (*itInclude).begin(); itProject != (*itInclude).end(); itProject++)
		{
...
void FFSet::GetRegisteredNames(TNameTable &NameTable)
{
	...
	{
		for (TProject::iterator itProject = mInclude[IncludeIndex].begin(); itProject != mInclude[IncludeIndex].end(); itProject++)
		{
			...
			for (MultiEffect *Effect = (MultiEffect *)(*itProject).second->GetCreatedEffect(i); Effect; Effect = (MultiEffect *)(*itProject).second->GetCreatedEffect(++i))
			{
...
void FFSet::Display(TNameTable &Unprocessed, TNameTable &Processed)
{
	for (TNameTable::iterator itName = Unprocessed.begin(); itName != Unprocessed.end();)
	{
		...
		else if (stricmp("files", (*itName).c_str()) == 0)
		{
			if (ff_developer->integer)
				DisplayLoadedFiles();
			//else
...
void FFSet::DisplayLoadedFiles(void)
{
	...
	*ProductName = 0;
	...
	for (int i = 0; i < mInclude.size(); i++)
	{
		for (TProject::iterator itProject = mInclude[i].begin(); itProject != mInclude[i].end(); itProject++)
		{
			if ((*itProject).second)
			{
				++total;

...
qboolean FFChannelSet::ParseChannels(const char *channels)
{
	...
	for (pos = channels; pos && sscanf(pos, "%d", &channel) == 1;)
	{
		int device;
		char *endpos;
		endpos = strchr(pos, ';');
		...
		{
			for (pos = strchr(pos, ','); pos && (!endpos || pos < endpos) && sscanf(pos, " ,%d", &device) == 1; pos = strchr(pos + 1, ','))
			{
				if (device >= 0 && device < mSet.size())
				{
					for (ChannelIterator itChannel(mChannel, channel); itChannel != mChannel.end() && (**itChannel).second != device // found duplicate
						 ;
						 ++itChannel)
						;
...
{
	for (ChannelIterator itChannel(mChannel, compound.GetChannel()); itChannel != mChannel.end(); ++itChannel)
	{
		MultiEffect *Effect;
...
void FFChannelSet::Display(TNameTable &Unprocessed, TNameTable &Processed)
{
	...
	for (TNameTable::iterator itName = Unprocessed.begin(); itName != Unprocessed.end();)
	{
		if (stricmp("channels", (*itName).c_str()) == 0)
		{
			...
			for (int i = 0; i < FF_CHANNEL_MAX; i++)
			{
				Com_Printf("%d) %s  devices:", i, gChannelName[i]);
				for (ChannelIterator itChannel(mChannel, i); itChannel != mChannel.end(); ++itChannel)
				{

...
void S_ReloadAllUsedSounds(void)
{
	if (s_soundStarted && !s_soundMuted)
	{
		...
		{
			if (s_sfxCodes[i] == INVALID_CODE || s_sfxCodes[i] == s_defaultSound)
				continue;
...
sfxHandle_t S_RegisterSound(const char *name)
{
	...
	if (handle < 0)
	{
		...
		if (strlen(name) < 5 || name[0] == '*')
			...
			sfx->iFileCode = S_GetFileCode(name);
...
static bool IsListenerEnt(int entnum)
{
	...
	{
		if (s_listeners[i].entnum == entnum)
			return true;
	}
	return false;
}

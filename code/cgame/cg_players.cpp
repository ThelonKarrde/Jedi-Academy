...
static void CG_PlayerAnimEventDo(centity_t *cent, animevent_t *animEvent)
{
	...
	case AEV_EFFECT:
		...
		{ //invalid effect
			if (animEvent->stringData != NULL && animEvent->stringData[0])
		...
		else
		{
			...
			{ //have a bolt name we want to use
				...
				if ((Q_stricmpn("*blade", animEvent->stringData, 6) == 0 || Q_stricmp("*flash", animEvent->stringData) == 0) && cent->gent->weaponModel[0] > 0)
		...
		if (animEvents[i].modelOnly == 0 || animEvents[i].modelOnly == myModel.handle())
		{
			...
			{									//given a range, see if keyFrame falls in that range
				...
				{ //if changed anims altogether, sorry, the sound is lost
					if (abs(oldFrame - animEvents[i].keyFrame) <= 3 || abs(frame - animEvents[i].keyFrame) <= 3)
...
static void CG_G2ClientSpineAngles(centity_t *cent, vec3_t viewAngles, const vec3_t angles, vec3_t thoracicAngles, vec3_t ulAngles, vec3_t llAngles)
{
	...
	*/
	...
	if (cg_motionBoneComp.integer && !PM_FlippingAnim(cent->currentState.legsAnim) && !PM_SpinningSaberAnim(cent->currentState.legsAnim) && !PM_SpinningSaberAnim(cent->currentState.torsoAnim) && cent->currentState.legsAnim != cent->currentState.torsoAnim //NOTE: presumes your legs & torso are on the same frame, though they *should* be because PM_SetAnimFinal tries to keep them in synch
...
static void CG_G2PlayerAngles(centity_t *cent, vec3_t legs[3], vec3_t angles)
{
	...
	*/
	...
	if (cent->gent && (cent->gent->flags & FL_NO_ANGLES))
	...
	if (cent->gent && cent->gent->client && (cent->gent->client->NPC_class != CLASS_GONK) && (cent->gent->client->NPC_class != CLASS_INTERROGATOR) && (cent->gent->client->NPC_class != CLASS_SENTRY) && (cent->gent->client->NPC_class != CLASS_PROBE) && (cent->gent->client->NPC_class != CLASS_R2D2) && (cent->gent->client->NPC_class != CLASS_R5D2) && (cent->gent->client->NPC_class != CLASS_ATST || !cent->gent->s.number))
	{ // If we are rendering third person, we should just force the player body to always fully face
		...
		*/
		...
		else
		{
			...
			if (((cent->gent->client->ps.eFlags & EF_FORCE_GRIPPED) || ((cent->gent->client->NPC_class == CLASS_BOBAFETT || cent->gent->client->NPC_class == CLASS_ROCKETTROOPER) && cent->gent->client->moveType == MT_FLYSWIM)) && cent->gent->client->ps.groundEntityNum == ENTITYNUM_NONE)
			{
				vec3_t centFwd, centRt;
				float divFactor = 1.0f;
				if ((cent->gent->client->NPC_class == CLASS_BOBAFETT || cent->gent->client->NPC_class == CLASS_ROCKETTROOPER) && cent->gent->client->moveType == MT_FLYSWIM)
				{
					divFactor = 3.0f;
				}
	...
	{
		if ((cent->gent->client->NPC_class == CLASS_PROBE) || (cent->gent->client->NPC_class == CLASS_R2D2) || (cent->gent->client->NPC_class == CLASS_R5D2) || (cent->gent->client->NPC_class == CLASS_RANCOR) || (cent->gent->client->NPC_class == CLASS_WAMPA) || (cent->gent->client->NPC_class == CLASS_ATST))
		{
			VectorCopy(cent->lerpAngles, angles);
			angles[PITCH] = 0;
...
{ //return true if this cent is in view
	...
	if (g_entities[0].client->ps.forcePowerLevel[FP_SEE] < FORCE_LEVEL_2 && cent->currentState.eType != ET_PLAYER)
...
void CG_AddForceSightShell(refEntity_t *ent, centity_t *cent)
{
	...
	case TEAM_FREE:
		if (cent->gent && cent->gent->client)
		{
			if (cent->gent->client->NPC_class == CLASS_TUSKEN || cent->gent->client->NPC_class == CLASS_RANCOR || cent->gent->client->NPC_class == CLASS_WAMPA || cent->gent->client->NPC_class == CLASS_SAND_CREATURE)
			{
				ent->shaderRGBA[0] = 255;
				ent->shaderRGBA[1] = 0;
				ent->shaderRGBA[2] = 0;
...
*/
	...
	if ((cent->gent->client->ps.forcePowersActive & (1 << FP_PROTECT)) && (cent->gent->client->ps.forcePowersActive & (1 << FP_ABSORB)))
	...
	if ((cg.snap->ps.forcePowersActive & (1 << FP_SEE)) && cg.snap->ps.clientNum != cent->currentState.number && (cent->currentState.eFlags & EF_FORCE_VISIBLE || ((cent->gent->health > 0 || cent->gent->message) && cent->currentState.eType == ET_PLAYER //other things handle this in their own render funcs
...
static int CG_SaberHumForceForEnt(gentity_t *gent)
{
	...
	if (gent && gent->client)
	{
		...
		else if (gent->client->NPC_class == CLASS_PLAYER || gent->client->NPC_class == CLASS_KYLE)
		{ //Kyle NPC and player
...
#ifdef _IMMERSION
		cgi_FF_AddLoopingForce(CG_SaberHumForceForEnt(&g_entities[cent->currentState.clientNum]), cent->currentState.number);
#endif // _IMMERSION
		return;
	}
			...
			if (trace.fraction < 1.0f)
			{
				...
				else
				{
					if (!(trace.surfaceFlags & SURF_NOIMPACT) // never spark on sky
						&& (trace.entityNum == ENTITYNUM_WORLD || cg_entities[trace.entityNum].currentState.solid == SOLID_BMODEL) && Q_irand(1, client->ps.saber[saberNum].numBlades) == 1)
					{
						//was "sparks/spark"
...
*/
	if (cent->gent->ghoul2.size()) //do we have ghoul models attached?
	{
		...
		else if (((cent->gent->client->ps.eFlags & EF_HELD_BY_RANCOR) || (cent->gent->client->ps.eFlags & EF_HELD_BY_WAMPA)) && cent->gent && cent->gent->activator)
		{
		...
		*/
		...
		if (cent->currentState.number != 0 || cg.renderingThirdPerson || cg.snap->ps.stats[STAT_HEALTH] <= 0 || (!cg.renderingThirdPerson && (cg.snap->ps.weapon == WP_SABER || cg.snap->ps.weapon == WP_MELEE)) //First person saber
		)
		...
		//Handle saber
		if (cent->gent && cent->gent->client && (cent->currentState.weapon == WP_SABER || cent->gent->client->ps.saberInFlight) && cent->gent->client->NPC_class != CLASS_ATST)
		{ //FIXME: somehow saberactive is getting lost over the network
			...
			for (int saberNum = 0; saberNum < numSabers; saberNum++)
			{
				...
				for (int bladeNum = 0; bladeNum < cent->gent->client->ps.saber[saberNum].numBlades; bladeNum++)
				{
					...
					if (cent->currentState.weapon == WP_SABER && (cent->gent->client->ps.saber[saberNum].blade[bladeNum].length > 0 || cent->gent->client->ps.saberInFlight))
					{
						calcedMp = qtrue;
					}
				}
		...
		{ //if NPC, third person, or dead, unless using saber
			...
			*/
			...
			if (!calcedMp)
			{
						 ...
						 && cent->gent->weaponModel[1])				  //one in each hand
				{
					...
					if ((cent->gent->weaponModel[cent->gent->count] != -1) && (cent->gent->ghoul2.size() > cent->gent->weaponModel[cent->gent->count]) && (cent->gent->ghoul2[cent->gent->weaponModel[cent->gent->count]].mModelindex != -1))
			...
			else if (cent->muzzleFlashTime > 0 && wData && !(cent->currentState.eFlags & EF_LOCKED_TO_WEAPON))
			{
				...
				if (/*( cent->currentState.eFlags & EF_FIRING || cent->currentState.eFlags & EF_ALT_FIRING ) &&*/ effect)
				{
					...
					{
						if (!VectorCompare(oldMP, vec3_origin) && !VectorCompare(oldMD, vec3_origin))
			...
			*/
			...
			if (cent->gent->client->ps.eFlags & EF_FORCE_DRAINED || (cent->currentState.powerups & (1 << PW_DRAINED)))
			{ //being drained
			...
			{ //doing the electrocuting
				...
				{ //arc
					...
					if (cent->gent->client->ps.torsoAnim == BOTH_FORCE_2HANDEDLIGHTNING || cent->gent->client->ps.torsoAnim == BOTH_FORCE_2HANDEDLIGHTNING_START || cent->gent->client->ps.torsoAnim == BOTH_FORCE_2HANDEDLIGHTNING_HOLD || cent->gent->client->ps.torsoAnim == BOTH_FORCE_2HANDEDLIGHTNING_RELEASE)
			...
			if (cent->gent->client->ps.forcePowersActive & (1 << FP_DRAIN) && cent->gent->client->ps.forceDrainEntityNum >= ENTITYNUM_WORLD)
		...
		if (cent->gent && cent->gent->client && cent->gent->client->ps.weapon == WP_SABER)
		{
			...
			if (!PM_KickingAnim(cent->gent->client->ps.torsoAnim) || cent->gent->client->ps.torsoAnim == BOTH_A7_KICK_S)
...
*/
	...
	if (cent->currentState.number == 0 && cg.renderingThirdPerson)
	{
		...
		if ((ps->weaponstate == WEAPON_CHARGING_ALT && ps->weapon == WP_BRYAR_PISTOL) || (ps->weaponstate == WEAPON_CHARGING_ALT && ps->weapon == WP_BLASTER_PISTOL) || (ps->weapon == WP_BOWCASTER && ps->weaponstate == WEAPON_CHARGING) || (ps->weapon == WP_DEMP2 && ps->weaponstate == WEAPON_CHARGING_ALT))
		{
			int shader = 0;

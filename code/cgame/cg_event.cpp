...
qboolean CG_ConfigForce(int index, const char *&name, int &channel)
{
	...
	result = qboolean(configstring && sscanf(configstring, "%d", &channel) == 1);
...
*/
	...
	case EV_FALL_MEDIUM:
		...
		else if (g_entities[es->number].s.weapon == WP_SABER || (g_entities[es->number].client && (g_entities[es->number].client->ps.forcePowersKnown & (1 << FP_LEVITATION))))

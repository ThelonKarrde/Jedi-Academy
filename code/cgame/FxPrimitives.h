...
public:
	...
	inline void SetSTScale(float s, float t)
	{
		mRefEnt.shaderTexCoord[0] = s;
		mRefEnt.shaderTexCoord[1] = t;
	}

	inline void SetMin(const vec3_t min)
	{
		if (min)
		{
			VectorCopy(min, mMin);
	...
	inline void SetMax(const vec3_t max)
	{
		if (max)
		{
			VectorCopy(max, mMax);
	...
	inline void SetOrigin1(const vec3_t org)
	{
		if (org)
		{
		...
		{
			VectorClear(mOrigin1);
		}
	}
	inline void SetTimeStart(int time)
	{
		mTimeStart = time;
		if (mFlags & FX_SET_SHADER_TIME)
		{
			mRefEnt.shaderTime = cg.time * 0.001f;
		}
	}
...
public:
	...
	inline void SetRGBStart(vec3_t rgb)
	{
		if (rgb)
		{
			VectorCopy(rgb, mRGBStart);
		}
		else
		{
			VectorClear(mRGBStart);
		}
	}
	inline void SetRGBEnd(vec3_t rgb)
	{
		if (rgb)
		{
			VectorCopy(rgb, mRGBEnd);
		}
		else
		{
			VectorClear(mRGBEnd);
...
public:
	...
	inline void SetOrgOffset(const vec3_t o)
	{
		if (o)
		{
			VectorCopy(o, mOrgOffset);
		}
		else
		{
			VectorClear(mOrgOffset);
		}
	}
	inline void SetVel(const vec3_t vel)
	{
		if (vel)
		{
			VectorCopy(vel, mVel);
	...
	inline void SetAccel(const vec3_t ac)
	{
		if (ac)
		{
			VectorCopy(ac, mAccel);
		}
		else
		{
			VectorClear(mAccel);
	...
	inline void SetRGBStart(const vec3_t rgb)
	{
		if (rgb)
		{
			VectorCopy(rgb, mRGBStart);
	...
	inline void SetRGBEnd(const vec3_t rgb)
	{
		if (rgb)
		{
			VectorCopy(rgb, mRGBEnd);
	...
	inline void SetClient(int clientID, int modelNum = -1, int boltNum = -1)
	{
		mClientID = clientID;
		mModelNum = modelNum;
		mBoltNum = boltNum;
...
public:
	...
	inline void SetControlPoints(const vec3_t ctrl1, const vec3_t ctrl2)
	{
		VectorCopy(ctrl1, mControl1);
		VectorCopy(ctrl2, mControl2);
	}
	inline void SetControlVel(const vec3_t ctrl1v, const vec3_t ctrl2v)
	{
...
public:
	...
	inline void SetAngles(const vec3_t ang)
	{
		if (ang)
		{
			VectorCopy(ang, mAngles);
		}
		else
		{
			VectorClear(mAngles);
	...
	inline void SetAngleDelta(const vec3_t ang)
	{
		if (ang)
		{
			VectorCopy(ang, mAngleDelta);
		}
		else
		{
			VectorClear(mAngleDelta);
		}
	}
	...
	inline void SetLastOrg(const vec3_t org)
	{
		if (org)
		{
			VectorCopy(org, mLastOrigin);
		}
		else
		{
			VectorClear(mLastOrigin);
		}
	}
	inline void SetLastVel(const vec3_t vel)
	{
		if (vel)
		{
			VectorCopy(vel, mOldVelocity);
		}
		else
		{
			VectorClear(mOldVelocity);
		}
	}
};
...
public:
	...
	inline void SetRot(vec3_t r)
	{
		if (r)
		{
			VectorCopy(r, mRotDelta);
		}
		else
		{

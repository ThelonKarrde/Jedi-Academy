...
void CG_SetDataPadWeaponText(void)
{
	...
	if (!cg.DataPadWeaponSelect)
	{ //Print nothing if you have no weapons.
...
void CG_NextWeapon_f(void)
{
	...
	*/
	...
	if (cg.snap->ps.viewEntity)
	{
		...
		if (g_entities[cg.snap->ps.viewEntity].client && (g_entities[cg.snap->ps.viewEntity].client->NPC_class == CLASS_R5D2 || g_entities[cg.snap->ps.viewEntity].client->NPC_class == CLASS_R2D2 || g_entities[cg.snap->ps.viewEntity].client->NPC_class == CLASS_MOUSE))
		{
			return;
		}
	}
		...
		{ //already have it up, let's try to toggle it
			...
			{ //player can't activate/deactivate saber when in a cinematic
				...
				{ //it's either in-hand or it's under telekinetic control
					...
					{ //a saber is on
						if (cg_entities[0].gent->client->ps.dualSabers && cg_entities[0].gent->client->ps.saber[1].Active())

		...
		for (int time = curTime - 25; time <= curTime + 25 && !hit; time += 25)
		{
			...
			{ //hit something
				...
				{ //smack
					...
					if (traceEnt->health > 0 && ((traceEnt->client && traceEnt->client->NPC_class == CLASS_JAWA && !Q_irand(0, 1)) || dmg > 19)) //FIXME: base on skill!
					{																															 //do pain on enemy
		...
		self->client->ps.torsoAnim == BOTH_TUSKENLUNGE1)
	{
										...
										NULL))
		{
			...
			case BOTH_TUSKENATTACK1:
				return (percentComplete > 0.3 && percentComplete < 0.7);
			case BOTH_TUSKENATTACK2:
				return (percentComplete > 0.3 && percentComplete < 0.7);
			case BOTH_TUSKENATTACK3:
				return (percentComplete > 0.1 && percentComplete < 0.5);
			case BOTH_TUSKENLUNGE1:
				return (percentComplete > 0.3 && percentComplete < 0.5);
			}
		}
	}
	return qfalse;

...
static void ST_CheckMoveState(void)
{
	...
	else if (NPC->client->NPC_class == CLASS_ROCKETTROOPER && NPC->client->ps.groundEntityNum == ENTITYNUM_NONE)
	{ //no squad stuff
		return;
	}
...
void ST_Commander(void)
{
	...
	for (i = curMemberNum; i < lastMemberNum; i++)
	{
		...
		if (NPC->s.weapon == WP_NONE && NPCInfo->goalEntity && NPCInfo->goalEntity == NPCInfo->tempGoal && NPCInfo->goalEntity->s.eType == ET_ITEM)
...
{
	if (!NPC->ghoul2.size() || NPC->weaponModel[0] <= 0)
	{
		return;
	}
	...
	if (boltIndex != -1)
	{
		...
		for (int time = curTime - 25; time <= curTime + 25 && !hit; time += 25)
		{
			...
			{ //hit something
				...
				if (traceEnt->takedamage && (!traceEnt->client || traceEnt == NPC->enemy || traceEnt->client->NPC_class != NPC->client->NPC_class))
				{							   //smack
...
void NPC_BSST_Attack(void)
{
	...
	if (NPC_ClearLOS(NPC->enemy))
	{
		...
		{ //can we shoot our target?
			if ((enemyDist < MIN_ROCKET_DIST_SQUARED) &&
				...
				((NPC->client->ps.weapon == WP_ROCKET_LAUNCHER || (NPC->client->ps.weapon == WP_CONCUSSION && !(NPCInfo->scriptFlags & SCF_ALT_FIRE)) || (NPC->client->ps.weapon == WP_FLECHETTE && (NPCInfo->scriptFlags & SCF_ALT_FIRE)))))
			{
				enemyCS = qfalse; //not true, but should stop us from firing
				hitAlly = qtrue;  //us!
			...
			{ //if enemy is FOV, go ahead and check for shooting
				...
				if (hit == NPC->enemy->s.number || (hitEnt && hitEnt->client && hitEnt->client->playerTeam == NPC->client->enemyTeam) || (hitEnt && hitEnt->takedamage && ((hitEnt->svFlags & SVF_GLASS_BRUSH) || hitEnt->health < 40 || NPC->s.weapon == WP_EMPLACED_GUN)))
	...
	{							 //move toward goal
		if (NPCInfo->goalEntity) //&& ( NPCInfo->goalEntity != NPC->enemy || enemyDist > 10000 ) )//100 squared
		{
			...
			{ //moving backwards at least 45 degrees
				if (NPCInfo->goalEntity && DistanceSquared(NPCInfo->goalEntity->currentOrigin, NPC->currentOrigin) > MIN_TURN_AROUND_DIST_SQ)
	...
	if (NPC->client->fireDelay)
	{
		...
		if (NPC->s.weapon == WP_ROCKET_LAUNCHER || (NPC->s.weapon == WP_CONCUSSION && !(NPCInfo->scriptFlags & SCF_ALT_FIRE)))
		{
			if (!enemyLOS || !enemyCS)
			{ //cancel it
	...
	{ //try to shoot if it's time
		...
		if (TIMER_Done(NPC, "attackDelay"))
		{
			...
			{
				if ((ucmd.buttons & BUTTON_ATTACK) && !move && g_spskill->integer > 1 && !Q_irand(0, 3))
			...
			else if (NPC->s.weapon == WP_NOGHRI_STICK && enemyDist < (48 * 48)) //?
			{
				ucmd.buttons &= ~BUTTON_ATTACK;
				ucmd.buttons |= BUTTON_ALT_ATTACK;

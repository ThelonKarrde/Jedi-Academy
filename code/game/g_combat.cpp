...
gentity_t *TossClientItems(gentity_t *self)
{
	...
	if (self->client->NPC_class == CLASS_SEEKER || self->client->NPC_class == CLASS_REMOTE || self->client->NPC_class == CLASS_SABER_DROID || self->client->NPC_class == CLASS_VEHICLE || self->client->NPC_class == CLASS_ATST)
	{
...
{
	if (ent1->s.number < MAX_CLIENTS && ent1->client && ent1->client->playerTeam == TEAM_FREE)
...
qboolean G_GetHitLocFromSurfName(gentity_t *ent, const char *surfName, int *hitLoc, vec3_t point, vec3_t dir, vec3_t bladeDir, int mod, saberType_t saberType)
{
	...
	*hitLoc = HL_NONE;
	...
	if (ent->client && (ent->client->NPC_class == CLASS_R2D2 || ent->client->NPC_class == CLASS_R2D2 || ent->client->NPC_class == CLASS_GONK || ent->client->NPC_class == CLASS_MOUSE || ent->client->NPC_class == CLASS_SENTRY || ent->client->NPC_class == CLASS_INTERROGATOR || ent->client->NPC_class == CLASS_SENTRY || ent->client->NPC_class == CLASS_PROBE))
...
{ //FIXME: just use object thinking?
	...
	if (ent->owner && ent->owner->client && (ent->owner->client->ps.eFlags & EF_HELD_BY_RANCOR))
	{
	...
	if (tr.fraction != 1)
	{
		...
		{ //stopped, stop spinning
			...
			{ //lay flat
				if (ent->owner && ent->owner->client && ent->owner->client->NPC_class == CLASS_PROTOCOL && ent->count == BOTH_DISMEMBER_TORSO1)
				{
...
qboolean G_LimbLost(gentity_t *ent, int hitLoc)
{
	...
	case HL_BACK_RT:
		...
		if (ent->locationDamage[HL_ARM_LT] >= Q3_INFINITE || ent->locationDamage[HL_CHEST_LT] >= Q3_INFINITE || ent->locationDamage[HL_BACK_RT] >= Q3_INFINITE || ent->locationDamage[HL_WAIST] >= Q3_INFINITE)
		{
			return qtrue;
		}
		return qfalse;
	...
	case HL_BACK_LT:
		//NOTE: hand falls through
		if (ent->locationDamage[HL_ARM_RT] >= Q3_INFINITE || ent->locationDamage[HL_CHEST_RT] >= Q3_INFINITE || ent->locationDamage[HL_BACK_LT] >= Q3_INFINITE || ent->locationDamage[HL_WAIST] >= Q3_INFINITE)
		{
			return qtrue;
		}
		return qfalse;
							...
							int damage, int hitLoc)
{
	...
	*/
	...
	else if (limbAnim == BOTH_DISMEMBER_HEAD1 || limbAnim == BOTH_DISMEMBER_LARM || limbAnim == BOTH_DISMEMBER_RARM)
...
//NOTE that you can only cut one thing off unless the dismemberment is >= 11381138
	...
	{ //FIXME: don't do strcmps here
		if (G_StandardHumanoid(self) && (force || g_dismemberProbabilities->value > 0.0f || G_Dismemberable2(self, hitLoc)))
...
static int G_CheckSpecialDeathAnim(gentity_t *self, vec3_t point, int damage, int mod, int hitLoc)
{
	...
	if (self->client->ps.legsAnim == BOTH_GETUP_BROLL_L || self->client->ps.legsAnim == BOTH_GETUP_BROLL_R)
	{ //rolling away to the side on our back
		deathAnim = BOTH_DEATH_LYING_UP;
	}
	else if (self->client->ps.legsAnim == BOTH_GETUP_FROLL_L || self->client->ps.legsAnim == BOTH_GETUP_FROLL_R)
	{ //rolling away to the side on our front
		deathAnim = BOTH_DEATH_LYING_DN;
	}
	else if (self->client->ps.legsAnim == BOTH_GETUP_BROLL_F && self->client->ps.legsAnimTimer > 350)
	{ //kicking up
		deathAnim = BOTH_DEATH_FALLING_UP;
	}
	else if (self->client->ps.legsAnim == BOTH_GETUP_BROLL_B && self->client->ps.legsAnimTimer > 950)
	{ //on back, rolling back to get up
		deathAnim = BOTH_DEATH_LYING_UP;
	}
	else if (self->client->ps.legsAnim == BOTH_GETUP_BROLL_B && self->client->ps.legsAnimTimer <= 950 && self->client->ps.legsAnimTimer > 250)
	{ //flipping over backwards
		deathAnim = BOTH_FALLDEATH1LAND;
	}
	else if (self->client->ps.legsAnim == BOTH_GETUP_FROLL_B && self->client->ps.legsAnimTimer <= 1100 && self->client->ps.legsAnimTimer > 250)
	{ //flipping over backwards
		deathAnim = BOTH_FALLDEATH1LAND;
	}
...
{ //FIXME: play dead flop anims on body if in an appropriate _DEAD anim when this func is called
	...
	{
		if (deathAnim == BOTH_DEADFLOP1 || deathAnim == BOTH_DEADFLOP2)
...
void player_die(gentity_t *self, gentity_t *inflictor, gentity_t *attacker, int damage, int meansOfDeath, int dflags, int hitLoc)
{
	...
	if (self->client && self->client->NPC_class != CLASS_VEHICLE && self->s.m_iVehicleNum != 0)
	{
		...
		if (pVeh)
		{
			if (pVeh->m_pOldPilot != self && pVeh->m_pPilot != self)
	...
	if (attacker)
	{
		if (attacker->client && !attacker->s.number)
		{
			if (self->client)
			{ //killed a client
				if (self->client->playerTeam == TEAM_ENEMY || self->client->playerTeam == TEAM_FREE || (self->NPC && self->NPC->charmedTime > level.time))
				{ //killed an enemy
		...
		if (d_slowmodeath->integer)
		{
			...
			{ //any jedi killed by player-saber
				...
				if (!attacker->s.number && (holdingSaber || self->client->NPC_class == CLASS_WAMPA) && meansOfDeath == MOD_SABER && attacker->client && attacker->client->ps.weapon == WP_SABER && !attacker->client->ps.saberInFlight //FIXME: if dualSabers, should still do slowmo if this killing blow was struck with the left-hand saber...
	...
	if (!holdingSaber
		//&& self->s.number != 0
		&& !(contents & CONTENTS_NODROP) && meansOfDeath != MOD_SNIPER && (!self->client || self->client->NPC_class != CLASS_GALAKMECH))
	{
		TossClientItems(self);
	}
	...
	*/
	...
	if ((self->client->ps.eFlags & EF_HELD_BY_RANCOR) || (self->client->ps.eFlags & EF_HELD_BY_SAND_CREATURE) || (self->client->ps.eFlags & EF_HELD_BY_WAMPA))
	...
	else if (meansOfDeath == MOD_FALLING || (self->client->ps.legsAnim == BOTH_FALLDEATH1INAIR && self->client->ps.torsoAnim == BOTH_FALLDEATH1INAIR) || (self->client->ps.legsAnim == BOTH_FALLDEATH1 && self->client->ps.torsoAnim == BOTH_FALLDEATH1))
	{
		...
		else
		{
			...
			if (gi.VoiceVolume[self->s.number] && self->NPC && (self->NPC->aiFlags & NPCAI_DIE_ON_IMPACT))
	...
	{ // normal death
		...
		if (anim == -1)
		{
			if (meansOfDeath == MOD_ELECTROCUTE || (meansOfDeath == MOD_CRUSH && self->s.eFlags & EF_FORCE_GRIPPED) || (meansOfDeath == MOD_FORCE_DRAIN && self->s.eFlags & EF_FORCE_DRAINED))
	...
	else
	{
		...
		{																																															   //-1 == don't play pain anim
			...
			{																																														   //FIXME: only strong-level saber attacks should make me play pain anim?
				if (!G_CheckForStrongAttackMomentum(self) && !PM_SpinningSaberAnim(self->client->ps.legsAnim) && !PM_SaberInSpecialAttack(self->client->ps.torsoAnim) && !PM_InKnockDown(&self->client->ps))
...
void G_Knockdown(gentity_t *self, gentity_t *attacker, const vec3_t pushDir, float strength, qboolean breakSaberLock)
{
	...
	if (self->health > 0)
	{
		...
		if (!PM_SpinningSaberAnim(self->client->ps.legsAnim) && !PM_FlippingAnim(self->client->ps.legsAnim) && !PM_RollingAnim(self->client->ps.legsAnim) && !PM_InKnockDown(&self->client->ps))
		{
...
void G_CheckKnockdown(gentity_t *targ, gentity_t *attacker, vec3_t newDir, int dflags, int mod)
{
	...
	{ //not inherently explosive damage, check mod
		if (mod != MOD_REPEATER_ALT && mod != MOD_FLECHETTE_ALT && mod != MOD_ROCKET && mod != MOD_ROCKET_ALT && mod != MOD_CONC && mod != MOD_CONC_ALT && mod != MOD_THERMAL && mod != MOD_THERMAL_ALT && mod != MOD_DETPACK && mod != MOD_LASERTRIP && mod != MOD_LASERTRIP_ALT && mod != MOD_EXPLOSIVE && mod != MOD_EXPLOSIVE_SPLASH)
		{
			return;
		}
	}
...
void G_ApplyKnockback(gentity_t *targ, vec3_t newDir, float knockback)
{
	...
	if (targ && targ->client && (targ->client->NPC_class == CLASS_ATST || targ->client->NPC_class == CLASS_RANCOR || targ->client->NPC_class == CLASS_SAND_CREATURE))
...
{
	if (meansOfDeath == MOD_EXPLOSIVE || meansOfDeath == MOD_REPEATER_ALT || meansOfDeath == MOD_FLECHETTE_ALT || meansOfDeath == MOD_ROCKET || meansOfDeath == MOD_ROCKET_ALT || meansOfDeath == MOD_CONC || meansOfDeath == MOD_THERMAL || meansOfDeath == MOD_THERMAL_ALT || meansOfDeath == MOD_DETPACK || meansOfDeath == MOD_LASERTRIP || meansOfDeath == MOD_LASERTRIP_ALT || meansOfDeath == MOD_MELEE || meansOfDeath == MOD_FORCE_GRIP || meansOfDeath == MOD_KNOCKOUT || meansOfDeath == MOD_CRUSH || meansOfDeath == MOD_EXPLOSIVE_SPLASH)
	{
		return qtrue;
	}
	return qfalse;
...
qboolean G_ImmuneToGas(gentity_t *ent)
{
	...
	if (ent->s.weapon == WP_NOGHRI_STICK																																																																																																					//assumes user is immune
		|| ent->client->NPC_class == CLASS_HAZARD_TROOPER || ent->client->NPC_class == CLASS_ATST || ent->client->NPC_class == CLASS_GONK || ent->client->NPC_class == CLASS_SAND_CREATURE || ent->client->NPC_class == CLASS_INTERROGATOR || ent->client->NPC_class == CLASS_MARK1 || ent->client->NPC_class == CLASS_MARK2 || ent->client->NPC_class == CLASS_GALAKMECH || ent->client->NPC_class == CLASS_MOUSE || ent->client->NPC_class == CLASS_PROBE // droid
		|| ent->client->NPC_class == CLASS_PROTOCOL																																																																																																			// droid
		|| ent->client->NPC_class == CLASS_R2D2																																																																																																				// droid
		|| ent->client->NPC_class == CLASS_R5D2																																																																																																				// droid
		|| ent->client->NPC_class == CLASS_REMOTE || ent->client->NPC_class == CLASS_SEEKER																																																																																									// droid
		|| ent->client->NPC_class == CLASS_SENTRY || ent->client->NPC_class == CLASS_SWAMPTROOPER || ent->client->NPC_class == CLASS_TUSKEN || ent->client->NPC_class == CLASS_BOBAFETT || ent->client->NPC_class == CLASS_ROCKETTROOPER || ent->client->NPC_class == CLASS_SABER_DROID || ent->client->NPC_class == CLASS_ASSASSIN_DROID || ent->client->NPC_class == CLASS_HAZARD_TROOPER || ent->client->NPC_class == CLASS_VEHICLE)
	{
		return qtrue;
	}
	return qfalse;
...
void G_Damage(gentity_t *targ, gentity_t *inflictor, gentity_t *attacker, const vec3_t dir, const vec3_t point, int damage, int dflags, int mod, int hitLoc)
{
	...
	if (targ->s.eFlags & EF_LOCKED_TO_WEAPON && targ->s.number == 0 && targ->owner && !targ->owner->bounceCount //not an EWeb
	...
	if ((targ->svFlags & SVF_BBRUSH) || (!targ->client && Q_stricmp("misc_model_breakable", targ->classname) == 0)) //FIXME: flag misc_model_breakables?
	{																												//breakable brush or misc_model_breakable
		if (targ->NPC_targetname)
		{ //only a certain attacker can destroy this
			if (!attacker || !attacker->targetname || Q_stricmp(targ->NPC_targetname, attacker->targetname) != 0)
	...
	if (targ->client && targ->client->NPC_class == CLASS_ATST)
	{
		...
		if (mod == MOD_SNIPER || mod == MOD_DISRUPTOR || mod == MOD_CONC_ALT)
		{
			// disruptor does not hurt an atst
			return;
		}
	}
	if (targ->client && targ->client->NPC_class == CLASS_RANCOR && (!attacker || !attacker->client || attacker->client->NPC_class != CLASS_RANCOR))
	{
	...
	{ //sabers do less damage to mark1's and atst's, and to hazard troopers and assassin droids
		if (targ->client)
		{
			if (targ->client->NPC_class == CLASS_ATST || targ->client->NPC_class == CLASS_MARK1)
			{
	...
	if (client && client->NPC_class == CLASS_GALAKMECH && !(dflags & DAMAGE_NO_PROTECTION))
	{ //hit Galak
		if (client->ps.stats[STAT_ARMOR] > 0)
		{								//shields are up
			dflags &= ~DAMAGE_NO_ARMOR; //always affect armor
			if (mod == MOD_ELECTROCUTE || mod == MOD_DEMP2 || mod == MOD_DEMP2_ALT)
		...
		{ //shields down
			...
			if ((dflags & DAMAGE_RADIUS) || mod == MOD_REPEATER_ALT || mod == MOD_FLECHETTE_ALT || mod == MOD_ROCKET || mod == MOD_ROCKET_ALT || mod == MOD_CONC || mod == MOD_THERMAL || mod == MOD_THERMAL_ALT || mod == MOD_DETPACK || mod == MOD_LASERTRIP || mod == MOD_LASERTRIP_ALT || mod == MOD_EXPLOSIVE_SPLASH || mod == MOD_ENERGY_SPLASH || mod == MOD_SABER)
	...
	if (targ && targ->client && !(dflags & DAMAGE_NO_PROTECTION) && !(dflags & DAMAGE_DIE_ON_IMPACT)) //falling to you death ignores force protect and force rage (but obeys godmode and undying flags)
	{																								  //force protections
		//rage
		...
		if ((targ->client->ps.forcePowersActive & (1 << FP_PROTECT)))
		{
			...
			*/
			...
			if (mod == MOD_FALLING && targ->NPC && (targ->NPC->aiFlags & NPCAI_DIE_ON_IMPACT))
			...
			else
			{
				...
				case MOD_MELEE:
					...
					case FORCE_LEVEL_4:
						//je suis invincible!!!
						if (targ->client && attacker->client && targ->client->playerTeam == attacker->client->playerTeam && (!targ->NPC || !targ->NPC->charmedTime))
	...
	if (targ->client && (targ->client->ps.forcePowersActive & (1 << FP_PROTECT)) && targ->client->ps.forcePowerLevel[FP_PROTECT] == FORCE_LEVEL_3)
	...
	if (((targ->flags & FL_GODMODE) || (targ->client && targ->client->ps.powerups[PW_INVINCIBLE] > level.time)) && !(dflags & DAMAGE_NO_PROTECTION))
	{
		if (targ->client && attacker->client && targ->client->playerTeam == attacker->client->playerTeam && (!targ->NPC || !targ->NPC->charmedTime))
		{ //complain, but don't turn on them
			G_FriendlyFireReaction(targ, attacker, dflags);
	...
	*/
	if (client)
	{
		...
		if (targ->client->NPC_class == CLASS_VEHICLE)
		{
			...
			if ((targ->m_pVehicle->m_iArmor <= 0) && targ->m_pVehicle->m_pVehicleInfo->type != VH_ANIMAL)
	...
	// Undying If:
	//--------------------------------------------------------------------------
	qboolean targUndying = (!alreadyDead && !(dflags & DAMAGE_NO_PROTECTION) && ((targ->flags & FL_UNDYING) || (dflags & DAMAGE_NO_KILL) || ((targ->client) && (targ->client->ps.forcePowersActive & (1 << FP_RAGE)) && !(dflags & DAMAGE_NO_PROTECTION) && !(dflags & DAMAGE_DIE_ON_IMPACT))));

	if (targ->client && targ->client->NPC_class == CLASS_WAMPA && targ->count && take >= targ->health)
	{ //wampa holding someone, don't die unless you can release them!
		qboolean removeArm = qfalse;
		if (targ->activator && attacker == targ->activator && mod == MOD_SABER)
		{
			removeArm = qtrue;
		}
	...
	if (take || (dflags & DAMAGE_NO_DAMAGE))
	{
		...
		{ //two clients
			...
			if (targTeam != attackerTeam || (targ->s.number < MAX_CLIENTS && targTeam == TEAM_FREE) //evil player hit

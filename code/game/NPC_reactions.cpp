...
void NPC_ChoosePainAnimation(gentity_t *self, gentity_t *other, const vec3_t point, int damage, int mod, int hitLoc, int voiceEvent = -1)
{
	...
	else
	{
		...
		{
			if (self->client->ps.weapon == WP_SABER && other->s.number < MAX_CLIENTS)
	...
	if (random() < pain_chance)
	{
		...
		{ //not being force-gripped or force-drained
			if (G_CheckForStrongAttackMomentum(self) || PM_SpinningAnim(self->client->ps.legsAnim) || PM_SaberInSpecialAttack(self->client->ps.torsoAnim) || PM_InKnockDown(&self->client->ps) || PM_RollingAnim(self->client->ps.legsAnim) || (PM_FlippingAnim(self->client->ps.legsAnim) && !PM_InCartwheel(self->client->ps.legsAnim)))
...
{
	if (enemy->e_UseFunc == useF_emplaced_gun_use || enemy->e_UseFunc == useF_eweb_use)
	{
		if (enemy->activator && enemy->activator->client)
...
*/
		...
		//		}
				 ...
				 && activator->s.number == 0 && !gi.VoiceVolume[self->s.number] && !(self->NPC->scriptFlags & SCF_NO_RESPONSE))
		{ //I don't have an enemy and I'm not talking and I was used by the player
			NPC_UseResponse(self, other, qfalse);
		}
	}

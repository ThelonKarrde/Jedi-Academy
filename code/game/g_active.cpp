...
qboolean G_ValidateLookEnemy(gentity_t *self, gentity_t *enemy)
{
	...
	{ //not valid
		if ((enemy->svFlags & SVF_NONNPC_ENEMY) && enemy->s.weapon == WP_TURRET && enemy->noDamageTeam != self->client->playerTeam && enemy->health > 0)
		{	//a turret
			//return qtrue;
		}
		else
	...
	if ((!InFront(enemy->currentOrigin, self->currentOrigin, self->client->ps.viewangles, 0.0f) || !G_ClearLOS(self, self->client->renderInfo.eyePoint, enemy)) && (DistanceHorizontalSquared(enemy->currentOrigin, self->currentOrigin) > 65536 || fabs(enemy->currentOrigin[2] - self->currentOrigin[2]) > 384))
...
void DoImpact(gentity_t *self, gentity_t *other, qboolean damageSelf, trace_t *trace)
{
	...
	if (!self->client || self->client->ps.lastOnGround + 300 < level.time || (self->client->ps.lastOnGround + 100 < level.time))
	{
		...
		else if (self->client && (PM_InKnockDown(&self->client->ps) || (self->client->ps.eFlags & EF_FORCE_GRIPPED)) && magnitude >= 120)
	...
	*/
				...
				else if (other->takedamage)
				{
					...
					{ //still alive?
						...
						if (other->client)
						{
							...
							else if (self->forcePuller != ENTITYNUM_NONE && g_entities[self->forcePuller].client && self->mass > Q_irand(50, 100))
							{
		...
		if (damageSelf && self->takedamage && !(self->flags & FL_NO_IMPACT_DMG))
		{
			...
			if ((magnitude >= 100 + self->health && self->s.number >= MAX_CLIENTS && self->s.weapon != WP_SABER) || self->client->NPC_class == CLASS_VEHICLE || (magnitude >= 700)) //health here is used to simulate structural integrity
			{
...
gentity_t *G_KickTrace(gentity_t *ent, vec3_t kickDir, float kickDist, vec3_t kickEnd, int kickDamage, float kickPush, qboolean doSoundOnWalls)
{
	...
	if (trace.fraction < 1.0f && !trace.startsolid && !trace.allsolid && trace.entityNum < ENTITYNUM_NONE)
	{
		...
		{ //we hit an entity
			if (hitEnt->client)
			{
				if (!(hitEnt->client->ps.pm_flags & PMF_TIME_KNOCKBACK) && TIMER_Done(hitEnt, "kickedDebounce")) //not already flying through air?  Intended to stop multiple hits, but...
				{																								 //FIXME: this should not always work
					...
					else if (ent->client->ps.legsAnim == BOTH_GETUP_BROLL_B || ent->client->ps.legsAnim == BOTH_GETUP_BROLL_F || ent->client->ps.legsAnim == BOTH_GETUP_FROLL_B || ent->client->ps.legsAnim == BOTH_GETUP_FROLL_F)
					{
						if (hitEnt->health > 0)
						{ //knock down
...
qboolean G_GrabClient(gentity_t *ent, usercmd_t *ucmd)
{
	...
	for (i = 0; i < numEnts; i++)
	{
		...
		if ((radiusEnts[i]->client->ps.eFlags & EF_HELD_BY_RANCOR) || (radiusEnts[i]->client->ps.eFlags & EF_HELD_BY_WAMPA) || (radiusEnts[i]->client->ps.eFlags & EF_HELD_BY_SAND_CREATURE))
		{ //can't be one being held
			continue;
		}

		if (PM_LockedAnim(radiusEnts[i]->client->ps.torsoAnim) || PM_LockedAnim(radiusEnts[i]->client->ps.legsAnim))
		{ //don't interrupt
			continue;
		}
...
qboolean G_PullAttack(gentity_t *ent, usercmd_t *ucmd)
{
	...
	if (ent->client->ps.torsoAnim == BOTH_PULL_IMPALE_STAB || ent->client->ps.torsoAnim == BOTH_PULL_IMPALE_SWING)
	{ //pulling
		if (ent->NPC)
		{
	...
	else if (ent->client->ps.torsoAnim == BOTH_PULLED_INAIR_B || ent->client->ps.torsoAnim == BOTH_PULLED_INAIR_F)
	{ //being pulled
...
qboolean G_CheckClampUcmd(gentity_t *ent, usercmd_t *ucmd)
{
	...
	if (ent->client->NPC_class == CLASS_PROTOCOL || ent->client->NPC_class == CLASS_R2D2 || ent->client->NPC_class == CLASS_R5D2 || ent->client->NPC_class == CLASS_GONK || ent->client->NPC_class == CLASS_MOUSE)
	...
	if (ent->client->ps.pullAttackEntNum < ENTITYNUM_WORLD && ent->client->ps.pullAttackTime > level.time)
	{
		return G_PullAttack(ent, ucmd);
	}

	if ((ent->s.number < MAX_CLIENTS || G_ControlledByPlayer(ent)) && ent->s.weapon == WP_MELEE && ent->client->ps.torsoAnim == BOTH_KYLE_GRAB)
	...
	if ((!ent->s.number && ent->aimDebounceTime > level.time) || (ent->client->ps.pm_time && (ent->client->ps.pm_flags & PMF_TIME_KNOCKBACK)) || ent->forcePushTime > level.time)
	...
	if (ent->client->ps.legsAnim == BOTH_FORCEWALLRUNFLIP_ALT && ent->client->ps.legsAnimTimer)
	{
	...
	if (ent->client->ps.saberMove == LS_A_BACKFLIP_ATK && ent->client->ps.legsAnim == BOTH_JUMPATTACK7)
	{																														  //backflip attack
	...
	if (ent->client->ps.legsAnim == BOTH_JUMPATTACK6 && ent->client->ps.legsAnimTimer > 0)
	{
		ucmd->forwardmove = ucmd->rightmove = ucmd->upmove = 0;
		...
		if ((ent->client->ps.legsAnimTimer >= 900																														 //not at end
			...
			|| (ent->client->ps.legsAnimTimer >= 1600 && PM_AnimLength(ent->client->clientInfo.animFileIndex, BOTH_JUMPATTACK6) - ent->client->ps.legsAnimTimer >= 400)) //not in beginning
		...
		if ((ent->client->ps.legsAnimTimer >= 1450 && PM_AnimLength(ent->client->clientInfo.animFileIndex, BOTH_JUMPATTACK6) - ent->client->ps.legsAnimTimer >= 400) || (ent->client->ps.legsAnimTimer >= 400 && PM_AnimLength(ent->client->clientInfo.animFileIndex, BOTH_JUMPATTACK6) - ent->client->ps.legsAnimTimer >= 1100))
	...
	{ //forward flip/spin attack
		...
		{
			//FIXME: don't slide off people/obstacles?
			if (ent->client->ps.legsAnim != BOTH_BUTTERFLY_LEFT && ent->client->ps.legsAnim != BOTH_BUTTERFLY_RIGHT)
			{
				if (ent->client->ps.legsAnimTimer >= 100																									//not at end
	...
	if (ent->client->ps.legsAnim == BOTH_A7_SOULCAL && ent->client->ps.saberMove == LS_STAFF_SOULCAL)
	{ //forward spinning staff attack
		...
		if (ent->client->ps.legsAnimTimer >= 2650 && ent->client->ps.legsAnimTimer < 2850)
		{ //the jump
			if (ent->client->ps.groundEntityNum != ENTITYNUM_NONE)
			{ //still on ground?
				//jump!
	...
	if (ent->client->ps.torsoAnim == BOTH_A6_FB || ent->client->ps.torsoAnim == BOTH_A6_LR)
	...
	else if (ent->client->ps.torsoAnim == BOTH_FORCE_DRAIN_GRAB_START || ent->client->ps.torsoAnim == BOTH_FORCE_DRAIN_GRAB_HOLD || ent->client->ps.torsoAnim == BOTH_FORCE_DRAIN_GRAB_END || ent->client->ps.legsAnim == BOTH_FORCE_DRAIN_GRABBED)
	{ //can't turn or move
		if (ent->s.number < MAX_CLIENTS || G_ControlledByPlayer(ent))
		{ //player
	...
	else if (ent->client->ps.legsAnim == BOTH_PLAYER_PA_1 || ent->client->ps.legsAnim == BOTH_PLAYER_PA_2 || ent->client->ps.legsAnim == BOTH_PLAYER_PA_3 || ent->client->ps.legsAnim == BOTH_PLAYER_PA_3_FLY || ent->client->ps.legsAnim == BOTH_KYLE_PA_1 || ent->client->ps.legsAnim == BOTH_KYLE_PA_2 || ent->client->ps.legsAnim == BOTH_KYLE_PA_3)
	{ //can't turn during Kyle's grappling
		if (ent->NPC)
		{
			VectorClear(ent->client->ps.moveDir);
		...
		{ //actually do some damage during sequence
			...
			if (TIMER_Done(ent, "grappleDamageDebounce"))
			{
				...
				case BOTH_PLAYER_PA_1:
					if (ent->client->ps.legsAnimTimer <= 3150 && ent->client->ps.legsAnimTimer > 3050)
					{
					...
					if (ent->client->ps.legsAnimTimer <= 1150 && ent->client->ps.legsAnimTimer > 10500)
					{
						TIMER_Set(ent, "grappleDamageDebounce", 150);
						if (ent->s.number < MAX_CLIENTS)
						{ //special case
				...
				case BOTH_PLAYER_PA_2:
					if (ent->client->ps.legsAnimTimer <= 5700 && ent->client->ps.legsAnimTimer > 5600)
					{
						TIMER_Set(ent, "grappleDamageDebounce", 150);
						if (ent->s.number < MAX_CLIENTS)
						{ //special case
					...
					if (ent->client->ps.legsAnimTimer <= 5200 && ent->client->ps.legsAnimTimer > 5100)
					{
						TIMER_Set(ent, "grappleDamageDebounce", 150);
						if (ent->s.number < MAX_CLIENTS)
						{ //special case
					...
					if (ent->client->ps.legsAnimTimer <= 4550 && ent->client->ps.legsAnimTimer > 4450)
					{
						TIMER_Set(ent, "grappleDamageDebounce", 150);
						if (ent->s.number < MAX_CLIENTS)
						{ //special case
					...
					if (ent->client->ps.legsAnimTimer <= 3550 && ent->client->ps.legsAnimTimer > 3450)
					{
						TIMER_Set(ent, "grappleDamageDebounce", 150);
						if (ent->s.number < MAX_CLIENTS)
						{ //special case
				...
				case BOTH_PLAYER_PA_3:
					if (ent->client->ps.legsAnimTimer <= 3800 && ent->client->ps.legsAnimTimer > 3700)
					{
						TIMER_Set(ent, "grappleDamageDebounce", 150);
						if (ent->s.number < MAX_CLIENTS)
						{ //special case
					...
					if (ent->client->ps.legsAnimTimer <= 3100 && ent->client->ps.legsAnimTimer > 3000)
					{
						TIMER_Set(ent, "grappleDamageDebounce", 150);
						if (ent->s.number < MAX_CLIENTS)
						{ //special case
					...
					if (ent->client->ps.legsAnimTimer <= 1650 && ent->client->ps.legsAnimTimer > 1550)
					{
						TIMER_Set(ent, "grappleDamageDebounce", 150);
						if (ent->s.number < MAX_CLIENTS)
						{ //special case
		...
		if (ent->s.number < MAX_CLIENTS && ent->client->ps.legsAnim == BOTH_PLAYER_PA_3_FLY)
		...
		if (d_slowmodeath->integer <= 3 && ent->s.number < MAX_CLIENTS)
		{ //no matrix effect, so slide the camera back and to the side
			G_CamPullBackForLegsAnim(ent);
			G_CamCircleForLegsAnim(ent);
		}
	}
	else if (ent->client->ps.legsAnim == BOTH_FORCELONGLEAP_START || ent->client->ps.legsAnim == BOTH_FORCELONGLEAP_ATTACK || ent->client->ps.legsAnim == BOTH_FORCELONGLEAP_LAND)
	...
	else if (PM_KickingAnim(ent->client->ps.legsAnim) || ent->client->ps.legsAnim == BOTH_ARIAL_F1 || ent->client->ps.legsAnim == BOTH_ARIAL_LEFT || ent->client->ps.legsAnim == BOTH_ARIAL_RIGHT || ent->client->ps.legsAnim == BOTH_CARTWHEEL_LEFT || ent->client->ps.legsAnim == BOTH_CARTWHEEL_RIGHT || ent->client->ps.legsAnim == BOTH_JUMPATTACK7 || ent->client->ps.legsAnim == BOTH_BUTTERFLY_FL1 || ent->client->ps.legsAnim == BOTH_BUTTERFLY_FR1 || ent->client->ps.legsAnim == BOTH_BUTTERFLY_RIGHT || ent->client->ps.legsAnim == BOTH_BUTTERFLY_LEFT || ent->client->ps.legsAnim == BOTH_A7_SOULCAL)
	{ //can't move, check for damage frame
		...
		else
		{
			...
			case BOTH_BUTTERFLY_RIGHT:
				if ((elapsedTime >= 100 && elapsedTime <= 450) || (elapsedTime >= 1100 && elapsedTime <= 1350))
				{																										//lead leg
			...
			case BOTH_GETUP_FROLL_F:
				...
				if (ent->client->ps.legsAnim == BOTH_GETUP_BROLL_B || ent->client->ps.legsAnim == BOTH_GETUP_FROLL_B)
	...
	else if (ent->client->ps.saberMove == LS_A_BACK || ent->client->ps.saberMove == LS_A_BACK_CR || ent->client->ps.saberMove == LS_A_BACKSTAB)
	...
	else if (ent->client->ps.torsoAnim == BOTH_WALL_FLIP_BACK1 || ent->client->ps.torsoAnim == BOTH_WALL_FLIP_BACK2 || ent->client->ps.legsAnim == BOTH_FORCEWALLRUNFLIP_END || ent->client->ps.legsAnim == BOTH_FORCEWALLREBOUND_BACK)
	{
		//pull back the view
		G_CamPullBackForLegsAnim(ent);
	}
	...
	{ //push forward
		...
		if (ent->client->ps.legsAnimTimer > 200.0f)
		{
			float animLength = PM_AnimLength(ent->client->clientInfo.animFileIndex, (animNumber_t)ent->client->ps.torsoAnim);
			float elapsedTime = (float)(animLength - ent->client->ps.torsoAnimTimer);
			if (elapsedTime < 750 || (elapsedTime >= 1650 && elapsedTime < 2400))
			{ //push forward
				ucmd->forwardmove = 64;
				ent->client->ps.speed = 200.0f;
			}
	...
	else if (ent->client->ps.legsAnim == BOTH_A1_SPECIAL && (ucmd->forwardmove || ucmd->rightmove || (VectorCompare(ent->client->ps.moveDir, vec3_origin) && ent->client->ps.speed > 0)))
	...
	{ //moving during full-body fast special
		...
		if (elapsedTime > 600 && elapsedTime < 800 && ent->client->ps.groundEntityNum != ENTITYNUM_NONE)
	...
	else if ((PM_SuperBreakWinAnim(ent->client->ps.torsoAnim) || PM_SuperBreakLoseAnim(ent->client->ps.torsoAnim)) && ent->client->ps.torsoAnimTimer)
	{ //can't move or turn
		ucmd->forwardmove = ucmd->rightmove = ucmd->upmove = 0;
		if (ent->NPC)
		{
	...
	else if (BG_FullBodyTauntAnim(ent->client->ps.legsAnim) && BG_FullBodyTauntAnim(ent->client->ps.torsoAnim))
	{
		if ((ucmd->buttons & BUTTON_ATTACK) || (ucmd->buttons & BUTTON_ALT_ATTACK) || (ucmd->buttons & BUTTON_USE_FORCE) || (ucmd->buttons & BUTTON_FORCEGRIP) || (ucmd->buttons & BUTTON_FORCE_LIGHTNING) || (ucmd->buttons & BUTTON_FORCE_DRAIN) || ucmd->upmove)
		{ //stop the anim
...
void G_CheckClientIdle(gentity_t *ent, usercmd_t *ucmd)
{
	...
	if (!VectorCompare(vec3_origin, ent->client->ps.velocity) || ucmd->buttons || ucmd->forwardmove || ucmd->rightmove || ucmd->upmove || !PM_StandingAnim(ent->client->ps.legsAnim) || ent->enemy || ent->client->ps.legsAnimTimer || ent->client->ps.torsoAnimTimer)
	{ //FIXME: also check for turning?
		if (!VectorCompare(vec3_origin, ent->client->ps.velocity) || ucmd->buttons || ucmd->forwardmove || ucmd->rightmove || ucmd->upmove || ent->enemy)
		{
...
void G_CheckMovingLoopingSounds(gentity_t *ent, usercmd_t *ucmd)
{
	if (ent->client)
	{
		...
		{ //not moving under your own control, stop loopSound
			if (ent->client->NPC_class == CLASS_R2D2 || ent->client->NPC_class == CLASS_R5D2 || ent->client->NPC_class == CLASS_MARK2 || ent->client->NPC_class == CLASS_MOUSE || ent->client->NPC_class == CLASS_PROBE)
			{
				ent->s.loopSound = 0;
			}
		}
	...
	{ //Client sets ucmds and such for speed alterations
		{
			...
			if (ent->client->ps.heldClient < ENTITYNUM_WORLD)
			{
...
void ClientThink_real(gentity_t *ent, usercmd_t *ucmd)
{
	...
	if ((ent->s.eFlags & EF_HELD_BY_RANCOR) || (ent->s.eFlags & EF_HELD_BY_WAMPA))
	{
	...
	if (ent->s.number == 0)
	{
		...
		if (cg.zoomMode == 2)
		{
			...
			if (((ucmd->forwardmove || ucmd->rightmove) && ucmd->upmove >= 0 //crouching-moving is ok
		...
		if ((player_locked || (ent->client->ps.eFlags & EF_FORCE_GRIPPED) || (ent->client->ps.eFlags & EF_FORCE_DRAINED) || (ent->client->ps.legsAnim == BOTH_PLAYER_PA_1) || (ent->client->ps.legsAnim == BOTH_PLAYER_PA_2) || (ent->client->ps.legsAnim == BOTH_PLAYER_PA_3)) && ent->client->ps.pm_type < PM_DEAD) // unless dead
	...
	if (pVeh && (pVeh->m_pVehicleInfo->Inhabited(pVeh) || pVeh->m_iBoarding != 0 || pVeh->m_pVehicleInfo->type != VH_ANIMAL))
	{
			...
			if (groundEnt && groundEnt->client)
			{
				...
				if (groundEnt->client && groundEnt->client->NPC_class == CLASS_VEHICLE)
				{
					...
					{ //um... vehicles shouldn't ride other vehicles, mmkay?
						if ((groundEnt->m_pVehicle->m_pVehicleInfo->type == VH_ANIMAL && PM_HasAnimation(ent, BOTH_VT_IDLE)) || (groundEnt->m_pVehicle->m_pVehicleInfo->type == VH_SPEEDER && PM_HasAnimation(ent, BOTH_VS_IDLE)))
						{
				...
				else if (groundEnt->client && groundEnt->client->NPC_class == CLASS_SAND_CREATURE && G_HasKnockdownAnims(ent))
				...
				{ //step on a Rancor, it bucks & throws you off
					if (groundEnt->client->ps.legsAnim != BOTH_ATTACK3 && groundEnt->client->ps.legsAnim != BOTH_ATTACK4 && groundEnt->client->ps.legsAnim != BOTH_BUCK_RIDER)
					{ //don't interrupt special anims
						...
						NPC_SetAnim(groundEnt, SETANIM_BOTH, BOTH_BUCK_RIDER, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
						 ...
						 groundEnt->health > 0 && !PM_InRoll(&groundEnt->client->ps) && !(groundEnt->client->ps.eFlags & EF_LOCKED_TO_WEAPON) && !(groundEnt->client->ps.eFlags & EF_HELD_BY_RANCOR) && !(groundEnt->client->ps.eFlags & EF_HELD_BY_WAMPA) && !(groundEnt->client->ps.eFlags & EF_HELD_BY_SAND_CREATURE) && !inSpinFlipAttack)
				...
				{ //landed on a live client who is on the ground, jump off them and knock them down
					...
					else if (ent->s.number && ent->NPC && ent->client->ps.forcePowerLevel[FP_LEVITATION] > FORCE_LEVEL_0) //ent->s.weapon == WP_SABER )
					...
					{
						if (!PM_InRoll(&ent->client->ps) && !PM_FlippingAnim(ent->client->ps.legsAnim))
						{
					...
					if (!forceKnockdown && groundEnt->client->NPC_class == CLASS_DESANN && ent->client->NPC_class != CLASS_LUKE)
	...
	if (client->ps.stats[STAT_HEALTH] <= 0)
	{
		...
		if (ent && !ent->s.number && ent->enemy && ent->enemy != ent && ent->enemy->s.number < ENTITYNUM_WORLD && ent->enemy->inuse && !(cg.overrides.active & CG_OVERRIDE_3RD_PERSON_ANG))
		{ //keep facing enemy
			vec3_t deadDir;
			float deadYaw;

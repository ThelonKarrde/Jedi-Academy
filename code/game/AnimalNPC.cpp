...
void CAnimalNPC::ProcessOrientCommands()
{
	...
	else
	{
		...
		if (!m_pVehicleInfo->turnWhenStopped && !m_pParentEntity->client->ps.speed) //FIXME: or !m_ucmd.forwardmove?
		...
		if (m_pPilot->client->ps.weapon != WP_NONE && m_pPilot->client->ps.weapon != WP_MELEE)
		{
			...
			if (m_ucmd.buttons & (BUTTON_ATTACK | BUTTON_ALT_ATTACK))
			{
				...
				case WP_THERMAL:
					...
					if (m_pPilot->client->ps.torsoAnim == BOTH_THERMAL_THROW || m_pPilot->client->ps.torsoAnim == BOTH_ATTACK10)
					{
						if (m_ucmd.rightmove > 0) //right side attack
						{
							Anim = BOTH_VT_ATR_T;

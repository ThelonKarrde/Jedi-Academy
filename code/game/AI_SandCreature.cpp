...
void SandCreature_MoveEffect(void)
{
	...
	if (TIMER_Done(NPC, "breaching") && TIMER_Done(NPC, "breachDebounce") && TIMER_Done(NPC, "pain") && TIMER_Done(NPC, "attacking") && !Q_irand(0, 10))
	{ //Breach!
		...
		if (NPC->enemy && NPC->enemy->client)
		{
			...
			if (g_spskill->integer > 0)
			{
				...
				if (enemyDist < 200 && NPC->enemy->client->ps.groundEntityNum != ENTITYNUM_NONE)
				{
					...
					{ //knock them down, too
						if (NPC->enemy->health > 0 && Q_flrand(50, 150) > enemyDist)
						{ //knock them down
...
void SandCreature_CheckMovingEnts(void)
{
	...
	for (i = 0; i < numEnts; i++)
	{
		...
		{ //must be a client
			if (radiusEnts[i]->s.eType != ET_MISSILE || radiusEnts[i]->s.weapon != WP_THERMAL)
...
void SandCreature_Chase(void)
{
	...
	if (NPC->enemy->client)
	{
		if ((NPC->enemy->client->ps.eFlags & EF_HELD_BY_SAND_CREATURE) || (NPC->enemy->client->ps.eFlags & EF_HELD_BY_RANCOR) || (NPC->enemy->client->ps.eFlags & EF_HELD_BY_WAMPA))
	...
	//chase the enemy
	if (NPC->enemy->client && NPC->enemy->client->ps.groundEntityNum != ENTITYNUM_WORLD && !(NPC->svFlags & SVF_LOCKEDENEMY))
	{	//off the ground!
	...
	else
	{
		...
		if (enemyScore < MIN_SCORE && !(NPC->svFlags & SVF_LOCKEDENEMY))
	...
	if ((level.time - NPCInfo->enemyLastSeenTime) > 5000 && !(NPC->svFlags & SVF_LOCKEDENEMY))
	...
	else if ((level.time - NPCInfo->enemyLastSeenTime) <= 5000 && !(NPC->svFlags & SVF_LOCKEDENEMY))
	...
	else if (enemyDistSq < MAX_MISS_DIST_SQ && enemyDistSq > MIN_MISS_DIST_SQ && NPC->enemy->client && TIMER_Done(NPC, "breaching") && TIMER_Done(NPC, "missDebounce") && !VectorCompare(NPC->pos1, NPC->currentOrigin) //so we don't come up again in the same spot
			 && !Q_irand(0, 10))
	{
		if (!(NPC->svFlags & SVF_LOCKEDENEMY))
		{
...
void SandCreature_Hunt(void)
{
	...
	//FIXME: keeps chasing goalEntity even when it's already reached it...?
	if (NPCInfo->goalEntity && SandCreature_DistSqToGoal(qfalse) >= MIN_ATTACK_DIST_SQ)
	{
		ucmd.buttons |= BUTTON_WALKING;
		if (SandCreature_Move())
		{

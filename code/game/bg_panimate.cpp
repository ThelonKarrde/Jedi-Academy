...
int PM_PowerLevelForSaberAnim(playerState_t *ps, int saberNum)
{
	...
	if ((anim >= BOTH_P1_S1_T_ && anim <= BOTH_P1_S1_BR) || (anim >= BOTH_P6_S6_T_ && anim <= BOTH_P6_S6_BR) || (anim >= BOTH_P7_S7_T_ && anim <= BOTH_P7_S7_BR))
	{ //parries
	...
	if ((anim >= BOTH_K1_S1_T_ && anim <= BOTH_K1_S1_BR) || (anim >= BOTH_K6_S6_T_ && anim <= BOTH_K6_S6_BR) || (anim >= BOTH_K7_S7_T_ && anim <= BOTH_K7_S7_BR))
	{ //knockaways
		return FORCE_LEVEL_3;
	}
	if ((anim >= BOTH_V1_BR_S1 && anim <= BOTH_V1_B__S1) || (anim >= BOTH_V6_BR_S6 && anim <= BOTH_V6_B__S6) || (anim >= BOTH_V7_BR_S7 && anim <= BOTH_V7_B__S7))
	{ //knocked-away attacks
		return FORCE_LEVEL_1;
	}
	if ((anim >= BOTH_H1_S1_T_ && anim <= BOTH_H1_S1_BR) || (anim >= BOTH_H6_S6_T_ && anim <= BOTH_H6_S6_BR) || (anim >= BOTH_H7_S7_T_ && anim <= BOTH_H7_S7_BR))
	{ //broken parries
		return FORCE_LEVEL_0;
	}
	switch (anim)
	...
	case BOTH_JUMPATTACK6:
		...
		*/
		if ((ps->torsoAnimTimer >= 1450 && animTimeElapsed >= 400) || (ps->torsoAnimTimer >= 400 && animTimeElapsed >= 1100))
		{ //pretty much sideways
			return FORCE_LEVEL_3;
		}
		return FORCE_LEVEL_0;
...
qboolean PM_SaberKataDone(int curmove = LS_NONE, int newmove = LS_NONE)
{
	...
	{ //FIXME: have chainAngle influence fast and medium chains as well?
		if ((pm->ps->saberAnimLevel == FORCE_LEVEL_2 || pm->ps->saberAnimLevel == SS_DUAL) && pm->ps->saberAttackChainCount > Q_irand(2, 5))
		{
			return qtrue;
		}
	}
...
qboolean PM_CheckEnemyInBack(float backCheckDist)
{
	...
	if ((pm->ps->clientNum < MAX_CLIENTS || PM_ControlledByPlayer()) && !g_saberAutoAim->integer && pm->cmd.forwardmove >= 0)
	{ //don't auto-backstab
		return qfalse;
	}
...
saberMoveName_t PM_PickBackStab(void)
{
	...
	else if (pm->ps->saberAnimLevel == FORCE_LEVEL_2 || pm->ps->saberAnimLevel == SS_DUAL)
...
saberMoveName_t PM_CheckStabDown(void)
{
	...
	{ //guy is on the ground below me, do a top-down attack
		if (pm->gent->enemy->s.number >= MAX_CLIENTS || !G_ControlledByPlayer(pm->gent->enemy))
		...
		//pick the right anim
		if (pm->ps->saberAnimLevel == SS_DUAL || (pm->ps->dualSabers && pm->ps->saber[1].Active()))
		{
...
saberMoveName_t PM_AttackForEnemyPos(qboolean allowFB, qboolean allowStabDown)
{
	...
	{ //enemy is in front
		...
		if ((pm->ps->clientNum < MAX_CLIENTS || PM_ControlledByPlayer()) && dot > 0.65f && enemyDist <= 64 && pm->gent->enemy->client && (enemyZDiff <= 20 || PM_InKnockDownOnGround(&pm->gent->enemy->client->ps) || PM_CrouchAnim(pm->gent->enemy->client->ps.legsAnim)))
		{ //swing down at them
			return LS_A_T2B;
		}
		if (allowFB)
	...
	{ //back attack allowed
		...
		{ //only when on ground
			...
			{ //enemy not a client or is a client and on ground
				if (dot < -0.75f && enemyDist < 128 && (pm->ps->saberAnimLevel == SS_FAST || pm->ps->saberAnimLevel == SS_STAFF || (pm->gent->client && (pm->gent->client->NPC_class == CLASS_TAVION || pm->gent->client->NPC_class == CLASS_ALORA) && Q_irand(0, 2))))
				{ //fast back-stab
...
{
	if (pm->ps->saber[0].numBlades > 1 && pm->ps->saber[0].singleBladeStyle && pm->ps->saber[0].singleBladeStyle != pm->ps->saber[0].style && pm->ps->saberAnimLevel == pm->ps->saber[0].singleBladeStyle)
	{
		return qtrue;
	}

	if (pm->ps->dualSabers && !pm->ps->saber[1].Active()) //pm->ps->saberAnimLevel != SS_DUAL )
	{
		return qtrue;
	}
	return qfalse;
...
qboolean PM_CheckLungeAttackMove(void)
{
	if (pm->ps->saberAnimLevel == SS_FAST	  //fast
		...
		|| pm->ps->saberAnimLevel == SS_DESANN || pm->ps->dualSabers)
	{ //alt+back+attack using fast, dual or staff attacks
		if (pm->ps->clientNum >= MAX_CLIENTS && !PM_ControlledByPlayer())
		{ //NPC
			if (pm->cmd.upmove < 0 || (pm->ps->pm_flags & PMF_DUCKED))
			{ //ducking
				if (pm->ps->legsAnim == BOTH_STAND2 || pm->ps->legsAnim == BOTH_SABERFAST_STANCE || pm->ps->legsAnim == BOTH_SABERSLOW_STANCE || pm->ps->legsAnim == BOTH_SABERSTAFF_STANCE || pm->ps->legsAnim == BOTH_SABERDUAL_STANCE || (level.time - pm->ps->lastStationary) <= 500)
				{																								 //standing or just stopped standing
					if (pm->gent && pm->gent->NPC																 //NPC
		...
		{																																												   //player
			if (G_TryingLungeAttack(pm->gent, &pm->cmd) && G_EnoughPowerForSpecialMove(pm->ps->forcePower, SABER_ALT_ATTACK_POWER_FB) /*pm->ps->forcePower >= SABER_ALT_ATTACK_POWER_FB*/) //have enough force power to pull it off
			{
				return qtrue;
			}
		}
	...
	)
	{
		...
		{													//dual and staff
				...
				&& (pm->cmd.buttons & BUTTON_ATTACK))		//want to attack
			{
				...
				{																																	 //PLAYER
					if (G_TryingJumpForwardAttack(pm->gent, &pm->cmd) && G_EnoughPowerForSpecialMove(pm->ps->forcePower, SABER_ALT_ATTACK_POWER_FB)) //have enough power to attack
					{
						return qtrue;
					}
				}
				 ...
				 || pm->ps->saberAnimLevel == SS_DESANN) //desann
		{
			...
			{ //strong attack: jump-hack
				...
				*/
				...
				{																//NPC
					if (pm->cmd.upmove > 0 || (pm->ps->pm_flags & PMF_JUMPING)) //NPC jumping
					{
						...
						{ //only acrobat or boss and higher can do this
							if (pm->ps->legsAnim == BOTH_STAND2 || pm->ps->legsAnim == BOTH_SABERFAST_STANCE || pm->ps->legsAnim == BOTH_SABERSLOW_STANCE || level.time - pm->ps->lastStationary <= 250)
							{ //standing or just started moving
								if (pm->gent->client && pm->gent->client->NPC_class == CLASS_DESANN)
								{
									if (!Q_irand(0, 1))
									{
										return qtrue;
				...
				{ //player
					if (G_TryingJumpForwardAttack(pm->gent, &pm->cmd) && G_EnoughPowerForSpecialMove(pm->ps->forcePower, SABER_ALT_ATTACK_POWER_FB))
					{
						return qtrue;
					}
				}
	...
	)
	{
		...
		{																														  //NPC
			...
			{																													  //flip over-forward down-attack
				if ((pm->gent->NPC && (pm->gent->NPC->rank == RANK_CREWMAN || pm->gent->NPC->rank >= RANK_LT) && !Q_irand(0, 2))) //NPC who can do this, 33% chance
		...
		{																																	 //player
			if (G_TryingJumpForwardAttack(pm->gent, &pm->cmd) && G_EnoughPowerForSpecialMove(pm->ps->forcePower, SABER_ALT_ATTACK_POWER_FB)) //have enough power
			{
				if (!pm->cmd.rightmove)
				{
					if (pm->ps->legsAnim == BOTH_JUMP1 || pm->ps->legsAnim == BOTH_FORCEJUMP1 || pm->ps->legsAnim == BOTH_INAIR1 || pm->ps->legsAnim == BOTH_FORCEINAIR1)
		...
		if (tryMove)
		{
			...
			{						 //based on presence of enemy
				if (pm->gent->enemy) //have an enemy
				{
					...
					if (pm->gent->enemy->health > 0 && pm->ps->forceRageRecoveryTime < pm->cmd.serverTime //not in a force Rage recovery period
						&& pm->gent->enemy->maxs[2] > 12 && (!pm->gent->enemy->client || !PM_InKnockDownOnGround(&pm->gent->enemy->client->ps)) && DistanceSquared(pm->gent->currentOrigin, pm->gent->enemy->currentOrigin) < 10000 && InFront(pm->gent->enemy->currentOrigin, pm->gent->currentOrigin, fwdAngles, 0.3f))
...
saberMoveName_t PM_CheckPullAttack(void)
{
	...
	{																				 //FIXME: some NPC logic to do this?
		...
		if (g_saberNewControlScheme->integer || g_crosshairEntNum < ENTITYNUM_WORLD) //in old control scheme, there has to be someone there
		{
			...
			if (g_crosshairEntNum < ENTITYNUM_WORLD && pm->gent && pm->gent->client)
			{
				...
				if (targEnt->client && targEnt->health > 0
					//FIXME: check other things like in knockdown, saberlock, uninterruptable anims, etc.
					&& !PM_InOnGroundAnim(&targEnt->client->ps) && !PM_LockedAnim(targEnt->client->ps.legsAnim) && !PM_SuperBreakLoseAnim(targEnt->client->ps.legsAnim) && !PM_SuperBreakWinAnim(targEnt->client->ps.legsAnim) && targEnt->client->ps.saberLockTime <= 0 && WP_ForceThrowable(targEnt, targEnt, pm->gent, qtrue, 1.0f, 0.0f, NULL))
				{
...
saberMoveName_t PM_SaberAttackForMovement(int forwardmove, int rightmove, int curmove)
{
...
#endif																																								  // _XBOX
	{																																								  //moving right
		if (!noSpecials && (pm->ps->groundEntityNum != ENTITYNUM_NONE || level.time - pm->ps->lastOnGround <= 250)													  //on ground or just jumped
	...
	{ //not moving left or right
		...
		{ //forward= T2B slash
			...
			{ //I have an active enemy
				...
				{ //a player who is running at an enemy
					...
					if (pm->gent->enemy->s.weapon != WP_SABER && pm->gent->enemy->client->NPC_class != CLASS_REMOTE //too small to do auto-aiming accurately
		...
		{ //backward= T2B slash//B2T uppercut?
			...
			if (g_saberNewControlScheme->integer && (pm->ps->clientNum < MAX_CLIENTS || PM_ControlledByPlayer())									   //PLAYER ONLY
			...
			{																													   //NPC or player in third person, not zoomed
				...
				{ //only when on ground
					...
					{ //FIXME: or just trace for a valid enemy standing behind me?  And no enemy in front?
						...
						if (dot < 0)
						{																										 //enemy is behind me
							if (dot < -0.75f && DistanceSquared(pm->gent->currentOrigin, pm->gent->enemy->currentOrigin) < 16384 //128 squared
		...
		{ //not moving in any direction
			...
			else if (curmove == LS_READY || curmove == LS_A_FLIP_STAB || curmove == LS_A_FLIP_SLASH || (curmove >= LS_PARRY_UP && curmove <= LS_REFLECT_LL))
...
void PM_SaberStartTransAnim(int saberAnimLevel, int anim, float *animSpeed, gentity_t *gent)
{
	...
	if (gent && gent->client && gent->client->ps.stats[STAT_WEAPONS] & (1 << WP_SCEPTER) && gent->client->ps.dualSabers && saberAnimLevel == SS_DUAL && gent->weaponModel[1])
...
float PM_GetTimeScaleMod(gentity_t *gent)
{
	...
	{
		if (!MatrixMode && gent->client->ps.legsAnim != BOTH_FORCELONGLEAP_START && gent->client->ps.legsAnim != BOTH_FORCELONGLEAP_ATTACK && gent->client->ps.legsAnim != BOTH_FORCELONGLEAP_LAND)
		{
					 ...
					 gentity_t *gent, int blendTime) // default blendTime=350
{
	...
	//--------------------------------------------------------------------------------------
	if (g_noFootSlide->integer && animFootMove && !(animSpeed < 0.0f)
		...
		&& gent->client->NPC_class != CLASS_HOWLER && gent->client->NPC_class != CLASS_WAMPA && gent->client->NPC_class != CLASS_GONK && gent->client->NPC_class != CLASS_HOWLER && gent->client->NPC_class != CLASS_MOUSE && gent->client->NPC_class != CLASS_PROBE && gent->client->NPC_class != CLASS_PROTOCOL && gent->client->NPC_class != CLASS_R2D2 && gent->client->NPC_class != CLASS_R5D2 && gent->client->NPC_class != CLASS_SEEKER)
	{
...
{ // FIXME : once torsoAnim and legsAnim are in the same structure for NPC and Players
	...
	{ //don't lock anims if the guy is dead
		if (pm->ps->torsoAnimTimer && PM_LockedAnim(pm->ps->torsoAnim) && !PM_LockedAnim(anim))
		{ //nothing can override these special anims
			setAnimParts &= ~SETANIM_TORSO;
		}

		if (pm->ps->legsAnimTimer && PM_LockedAnim(pm->ps->legsAnim) && !PM_LockedAnim(anim))
		{ //nothing can override these special anims
...
void PM_TorsoAnimLightsaber()
{
	...
	if (pm->ps->saber[0].blade[0].active && pm->ps->saber[0].blade[0].length < 3 && !(pm->ps->saberEventFlags & SEF_HITWALL) && pm->ps->weaponstate == WEAPON_RAISING)
	{
	...
	{ //ready
		...
		{ //saber is on
			...
			{
				if (PM_JumpingAnim(pm->ps->legsAnim) || PM_LandingAnim(pm->ps->legsAnim) || PM_InCartwheel(pm->ps->legsAnim) || PM_FlippingAnim(pm->ps->legsAnim))
				{
					PM_SetAnim(pm, SETANIM_TORSO, pm->ps->legsAnim, SETANIM_FLAG_NORMAL);
				}
				else
				{
					...
					{
						if ((PM_RunningAnim(pm->ps->legsAnim) || pm->ps->legsAnim == BOTH_WALK_STAFF || pm->ps->legsAnim == BOTH_WALK_DUAL || pm->ps->legsAnim == BOTH_WALKBACK_STAFF || pm->ps->legsAnim == BOTH_WALKBACK_DUAL) && pm->ps->saberBlockingTime < cg.time)
	...
	else if (pm->ps->weaponstate == WEAPON_IDLE)
	{
		...
		else if (pm->ps->legsAnim == BOTH_STAND1IDLE1 || pm->ps->legsAnim == BOTH_STAND2IDLE1 || pm->ps->legsAnim == BOTH_STAND2IDLE2 || pm->ps->legsAnim == BOTH_STAND3IDLE1 || pm->ps->legsAnim == BOTH_STAND5IDLE1)
		{
			PM_SetAnim(pm, SETANIM_TORSO, pm->ps->legsAnim, SETANIM_FLAG_NORMAL);
			pm->ps->saberMove = LS_READY;
		}
		...
		else
		{
			...
			if (pm->ps->saberInFlight && saberInAir && (!pm->ps->dualSabers || !pm->ps->saber[1].Active()))
			{
...
{ //FIXME: Write a much smarter and more appropriate anim picking routine logic...
	...
	if (pm->ps->taunting > level.time)
	{
		...
		else if (pm->ps->weapon == WP_SABER && pm->ps->saberAnimLevel == SS_STAFF) //pm->ps->saber[0].type == SABER_STAFF )
		{																		   //turn on the blades
	...
	if (pm->ps->weapon == WP_SABER) // WP_LIGHTSABER
	{
		...
		else
		{
			...
			else
			{
				...
				if (pm->ps->saberInFlight && saberInAir && (!pm->ps->dualSabers							  //not using 2 sabers
	...
	if (PM_ForceAnim(pm->ps->torsoAnim) && pm->ps->torsoAnimTimer > 0)
		...
		pm->ps->weaponstate == WEAPON_CHARGING_ALT)
	{
		...
		{ //Used to default to both_stand1 which is an arms-down anim
			...
			else if (pm->gent != NULL && (pm->gent->s.number < MAX_CLIENTS || G_ControlledByPlayer(pm->gent)) && pm->ps->weaponstate != WEAPON_CHARGING && pm->ps->weaponstate != WEAPON_CHARGING_ALT)
			...
			else
			{
				...
				case WP_BRYAR_PISTOL:
					...
					else if (PM_RunningAnim(pm->ps->legsAnim) || PM_WalkingAnim(pm->ps->legsAnim) || PM_JumpingAnim(pm->ps->legsAnim) || PM_SwimmingAnim(pm->ps->legsAnim))
					{ //running w/1-handed weapon uses full-body anim
						PM_SetAnim(pm, SETANIM_TORSO, pm->ps->legsAnim, SETANIM_FLAG_NORMAL);
					}
					else
				...
				case WP_NONE:
					...
					break;
				case WP_MELEE:
					if (PM_RunningAnim(pm->ps->legsAnim) || PM_WalkingAnim(pm->ps->legsAnim) || PM_JumpingAnim(pm->ps->legsAnim) || PM_SwimmingAnim(pm->ps->legsAnim))
					{ //running w/1-handed weapon uses full-body anim
						PM_SetAnim(pm, SETANIM_TORSO, pm->ps->legsAnim, SETANIM_FLAG_NORMAL);
					}
					else
				...
				case WP_TUSKEN_RIFLE:
					if ((pm->ps->weaponstate != WEAPON_FIRING && pm->ps->weaponstate != WEAPON_CHARGING && pm->ps->weaponstate != WEAPON_CHARGING_ALT) || PM_RunningAnim(pm->ps->legsAnim) || PM_WalkingAnim(pm->ps->legsAnim) || PM_JumpingAnim(pm->ps->legsAnim) || PM_SwimmingAnim(pm->ps->legsAnim))
	...
	else if (pm->ps->weaponstate == WEAPON_IDLE)
	{
		...
		{
			if (!weaponBusy && pm->ps->weapon != WP_BOWCASTER && pm->ps->weapon != WP_REPEATER && pm->ps->weapon != WP_FLECHETTE && pm->ps->weapon != WP_ROCKET_LAUNCHER && pm->ps->weapon != WP_CONCUSSION && (PM_RunningAnim(pm->ps->legsAnim) || (PM_WalkingAnim(pm->ps->legsAnim) && (pm->ps->clientNum < MAX_CLIENTS || PM_ControlledByPlayer())) || PM_JumpingAnim(pm->ps->legsAnim) || PM_SwimmingAnim(pm->ps->legsAnim)))
...
{
	if (PM_HasAnimation(ent, BOTH_KNOCKDOWN1) && PM_HasAnimation(ent, BOTH_KNOCKDOWN2) && PM_HasAnimation(ent, BOTH_KNOCKDOWN3) && PM_HasAnimation(ent, BOTH_KNOCKDOWN4) && PM_HasAnimation(ent, BOTH_KNOCKDOWN5))
	{
		return qtrue;
	}
	return qfalse;
...
{
	if (ps->legsAnim == BOTH_A7_SOULCAL && ps->legsAnimTimer < 700 && ps->legsAnimTimer > 250)
	{
		return qtrue;
	}
	return qfalse;

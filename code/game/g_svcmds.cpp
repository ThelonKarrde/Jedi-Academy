...
{
	if (g_entities[0].client->ps.viewEntity > 0 && g_entities[0].client->ps.viewEntity < ENTITYNUM_WORLD && g_entities[g_entities[0].client->ps.viewEntity].client && g_entities[g_entities[0].client->ps.viewEntity].s.weapon == WP_SABER)
...
void Svcmd_SaberAttackCycle_f(void)
{
	...
	saberAnimLevel++;
	int sanityCheck = 0;
	while (self->client->ps.saberAnimLevel != saberAnimLevel && !(self->client->ps.saberStylesKnown & (1 << saberAnimLevel)) && sanityCheck < SS_NUM_SABER_STYLES + 1)
	{
		saberAnimLevel++;

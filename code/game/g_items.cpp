...
{
	if ((item->flags & FL_DROPPED_ITEM) && item->activator != &g_entities[0] && pickerupper->s.number && pickerupper->s.weapon == WP_NONE && pickerupper->enemy && pickerupper->painDebounceTime < level.time && pickerupper->NPC && pickerupper->NPC->surrenderTime < level.time //not surrendering
...
gentity_t *LaunchItem(gitem_t *item, const vec3_t origin, const vec3_t velocity, char *target)
{
	...
	if (item->giType == IT_WEAPON)
	{
		...
		if (item->giTag != WP_BOWCASTER && item->giTag != WP_THERMAL && item->giTag != WP_TRIP_MINE && item->giTag != WP_DET_PACK)
		{
...
void FinishSpawningItem(gentity_t *ent)
{
	...
	if (ent->item->giType == IT_WEAPON && ent->item->giTag == WP_SABER && ent->NPC_type && ent->NPC_type[0] && Q_stricmp("player", ent->NPC_type) == 0 && g_saber->string && g_saber->string[0] && Q_stricmp("none", g_saber->string) && Q_stricmp("NULL", g_saber->string))
	{ //player's saber

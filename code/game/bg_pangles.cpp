...
qboolean PM_AdjustAngleForWallRun(gentity_t *ent, usercmd_t *ucmd, qboolean doMove)
{
	...
	{ //wall-running and not at end of anim
		...
		if (trace.fraction < 1.0f && (trace.plane.normal[2] >= 0.0f && trace.plane.normal[2] <= 0.4f)) //&& ent->client->ps.groundEntityNum == ENTITYNUM_NONE )
		{
			trace_t trace2;
			vec3_t traceTo2;
...
qboolean PM_AdjustAnglesForBackAttack(gentity_t *ent, usercmd_t *ucmd)
{
	...
	if ((ent->client->ps.saberMove == LS_A_BACK || ent->client->ps.saberMove == LS_A_BACK_CR || ent->client->ps.saberMove == LS_A_BACKSTAB) && PM_InAnimForSaberMove(ent->client->ps.torsoAnim, ent->client->ps.saberMove))
	{
...
{
	if (ent && ent->client && (ent->client->ps.legsAnim == BOTH_PLAYER_PA_3_FLY || ent->client->ps.legsAnim == BOTH_LK_DL_ST_T_SB_1_L || ent->client->ps.legsAnim == BOTH_RELEASED))
	{ //special cases
		return 200;
	}
...
qboolean PM_AdjustAnglesForKnockdown(gentity_t *ent, usercmd_t *ucmd, qboolean angleClampOnly)
{
	...
	{ //being knocked down or getting up, can't do anything!
		if (!angleClampOnly)
		{
			if (ent->client->ps.legsAnimTimer > G_MinGetUpTime(ent) || (ent->s.number >= MAX_CLIENTS && !G_ControlledByPlayer(ent)))
			{ //can't get up yet
				ucmd->forwardmove = 0;
				ucmd->rightmove = 0;
			}
...
qboolean PM_AdjustAngleForWallRunUp(gentity_t *ent, usercmd_t *ucmd, qboolean doMove)
{
	...
	{ //wall-running up
		...
		{ //hmm, some room, see if there's a floor right here
			...
			if (!trace2.allsolid && !trace2.startsolid && trace2.fraction < 1.0f && trace2.plane.normal[2] > 0.7f) //slope we can stand on
		...
		if ( //ucmd->upmove <= 0 &&
			ent->client->ps.legsAnimTimer > 0 && ucmd->forwardmove > 0 && trace.fraction < 1.0f && (trace.plane.normal[2] >= 0.0f && trace.plane.normal[2] <= MAX_WALL_RUN_Z_NORMAL))
		{ //still a vertical wall there
...
{
	if (PM_InReboundJump(ent->client->ps.legsAnim) || PM_InReboundHold(ent->client->ps.legsAnim))
	{ //hugging wall, getting ready to jump off
		...
		else if (doMove && (ent->client->ps.pm_flags & PMF_STUCK_TO_WALL))
		{ //jump off
			//push off of it!
			ent->client->ps.pm_flags &= ~PMF_STUCK_TO_WALL;
...
{
	if (PM_StabDownAnim(ent->client->ps.torsoAnim) && ent->client->ps.torsoAnimTimer)
	{ //lock our angles
...
void PM_UpdateViewAngles(playerState_t *ps, usercmd_t *cmd, gentity_t *gent)
{
	...
	{ //only in the real pmove
		...
		{ //check leaning
			...
			{																										 //third person lean
				if (G_OkayToLean(ps, cmd, qtrue) && (cmd->rightmove || (cmd->forwardmove && g_debugMelee->integer))) //pushing a direction
				{
					int anim = -1;
					if (cmd->rightmove > 0)
					{ //lean right

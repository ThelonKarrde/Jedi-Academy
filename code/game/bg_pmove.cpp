...
qboolean PM_CheckGrabWall(trace_t *trace)
{
	...
	if ((pm->ps->clientNum < MAX_CLIENTS || PM_ControlledByPlayer()) && pm->gent->client->ps.forcePowerLevel[FP_LEVITATION] < FORCE_LEVEL_3)
...
static void PM_Friction(void)
{
	...
	if (pm->gent && pm->gent->client && pm->gent->client->NPC_class == CLASS_VEHICLE && pm->gent->m_pVehicle && pm->gent->m_pVehicle->m_pVehicleInfo->type != VH_ANIMAL)
	{
	...
	else if (Flying != FLY_NORMAL)
	{
		if ((pm->watertype & CONTENTS_LADDER) || pm->waterlevel <= 1)
		{
			if ((pm->watertype & CONTENTS_LADDER) || (pml.walking && !(pml.groundTrace.surfaceFlags & SURF_SLICK)))
			{
				...
				if (!(pm->ps->pm_flags & PMF_TIME_KNOCKBACK) && !(pm->ps->pm_flags & PMF_TIME_NOFRICTION))
				{
					if (pm->ps->legsAnim == BOTH_FORCELONGLEAP_START || pm->ps->legsAnim == BOTH_FORCELONGLEAP_ATTACK || pm->ps->legsAnim == BOTH_FORCELONGLEAP_LAND)
					{ //super forward jump
						if (pm->ps->groundEntityNum != ENTITYNUM_NONE)
						{ //not in air
...
qboolean PM_ForceJumpingUp(gentity_t *gent)
{
	...
	{ //this is ONLY for the player
		if (player && player->client && player->client->ps.viewEntity == gent->s.number)
	...
	*/
...
#if METROID_JUMP
	if ((pm->ps->clientNum < MAX_CLIENTS || PM_ControlledByPlayer()) && pm->gent && pm->gent->client && (pm->gent->client->NPC_class == CLASS_BOBAFETT || pm->gent->client->NPC_class == CLASS_ROCKETTROOPER))
	...
	else if (pm->waterlevel < 3) //|| (pm->ps->waterHeightLevel==WHL_SHOULDERS&&pm->cmd.upmove>0) )
	{
		...
		{ //can't do this in zero-G
			...
			{ //in the middle of a force long-jump
				...
				*/
				if ((pm->ps->legsAnim == BOTH_FORCELONGLEAP_START || pm->ps->legsAnim == BOTH_FORCELONGLEAP_ATTACK) && pm->ps->legsAnimTimer > 0)
				{ //in the air
			...
			else if (PM_InCartwheel(pm->ps->legsAnim) || PM_InButterfly(pm->ps->legsAnim))
					...
					{															 //passed normal jump height  *2?
						if (!(pm->ps->forcePowersActive & (1 << FP_LEVITATION))) //haven't started forcejump yet
						{
							...
							{ //still in kick
									 ...
									 pm->ps->legsAnim != BOTH_ALORA_FLIP_3 && cg.renderingThirdPerson //third person only
			 ...
			 && ((pm->ps->clientNum && !PM_ControlledByPlayer()) || ((pm->ps->clientNum < MAX_CLIENTS || PM_ControlledByPlayer()) && cg.renderingThirdPerson && !cg.zoomMode && !(pm->gent->flags & FL_LOCK_PLAYER_WEAPONS)))) // yes this locked weapons check also includes force powers, if we need a separate check later I'll make one
	{
		...
		{ //on the ground
			...
			{																																							 //holding attack and jumping
				if (pm->cmd.rightmove > 0)
				{
					...
					{
						if ((pm->ps->clientNum >= MAX_CLIENTS && !PM_ControlledByPlayer()) || pm->ps->forcePowerLevel[FP_LEVITATION] > FORCE_LEVEL_2)
						{
							anim = BOTH_BUTTERFLY_RIGHT;
							forcePowerCostOverride = G_CostForSpecialMove(SABER_ALT_ATTACK_POWER_LR);
						}
					}
					else if ((pm->ps->clientNum >= MAX_CLIENTS && !PM_ControlledByPlayer()) || pm->ps->forcePowerLevel[FP_LEVITATION] > FORCE_LEVEL_1)
					{
						if (pm->ps->clientNum < MAX_CLIENTS || PM_ControlledByPlayer())
			...
			if (anim != -1 && PM_HasAnimation(pm->gent, anim))
			{
				...
				if (doTrace)
				{
					...
					{
						if (anim == BOTH_WALL_FLIP_BACK1 || anim == BOTH_FORCEWALLRUNFLIP_START)
				...
				{ //there is a wall there
					...
					if ((anim != BOTH_WALL_RUN_LEFT && anim != BOTH_WALL_RUN_RIGHT && anim != BOTH_FORCEWALLRUNFLIP_START) || (wallNormal[2] >= 0.0f && wallNormal[2] <= MAX_WALL_RUN_Z_NORMAL))
					{ //wall-runs can only run on relatively flat walls, sorry.
						...
						else if (anim == BOTH_FLIP_BACK1 || anim == BOTH_FLIP_BACK2 || anim == BOTH_FLIP_BACK3 || anim == BOTH_ALORA_FLIP_B || anim == BOTH_WALL_FLIP_BACK1)
						{
							pm->ps->velocity[0] = pm->ps->velocity[1] = 0;
							VectorMA(pm->ps->velocity, -150, fwd, pm->ps->velocity);
						}
						//kick if jumping off an ent
						if (doTrace && anim != BOTH_WALL_RUN_LEFT && anim != BOTH_WALL_RUN_RIGHT && anim != BOTH_FORCEWALLRUNFLIP_START)
						{
							if (pm->gent && trace.entityNum < ENTITYNUM_WORLD)
							{
								if (traceEnt && traceEnt->client && traceEnt->health > 0 && traceEnt->takedamage && traceEnt->client->NPC_class != CLASS_GALAKMECH && traceEnt->client->NPC_class != CLASS_DESANN && !(traceEnt->flags & FL_NO_KNOCKBACK))
								{ //push them away and do pain
									...
									{ //didn't kill him
										if ((traceEnt->s.number == 0 && !Q_irand(0, g_spskill->integer)) || (traceEnt->NPC != NULL && Q_irand(RANK_CIVILIAN, traceEnt->NPC->rank) + Q_irand(-2, 2) < RANK_ENSIGN))
										{
						...
						{ //not a proper saberMove, so do set all the details manually
							...
							if (/*anim == BOTH_BUTTERFLY_LEFT
								...
								||*/
								anim == BOTH_FJSS_TR_BL ||
								anim == BOTH_FJSS_TL_BR || anim == BOTH_FORCEWALLRUNFLIP_START)
							{
								pm->ps->weaponTime = pm->ps->torsoAnimTimer;
							}
							else if (anim == BOTH_WALL_FLIP_LEFT || anim == BOTH_WALL_FLIP_RIGHT || anim == BOTH_WALL_FLIP_BACK1)
		...
		{ //in the air
			...
			*/
			else if ((pm->ps->legsAnimTimer <= 100 || !PM_InSpecialJump(legsAnim) //not in a special jump anim
			...
			{ //see if we're pushing at a wall and jump off it if so
				...
				{																							   //trace in the dir we're pushing in and see if there's a vertical wall there
					...
					VectorNormalize(idealNormal);
					gentity_t *traceEnt = &g_entities[trace.entityNum];
					if (trace.fraction < 1.0f && fabs(trace.plane.normal[2]) <= MAX_WALL_GRAB_SLOPE && ((trace.entityNum < ENTITYNUM_WORLD && traceEnt && traceEnt->s.solid != SOLID_BMODEL) || DotProduct(trace.plane.normal, idealNormal) > 0.7))
					{ //there is a wall there
	...
	if (pm->gent
		//&& pm->cmd.upmove > 0
		&& pm->ps->forceRageRecoveryTime < pm->cmd.serverTime //not in a force Rage recovery period
		&& pm->ps->weapon == WP_SABER && (pm->ps->weaponTime > 0 || (pm->cmd.buttons & BUTTON_ATTACK)) && ((pm->ps->clientNum && !PM_ControlledByPlayer()) || ((pm->ps->clientNum < MAX_CLIENTS || PM_ControlledByPlayer()) && cg.renderingThirdPerson && !cg.zoomMode)))
	{ //okay, we just jumped and we're in an attack
		if (!PM_RollingAnim(pm->ps->legsAnim) && !PM_InKnockDown(pm->ps) && !PM_InDeathAnim() && !PM_PainAnim(pm->ps->torsoAnim) && !PM_FlippingAnim(pm->ps->legsAnim) && !PM_SpinningAnim(pm->ps->legsAnim) && !PM_SaberInSpecialAttack(pm->ps->torsoAnim))
...
static void PM_FlyMove(void)
{
	...
	else if (pm->ps->gravity <= 0 && ((pm->ps->clientNum < MAX_CLIENTS || PM_ControlledByPlayer()) || (pm->gent && pm->gent->client && pm->gent->client->moveType == MT_RUNJUMP)))
	{
		PM_CheckJump();
		accel = 1.0f;
		pm->ps->velocity[2] -= pm->ps->gravity * pml.frametime;
...
qboolean PM_GroundSlideOkay(float zNormal)
{
	if (zNormal > 0)
	{
		if (pm->ps->velocity[2] > 0)
		{
			if (pm->ps->legsAnim == BOTH_WALL_RUN_RIGHT || pm->ps->legsAnim == BOTH_WALL_RUN_LEFT || pm->ps->legsAnim == BOTH_WALL_RUN_RIGHT_STOP || pm->ps->legsAnim == BOTH_WALL_RUN_LEFT_STOP || pm->ps->legsAnim == BOTH_FORCEWALLRUNFLIP_START || pm->ps->legsAnim == BOTH_FORCELONGLEAP_START || pm->ps->legsAnim == BOTH_FORCELONGLEAP_ATTACK || pm->ps->legsAnim == BOTH_FORCELONGLEAP_LAND || PM_InReboundJump(pm->ps->legsAnim))
			{
				return qfalse;
			}
		}
...
static void PM_AirMove(void)
{
	...
	if ((pm->ps->clientNum < MAX_CLIENTS || PM_ControlledByPlayer()) && pm->ps->forcePowerLevel[FP_LEVITATION] > FORCE_LEVEL_0 && pm->ps->forceJumpZStart && pm->ps->velocity[2] > 0)
...
static float PM_DamageForDelta(int delta)
{
	...
	if (pm->gent->NPC)
	{
		if (pm->ps->weapon == WP_SABER || (pm->gent->client && pm->gent->client->NPC_class == CLASS_REBORN))
...
static qboolean PM_TryRoll(void)
{
	...
	if (PM_SaberInAttack(pm->ps->saberMove) || PM_SaberInSpecialAttack(pm->ps->torsoAnim) || PM_SpinningSaberAnim(pm->ps->legsAnim) || ((pm->ps->clientNum < MAX_CLIENTS || PM_ControlledByPlayer()) && PM_SaberInStart(pm->ps->saberMove)))
	...
	{ //NPC
		...
		{																							  //non-jedi/reborn
			...
			{																						  //only jedi/reborn NPCs should be able to do rolls (with a few exceptions)
				if (!pm->gent || !pm->gent->client || (pm->gent->client->NPC_class != CLASS_BOBAFETT  //boba can roll with it, baby
...
static void PM_CrashLand(void)
{
	...
	else
	{
		...
		else if (pm->gent && pm->gent->client && (pm->gent->client->NPC_class == CLASS_BOBAFETT || pm->gent->client->NPC_class == CLASS_ROCKETTROOPER))
		{
			if (JET_Flying(pm->gent))
			{
				if (pm->gent->client->NPC_class == CLASS_BOBAFETT || (pm->gent->client->NPC_class == CLASS_ROCKETTROOPER && pm->gent->NPC && pm->gent->NPC->rank < RANK_LT))
				{
					JET_FlyStop(pm->gent);
				}
				else
	...
	if (!PM_InDeathAnim())
	{
		if (PM_InAirKickingAnim(pm->ps->legsAnim) && pm->ps->torsoAnim == pm->ps->legsAnim)
		{
		...
		{																																   //not crouching
			if (delta > 10 || pm->ps->pm_flags & PMF_BACKWARDS_JUMP || (pm->ps->forcePowersActive & (1 << FP_LEVITATION)) || forceLanding) //EV_FALL_SHORT or jumping back or force-land
			{																															   // decide which landing animation to use
				if (pm->gent && pm->gent->client && (pm->gent->client->NPC_class == CLASS_RANCOR || pm->gent->client->NPC_class == CLASS_WAMPA))
				{
				}
				else
				{
					...
					if (anim != -1)
					{
						if (PM_FlippingAnim(pm->ps->torsoAnim) || PM_SpinningAnim(pm->ps->torsoAnim) || pm->ps->torsoAnim == BOTH_FLIP_LAND)
						...
						else if (anim == BOTH_FLIP_LAND || (pm->ps->torsoAnim == BOTH_FLIP_LAND))
						{
							PM_SetAnim(pm, SETANIM_BOTH, anim, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD, 100); // Only blend over 100ms
						}
						else if (PM_InAirKickingAnim(pm->ps->legsAnim) && pm->ps->torsoAnim == pm->ps->legsAnim)
						{																						//interrupting a kick clears everything
							PM_SetAnim(pm, SETANIM_BOTH, anim, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD, 100); // Only blend over 100ms
							pm->ps->saberMove = LS_READY;
							pm->ps->weaponTime = 0;
						}
						else if (PM_ForceJumpingAnim(pm->ps->legsAnim) && pm->ps->torsoAnim == pm->ps->legsAnim)
	...
	else
	{
		...
		if (pm->gent)
		{
				...
				(!(pm->pointcontents(pm->ps->origin, pm->ps->clientNum) & CONTENTS_NODROP)))
			{
				...
				if (gi.VoiceVolume[pm->ps->clientNum] && pm->gent->NPC && (pm->gent->NPC->aiFlags & NPCAI_DIE_ON_IMPACT))
...
{
	if (pm->gent->NPC && pm->gent->client && (pm->gent->client->NPC_class == CLASS_BOBAFETT || pm->gent->client->NPC_class == CLASS_ROCKETTROOPER))
...
void BG_VehicleTurnRateForSpeed(Vehicle_t *pVeh, float speed, float *mPitchOverride, float *mYawOverride)
{
	if (pVeh && pVeh->m_pVehicleInfo)
	{
		...
		{
			if (pVeh->m_LandTrace.fraction >= 1.0f || pVeh->m_LandTrace.plane.normal[2] < MIN_LANDING_SLOPE)
			{
	...
	if (Flying != FLY_HOVER)
	{
		if (!(pm->ps->eFlags & EF_FORCE_DRAINED))
		{
			...
			if (pm->ps->clientNum != 0 && pm->gent && pm->gent->NPC && pm->gent->client && pm->gent->client->NPC_class != CLASS_DESANN) //desann never falls to his death
			{
				if (pm->ps->groundEntityNum == ENTITYNUM_NONE)
				{
					if (pm->ps->stats[STAT_HEALTH] > 0 && !(pm->gent->NPC->aiFlags & NPCAI_DIE_ON_IMPACT) && !(pm->gent->NPC->aiFlags & NPCAI_JUMP) // doing a path jump
						&& !(pm->gent->NPC->scriptFlags & SCF_NO_FALLTODEATH) && pm->gent->NPC->behaviorState != BS_JUMP)							//not being scripted to jump
					{
						...
						{
							if (!FlyingCreature(pm->gent) && g_gravity->value > 0)
							{
								if (!(pm->gent->flags & FL_UNDYING) && !(pm->gent->flags & FL_GODMODE))
								{
									if (!(pm->ps->eFlags & EF_FORCE_GRIPPED) && !(pm->ps->eFlags & EF_FORCE_DRAINED) && !(pm->ps->pm_flags & PMF_TRIGGER_PUSHED))
									{
										if (!pm->ps->forceJumpZStart || pm->ps->forceJumpZStart > pm->ps->origin[2]) // && fabs(pm->ps->velocity[0])<10 && fabs(pm->ps->velocity[1])<10 && pm->ps->velocity[2]<0)//either not force-jumping or force-jumped and now fell below original jump start height
										{
											...
											if ((trace.contents & CONTENTS_LAVA) && PM_RocketeersAvoidDangerousFalls())
											{ //got out of it
											}
											else if (!trace.allsolid && !trace.startsolid && (pm->ps->origin[2] - trace.endpos[2]) >= 128) //>=128 so we don't die on steps!
											{
												...
												else if (trace.entityNum < ENTITYNUM_NONE && pm->ps->weapon != WP_SABER && (!pm->gent || !pm->gent->client || (pm->gent->client->NPC_class != CLASS_BOBAFETT && pm->gent->client->NPC_class != CLASS_REBORN && pm->gent->client->NPC_class != CLASS_ROCKETTROOPER)))
			...
			if (!cliff_fall)
			{
				if (pm->ps->legsAnim == BOTH_FORCELONGLEAP_START || pm->ps->legsAnim == BOTH_FORCELONGLEAP_ATTACK || pm->ps->legsAnim == BOTH_FORCEWALLRUNFLIP_START || pm->ps->legsAnim == BOTH_FLIP_LAND)
				{ //let it stay on this anim
				}
				else if (PM_KnockDownAnimExtended(pm->ps->legsAnim))
				{ //no in-air anims when in knockdown anim
				}
				else if ((pm->ps->legsAnim == BOTH_WALL_RUN_RIGHT || pm->ps->legsAnim == BOTH_WALL_RUN_LEFT || pm->ps->legsAnim == BOTH_WALL_RUN_RIGHT_STOP || pm->ps->legsAnim == BOTH_WALL_RUN_LEFT_STOP || pm->ps->legsAnim == BOTH_WALL_RUN_RIGHT_FLIP || pm->ps->legsAnim == BOTH_WALL_RUN_LEFT_FLIP || pm->ps->legsAnim == BOTH_WALL_FLIP_RIGHT || pm->ps->legsAnim == BOTH_WALL_FLIP_LEFT || pm->ps->legsAnim == BOTH_FORCEWALLREBOUND_FORWARD || pm->ps->legsAnim == BOTH_FORCEWALLREBOUND_LEFT || pm->ps->legsAnim == BOTH_FORCEWALLREBOUND_BACK || pm->ps->legsAnim == BOTH_FORCEWALLREBOUND_RIGHT || pm->ps->legsAnim == BOTH_CEILING_DROP) && !pm->ps->legsAnimTimer)
				...
				else if (pm->ps->legsAnim == BOTH_FLIP_ATTACK7 || pm->ps->legsAnim == BOTH_FLIP_HOLD7)
				{
					if (!pm->ps->legsAnimTimer)
					{																								   //done?
				...
				else if (!PM_InRoll(pm->ps) && !PM_SpinningAnim(pm->ps->legsAnim) && !PM_FlippingAnim(pm->ps->legsAnim) && !PM_InSpecialJump(pm->ps->legsAnim))
				{
					if (pm->ps->groundEntityNum != ENTITYNUM_NONE)
					{
...
static void PM_GroundTrace(void)
{
	...
	if ((pm->ps->eFlags & EF_LOCKED_TO_WEAPON) || (pm->ps->eFlags & EF_HELD_BY_RANCOR) || (pm->ps->eFlags & EF_HELD_BY_WAMPA) || (pm->ps->eFlags & EF_HELD_BY_SAND_CREATURE) || PM_RidingVehicle())
	{
	...
	else if (pm->ps->legsAnimTimer > 300 && (pm->ps->legsAnim == BOTH_WALL_RUN_RIGHT || pm->ps->legsAnim == BOTH_WALL_RUN_LEFT || pm->ps->legsAnim == BOTH_FORCEWALLRUNFLIP_START))
	...
	if (pm->gent && pm->gent->client && pm->gent->client->NPC_class != CLASS_SAND_CREATURE && (pm->gent->client->NPC_class != CLASS_VEHICLE && !PM_RidingVehicle()))
	{
		...
		{ //either thrown off ground (PMF_TIME_KNOCKBACK) or going off the ground at a large velocity
			...
			else if (pm->gent && pm->gent->client && (pm->gent->client->NPC_class == CLASS_RANCOR || pm->gent->client->NPC_class == CLASS_WAMPA))
			{
			}
			else
			{
...
static void PM_CheckDuck(void)
{
	...
	if (pm->gent && pm->gent->client)
	{
		...
		if (pm->ps->clientNum < MAX_CLIENTS && (pm->gent->client->NPC_class == CLASS_ATST || pm->gent->client->NPC_class == CLASS_RANCOR) && !cg.renderingThirdPerson)
		{
...
{
	if (pm->ps->legsAnim == BOTH_KNOCKDOWN1 || pm->ps->legsAnim == BOTH_KNOCKDOWN2 || pm->ps->legsAnim == BOTH_KNOCKDOWN3 || pm->ps->legsAnim == BOTH_KNOCKDOWN4 || pm->ps->legsAnim == BOTH_KNOCKDOWN5 || pm->ps->legsAnim == BOTH_LK_DL_ST_T_SB_1_L || pm->ps->legsAnim == BOTH_PLAYER_PA_3_FLY || pm->ps->legsAnim == BOTH_RELEASED)
	{																																												 //lying on back or front
		...
		{																																											 //roll away!
			...
			if (pm->cmd.forwardmove > 0)
			{
				if (pm->ps->legsAnim == BOTH_KNOCKDOWN3 || pm->ps->legsAnim == BOTH_KNOCKDOWN5 || pm->ps->legsAnim == BOTH_LK_DL_ST_T_SB_1_L)
				{
			...
			if (forceGetUp)
			{
				if (pm->gent && pm->gent->client && pm->gent->client->playerTeam == TEAM_ENEMY && pm->gent->NPC && pm->gent->NPC->blockedSpeechDebounceTime < level.time && !Q_irand(0, 1))
				{
...
{
	int legsAnim = pm->ps->legsAnim;
	if (legsAnim == BOTH_KNOCKDOWN1 || legsAnim == BOTH_KNOCKDOWN2 || legsAnim == BOTH_KNOCKDOWN3 || legsAnim == BOTH_KNOCKDOWN4 || legsAnim == BOTH_KNOCKDOWN5 || legsAnim == BOTH_PLAYER_PA_3_FLY || legsAnim == BOTH_LK_DL_ST_T_SB_1_L || legsAnim == BOTH_RELEASED)
	{ //in a knockdown
...
void PM_FootSlopeTrace(float *pDiff, float *pInterval)
{
	...
	for (int i = 0; i < 3; i++)
	{
		if (_isnan(pm->gent->client->renderInfo.footLPoint[i]) || _isnan(pm->gent->client->renderInfo.footRPoint[i]))
		{
			if (pDiff != NULL)
			{
				*pDiff = 0;
...
qboolean PM_AdjustStandAnimForSlope(void)
{
	...
	if (pm->gent->client->NPC_class != CLASS_ATST && (!pm->gent || !G_StandardHumanoid(pm->gent)))
...
static void PM_Footsteps(void)
{
	...
	if (pm->ps->groundEntityNum == ENTITYNUM_NONE || (pm->watertype & CONTENTS_LADDER) || pm->ps->waterHeightLevel >= WHL_TORSO)
	{ //in air or submerged in water or in ladder
		...
		if (pm->waterlevel > 0)
		{
			...
			else if (pm->ps->waterHeightLevel >= WHL_TORSO && ((pm->ps->clientNum < MAX_CLIENTS || PM_ControlledByPlayer()) || pm->ps->weapon == WP_SABER || pm->ps->weapon == WP_NONE || pm->ps->weapon == WP_MELEE)) //pm->waterlevel > 1 )	//in deep water
			{
	...
	if (!pm->cmd.forwardmove && !pm->cmd.rightmove)
	{
		...
		else
		{
			...
			if (pm->ps->legsAnimTimer && (pm->ps->legsAnim == BOTH_THERMAL_READY || pm->ps->legsAnim == BOTH_THERMAL_THROW || pm->ps->legsAnim == BOTH_ATTACK10))
			...
			else if (pm->ps->weapon == WP_SABER && pm->ps->saberInFlight && saberInAir && (!pm->ps->dualSabers || !pm->ps->saber[1].Active()))
			{
				if (!PM_AdjustStandAnimForSlope())
				{
					if (pm->ps->legsAnim != BOTH_LOSE_SABER || !pm->ps->legsAnimTimer)
					{
						PM_SetAnim(pm, SETANIM_LEGS, BOTH_SABERPULL, SETANIM_FLAG_NORMAL);
					}
				}
			}
			else if ((pm->ps->weapon == WP_SABER && pm->ps->SaberLength() > 0 && !pm->ps->saberInFlight && !PM_SaberDrawPutawayAnim(pm->ps->legsAnim)))
			{
				if (!PM_AdjustStandAnimForSlope())
				{
					int legsAnim;
					if ((pm->ps->torsoAnim == BOTH_SPINATTACK6 || pm->ps->torsoAnim == BOTH_SPINATTACK7 || PM_SaberInAttack(pm->ps->saberMove) || PM_SaberInTransitionAny(pm->ps->saberMove)) && pm->ps->legsAnim != BOTH_FORCELONGLEAP_LAND && (pm->ps->groundEntityNum == ENTITYNUM_NONE											  //in air
			...
			{																				  //Squadmates use BOTH_STAND3
				...
				if (oldAnim != BOTH_GUARD_LOOKAROUND1 && oldAnim != BOTH_GUARD_IDLE1 && oldAnim != BOTH_STAND2TO4 && oldAnim != BOTH_STAND4TO2 && oldAnim != BOTH_STAND4)
	...
	if ((pm->ps->eFlags & EF_IN_ATST) || (pm->gent && pm->gent->client && pm->gent->client->NPC_class == CLASS_RANCOR) //does this catch NPCs, too?
	...
	{																																																				   //all other NPCs...
		if ((PM_InSaberAnim(pm->ps->legsAnim) && !PM_SpinningSaberAnim(pm->ps->legsAnim)) || PM_SaberStanceAnim(pm->ps->legsAnim) || PM_SaberDrawPutawayAnim(pm->ps->legsAnim) || pm->ps->legsAnim == BOTH_SPINATTACK6 //not a full-body spin, just spinning the saber
			|| pm->ps->legsAnim == BOTH_SPINATTACK7																																									   //not a full-body spin, just spinning the saber
			|| pm->ps->legsAnim == BOTH_BUTTON_HOLD || pm->ps->legsAnim == BOTH_BUTTON_RELEASE || pm->ps->legsAnim == BOTH_THERMAL_READY || pm->ps->legsAnim == BOTH_THERMAL_THROW || pm->ps->legsAnim == BOTH_ATTACK10 || PM_LandingAnim(pm->ps->legsAnim) || PM_PainAnim(pm->ps->legsAnim) || PM_ForceAnim(pm->ps->legsAnim))
	...
	if (pm->ps->pm_flags & PMF_DUCKED)
	{
			...
			&& (!PM_InRollIgnoreTimer(pm->ps) || (!pm->ps->legsAnimTimer && pm->cmd.upmove < 0))) //not in a roll (or you just finished one and you're still holding crouch)
		{
			...
			if (PM_RunningAnim(pm->ps->legsAnim) || pm->ps->legsAnim == BOTH_FORCEHEAL_START || PM_CanRollFromSoulCal(pm->ps))
			{ //roll!
				rolled = PM_TryRoll();
			}
			if (!rolled)
...
static void PM_BeginWeaponChange(int weapon)
{
	...
	if (pm->gent && pm->gent->client && (pm->gent->client->NPC_class == CLASS_ATST || pm->gent->client->NPC_class == CLASS_RANCOR))
	{
		if (pm->ps->clientNum < MAX_CLIENTS)
		{
...
static void PM_FinishWeaponChange(void)
{
	...
	{ //switched away from saber
		...
		if (!(pm->ps->eFlags & EF_HELD_BY_WAMPA))
		{
			if (pm->ps->weapon != WP_THERMAL && pm->ps->weapon != WP_TRIP_MINE && pm->ps->weapon != WP_DET_PACK && !G_IsRidingVehicle(pm->gent))
			{
				PM_SetAnim(pm, SETANIM_TORSO, TORSO_RAISEWEAP1, SETANIM_FLAG_HOLD);
			}
		}

		if (pm->ps->clientNum < MAX_CLIENTS && cg_gunAutoFirst.integer && !PM_RidingVehicle()
			//			&& oldWeap == WP_SABER
			&& weapon != WP_NONE)
		{
...
void PM_SetSaberMove(saberMoveName_t newMove)
{
	...
	{
		if (pm->ps->saber[0].style == SS_STAFF && pm->ps->saber[0].blade[1].active)
		{
			anim = BOTH_S7_S1;
		}
		else if (pm->ps->dualSabers && pm->ps->saber[0].style == SS_NONE && pm->ps->saber[1].style == SS_NONE && pm->ps->saber[1].Active())
		{
			anim = BOTH_S6_S1;
		}
	...
	else if ((pm->ps->saberAnimLevel == SS_DUAL || (pm->ps->dualSabers && pm->ps->saber[1].Active())) && newMove >= LS_S_TL2BR && newMove < LS_REFLECT_LL)
	{ //staff has an entirely new set of anims, besides special attacks
		//FIXME: include ready and draw/putaway?
		//FIXME: get hand-made bounces and deflections?
		...
		else if ((newMove == LS_A_R2L || newMove == LS_S_R2L || newMove == LS_A_L2R || newMove == LS_S_L2R) && PM_CanDoDualDoubleAttacks() && G_CheckEnemyPresence(pm->gent, DIR_RIGHT, 150.0f) && G_CheckEnemyPresence(pm->gent, DIR_LEFT, 150.0f))
		...
		else if ((newMove == LS_A_T2B || newMove == LS_S_T2B || newMove == LS_A_BACK || newMove == LS_A_BACK_CR) && PM_CanDoDualDoubleAttacks() && G_CheckEnemyPresence(pm->gent, DIR_FRONT, 150.0f) && G_CheckEnemyPresence(pm->gent, DIR_BACK, 150.0f))
	...
	*/
	...
	else if (newMove == LS_KICK_F_AIR || newMove == LS_KICK_B_AIR || newMove == LS_KICK_R_AIR || newMove == LS_KICK_L_AIR)
	{
		if (pm->ps->groundEntityNum != ENTITYNUM_NONE)
		{
	...
	if ((pm->ps->torsoAnim == anim || pm->ps->legsAnim == anim) && newMove > LS_PUTAWAY)
	{
		setflags |= SETANIM_FLAG_RESTART;
	}

	if ((anim == BOTH_STAND1 && (pm->ps->saber[0].type == SABER_ARC || (pm->ps->dualSabers && pm->ps->saber[1].Active()))) || anim == BOTH_STAND2
		//FIXME: temp hack to stop it from using run2 with staff
		|| (0 && anim == BOTH_SABERSTAFF_STANCE) || anim == BOTH_SABERDUAL_STANCE || anim == BOTH_SABERFAST_STANCE || anim == BOTH_SABERSLOW_STANCE)
	...
	if (!PM_RidingVehicle())
	{
		if (!manualBlocking)
		{
			...
			{ //not trying to run, duck or jump
				...
				else if (!(pm->ps->pm_flags & PMF_DUCKED) && (newMove == LS_SPINATTACK_DUAL || newMove == LS_SPINATTACK))
				{
					parts = SETANIM_BOTH;
				}
			}
	...
	{ //successfully changed anims
		...
		*/
		...
		{
			if (pm->ps->saberBlocked >= BLOCKED_UPPER_RIGHT_PROJ && pm->ps->saberBlocked <= BLOCKED_TOP_PROJ && newMove >= LS_REFLECT_UP && newMove <= LS_REFLECT_LL)
...
*/
	...
	if (winAnim != -1)
	{
		...
		if (breakType == SABERLOCK_SUPERBREAK && winAnim != BOTH_LK_ST_DL_T_SB_1_W)
...
int PM_SaberLockResultAnim(gentity_t *duelist, int lockOrBreakOrSuperBreak, int winOrLose)
{
	...
	if (lockOrBreakOrSuperBreak == SABERLOCK_SUPERBREAK && winOrLose == SABERLOCK_LOSE)
	...
	if (lockOrBreakOrSuperBreak == SABERLOCK_SUPERBREAK && winOrLose == SABERLOCK_WIN && baseAnim != BOTH_LK_ST_DL_T_SB_1_W)
	{ //going to attack with saber, do a saber trail
...
void PM_SaberLockBreak(gentity_t *gent, gentity_t *genemy, saberLockResult_t result, int victoryStrength)
{
	...
	{
		if (genemy && genemy->NPC && ((genemy->NPC->aiFlags & NPCAI_BOSS_CHARACTER) || (genemy->NPC->aiFlags & NPCAI_SUBBOSS_CHARACTER) || (genemy->client && genemy->client->NPC_class == CLASS_SHADOWTROOPER)) && Q_irand(0, 4))
	...
	else
	{
		...
		{ //one person won
			if (Q_irand(FORCE_LEVEL_1, FORCE_LEVEL_2) < pm->ps->forcePowerLevel[FP_SABER_OFFENSE])
			{
				...
				else if (winAnim == BOTH_LK_S_S_S_SB_1_W || winAnim == BOTH_LK_S_S_T_SB_1_W)
				...
				{ //JK2-style
					...
					if (winAnim != BOTH_CCWCIRCLEBREAK)
					{
						...
						{																																																													  //possibly knock saber out of hand OR cut hand off!
							if (Q_irand(0, 25) < victoryStrength && ((!genemy->s.number && genemy->health <= 10) || genemy->s.number))
							{
				...
				if (genemy->client->ps.saber[0].disarmable)
				{
					...
					{
						if (!genemy->client->ps.saber[0].twoHanded || !Q_irand(0, 1))
...
int G_SaberLockStrength(gentity_t *gent)
{
	...
	if (gent->s.number >= MAX_CLIENTS)
	{
		...
		else
		{
			...
			if (gent->NPC)
			{
				if ((gent->NPC->aiFlags & NPCAI_BOSS_CHARACTER) || (gent->NPC->aiFlags & NPCAI_ROSH) || gent->client->NPC_class == CLASS_SHADOWTROOPER)
				{
...
qboolean PM_SaberLocked(void)
{
	...
	{
		if (pm->ps->saberLockTime <= level.time + 500 && pm->ps->saberLockEnemy != ENTITYNUM_NONE)
		{ //lock just ended
...
{
	if (target && target->client && !PM_InKnockDown(&target->client->ps) && G_EnemyInKickRange(self, target))
	{
		return qtrue;
	}
	return qfalse;
...
saberMoveName_t G_PickAutoMultiKick(gentity_t *self, qboolean allowSingles, qboolean storeMove)
{
	...
	for (e = 0; e < numListedEntities; e++)
	{
		...
		if (kickMove == LS_KICK_F_AIR && kickMove == LS_KICK_B_AIR && kickMove == LS_KICK_R_AIR && kickMove == LS_KICK_L_AIR)
		...
		if (allowSingles)
		{
			if (kickMove != LS_NONE && distToEnt < bestDistToEnt)
			{
				bestKick = kickMove;
				bestEnt = ent;
			}
	...
	{ //can't do the multikicks in air
		if (enemiesFront && enemiesBack && (enemiesFront + enemiesBack) - (enemiesRight + enemiesLeft) > 1)
		{ //more enemies in front/back than left/right
			kickMove = LS_KICK_BF;
		}
		else if (enemiesRight && enemiesLeft && (enemiesRight + enemiesLeft) - (enemiesFront + enemiesBack) > 1)
...
{
	if ((pm->cmd.buttons & BUTTON_ALT_ATTACK) && (!(pm->ps->pm_flags & PMF_ALT_ATTACK_HELD) || PM_SaberInReturn(pm->ps->saberMove)) && (!PM_FlippingAnim(pm->ps->legsAnim) || pm->ps->legsAnimTimer <= 250) && (!PM_SaberThrowable()) && pm->ps->SaberActive())
	{
		return qtrue;
	}
	return qfalse;
...
qboolean PM_CheckUpsideDownAttack(void)
{
	...
	if (pm->ps->saberAnimLevel < SS_FAST || pm->ps->saberAnimLevel > SS_STRONG)
	{
		return qfalse;
	}
	if ((pm->ps->clientNum >= MAX_CLIENTS && !PM_ControlledByPlayer()))
	...
	case BOTH_FORCEWALLRUNFLIP_END:
	{
		...
		if (elapsedTime < midPoint - 100.0f || elapsedTime > midPoint + 100.0f)
...
qboolean PM_SaberMoveOkayForKata(void)
{
	...
	{																			  //old control scheme, allow it to interrupt a start or ready
		if (pm->ps->saberMove == LS_READY || PM_SaberInReflect(pm->ps->saberMove) //interrupt a projectile blocking move
	...
	{										//player kicks
		...
		{		//auto-pick?
			...
			{ //kicked!
				...
				if (pm->ps->groundEntityNum == ENTITYNUM_NONE && (pm->ps->saberMove == LS_KICK_S || pm->ps->saberMove == LS_KICK_BF || pm->ps->saberMove == LS_KICK_RL))
...
void PM_CheckClearSaberBlock(void)
{
	...
	{ //player
		...
		else if (pm->ps->saberBlocked == BLOCKED_UPPER_LEFT && pm->ps->powerups[PW_SHOCKED] > level.time)
...
qboolean PM_NPCCheckAttackRoll(void)
{
	if (pm->ps->clientNum >= MAX_CLIENTS && !PM_ControlledByPlayer() //NPC
		...
		&& (pm->gent->NPC->rank > RANK_CREWMAN && !Q_irand(0, 3 - g_spskill->integer)) && pm->gent->enemy && fabs(pm->gent->enemy->currentOrigin[2] - pm->ps->origin[2]) < 32 && DistanceHorizontalSquared(pm->gent->enemy->currentOrigin, pm->ps->origin) < (128.0f * 128.0f) && InFOV(pm->gent->enemy->currentOrigin, pm->ps->origin, pm->ps->viewangles, 30, 90))
	{ //stab!
		return qtrue;
	}
	return qfalse;
...
void PM_WeaponLightsaber(void)
{
	...
	if (pm->gent && pm->gent->client && pm->gent->client->NPC_class == CLASS_SABER_DROID)
	...
	if (pm->ps->stats[STAT_WEAPONS] & (1 << WP_SCEPTER) && !pm->ps->dualSabers && pm->gent && pm->gent->weaponModel[1])
	{ //holding scepter in left hand, use dual style
		...
		animLevelOverridden = qtrue;
			 ...
			 && !pm->ps->dualSabers && pm->ps->saber[0].blade[0].active && !pm->ps->saber[0].blade[1].active)
	...
	{
		if ((pm->ps->clientNum < MAX_CLIENTS || PM_ControlledByPlayer()) && cg.saberAnimLevelPending > SS_NONE && cg.saberAnimLevelPending != pm->ps->saberAnimLevel)
		{
			if (!PM_SaberInStart(pm->ps->saberMove) && !PM_SaberInTransition(pm->ps->saberMove) && !PM_SaberInAttack(pm->ps->saberMove))
	...
	*/
	if (PM_InKnockDown(pm->ps) || PM_InRoll(pm->ps))
	{ //in knockdown
		if (pm->ps->legsAnim == BOTH_ROLL_F && pm->ps->legsAnimTimer <= 250)
		{
			if ((pm->cmd.buttons & BUTTON_ATTACK) || PM_NPCCheckAttackRoll())
			{
	...
	if (pm->ps->torsoAnim == BOTH_FORCELONGLEAP_LAND || (pm->ps->torsoAnim == BOTH_FORCELONGLEAP_START && !(pm->cmd.buttons & BUTTON_ATTACK)))
	...
	if (pm->ps->legsAnim == BOTH_FLIP_HOLD7 && !(pm->cmd.buttons & BUTTON_ATTACK))
	...
	if (pm->ps->saberMoveNext != LS_NONE && (pm->ps->saberMove == LS_READY || pm->ps->saberMove == LS_NONE)) //ready for another one
	...
	{ //we're not stuck in a broken parry
		...
		{																			   //guiding saber
			...
			else if (pm->ps->weapon == WP_SABER && (!pm->ps->dualSabers || !pm->ps->saber[1].Active()))
			{ //guiding saber
				if (saberInAir)
				{
	...
	if (PM_LockedAnim(pm->ps->torsoAnim) && pm->ps->torsoAnimTimer)
	{ //can't interrupt these anims ever
		return;
	}
	if (PM_SuperBreakLoseAnim(pm->ps->torsoAnim) && pm->ps->torsoAnimTimer)
	{ //don't interrupt these anims
		return;
	}
	if (PM_SuperBreakWinAnim(pm->ps->torsoAnim) && pm->ps->torsoAnimTimer)
	{ //don't interrupt these anims
		return;
	}
	...
	if (pm->ps->weaponTime > 0)
	{
		...
		{
			if (!pm->cmd.rightmove && !pm->cmd.forwardmove && (pm->cmd.buttons & BUTTON_ATTACK))
			{
				/*
	...
	{ //Just selected the weapon
		...
		else
		{
			...
			if (pm->ps->weapon == WP_SABER && pm->ps->saberInFlight && saberInAir && (!pm->ps->dualSabers || !pm->ps->saber[1].Active()))
			{ //guiding saber
				if (!PM_ForceAnim(pm->ps->torsoAnim) || pm->ps->torsoAnimTimer < 300)
				{ //don't interrupt a force power anim
					if (pm->ps->torsoAnim != BOTH_LOSE_SABER || !pm->ps->torsoAnimTimer)
	...
	{ //trying to do a kick
		//FIXME: in-air kicks?
		if (pm->ps->saberAnimLevel == SS_STAFF && (pm->ps->clientNum >= MAX_CLIENTS || PM_ControlledByPlayer()))
		{ //NPCs spin the staff
	...
	if (!delayed_fire)
	{
		...
		{
			int both = qfalse;
			if (pm->ps->torsoAnim == BOTH_FORCELONGLEAP_ATTACK || pm->ps->torsoAnim == BOTH_FORCELONGLEAP_LAND)
			...
			if (anim == -1)
			{
				...
				{	 //get attack move from movement command
					...
					if (pm->ps->clientNum >= MAX_CLIENTS && !PM_ControlledByPlayer() && (Q_irand(0, pm->ps->forcePowerLevel[FP_SABER_OFFENSE] - 1) || (pm->gent && pm->gent->enemy && pm->gent->enemy->client && PM_InKnockDownOnGround(&pm->gent->enemy->client->ps)) //enemy knocked down, use some logic
					...
					{
						newmove = PM_SaberAttackForMovement(pm->cmd.forwardmove, pm->cmd.rightmove, curmove);
						if ((PM_SaberInBounce(curmove) || PM_SaberInBrokenParry(curmove)) && saberMoveData[newmove].startQuad == saberMoveData[curmove].endQuad)
...
//---------------------------------------
{
	...
	else if ((pm->ps->weapon == WP_BRYAR_PISTOL && pm->cmd.buttons & BUTTON_ALT_ATTACK) || (pm->ps->weapon == WP_BLASTER_PISTOL && pm->cmd.buttons & BUTTON_ALT_ATTACK))
	{
...
{
	if (npc_class == CLASS_PROBE || npc_class == CLASS_SEEKER || npc_class == CLASS_INTERROGATOR || npc_class == CLASS_SENTRY || npc_class == CLASS_REMOTE)
	{
		return qtrue;
	}
	return qfalse;
...
void PM_WeaponWampa(void)
{
	...
	if (pm->ps->weapon == WP_SABER && (pm->cmd.buttons & BUTTON_ATTACK) && pm->ps->torsoAnim == BOTH_HANG_IDLE)
	{
		pm->ps->SaberActivate();
...
static void PM_Weapon(void)
{
	...
	if (pm->gent && pm->gent->client)
	{
		...
		{ //FIXME: this is going to fire off one frame before you expect, actually
			...
			{ //just finished delay timer
				...
				pm->gent->client->fireDelay = 0;
				delayed_fire = qtrue;
				if ((pm->ps->clientNum < MAX_CLIENTS || PM_ControlledByPlayer()) && pm->ps->weapon == WP_THERMAL && pm->gent->alt_fire)
				{
					pm->cmd.buttons |= BUTTON_ALT_ATTACK;
				}
			}
	...
	{ //ready to throw thermal again, add it
		if (pm->ps->weapon == WP_THERMAL && pm->gent->weaponModel[0] == -1)
	...
	if (!delayed_fire)
	{
		...
		if (!(pm->cmd.buttons & (BUTTON_ATTACK | BUTTON_ALT_ATTACK)))
		{
			...
			if (pm->ps->weapon == WP_MELEE && (pm->ps->clientNum < MAX_CLIENTS || PM_ControlledByPlayer()) && PM_KickMove(pm->ps->saberMove))
	...
	*/
			...
			case WP_MELEE:
				...
				if (pm->gent && pm->gent->client)
				{
					...
					{
						int anim = -1;
						if ((pm->ps->clientNum < MAX_CLIENTS || PM_ControlledByPlayer()) && g_debugMelee->integer)
						{
							if ((pm->cmd.buttons & BUTTON_ALT_ATTACK))
							{
								if ((pm->cmd.buttons & BUTTON_ATTACK))
	...
	{
		if (pm->gent && pm->gent->owner && pm->gent->owner->e_UseFunc == useF_eweb_use)
	...
	else if ((pm->ps->weapon == WP_MELEE && (pm->ps->clientNum >= MAX_CLIENTS || !g_debugMelee->integer)) || pm->ps->weapon == WP_TUSKEN_STAFF || (pm->ps->weapon == WP_TUSKEN_RIFLE && !(pm->cmd.buttons & BUTTON_ALT_ATTACK)))
	{
		PM_AddEvent(EV_FIRE_WEAPON);
...
//-------------------------------------------
{
	...
	{ //don't let the alt-attack be interpreted as an actual attack command
		...
		if (pm->ps->saberAnimLevel != SS_STAFF && pm->ps->saber[0].throwable)
		{ //using a throwable saber, so remove the saber throw button
			if (!g_saberNewControlScheme->integer && PM_CanDoKata())
	...
	if (pm->ps->weapon == WP_DISRUPTOR && pm->gent && (pm->gent->s.number < MAX_CLIENTS || G_ControlledByPlayer(pm->gent)) && pm->ps->weaponstate != WEAPON_DROPPING)
	{
		...
		if (!(pm->ps->eFlags & EF_ALT_FIRING) && (pm->cmd.buttons & BUTTON_ALT_ATTACK) && (pm->cmd.upmove < 0 || (!pm->cmd.forwardmove && !pm->cmd.rightmove)))
		{
...
*/
	...
	if (saberMove != LS_INVALID)
	{
		...
		else if (pm->ps->torsoAnimTimer && !pm->ps->weaponTime)
		{
			PM_SetSaberMove(LS_READY);
			pm->ps->saberMove = LS_READY;
			pm->ps->weaponstate = WEAPON_IDLE;
...
void Pmove(pmove_t *pmove)
{
	...
	{
		if (pm->ps->vehTurnaroundIndex && pm->ps->vehTurnaroundTime > pm->cmd.serverTime)
	...
	if (pm->ps && ((pm->ps->eFlags & EF_LOCKED_TO_WEAPON) || (pm->ps->eFlags & EF_HELD_BY_RANCOR) || (pm->ps->eFlags & EF_HELD_BY_WAMPA) || (pm->ps->eFlags & EF_HELD_BY_SAND_CREATURE)))
	{ //in an emplaced gun
		PM_NoclipMove();
	}

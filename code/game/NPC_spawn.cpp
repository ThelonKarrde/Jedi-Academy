...
void NPC_SetMiscDefaultData(gentity_t *ent)
{
	...
	if (Q_stricmp("DKothos", ent->NPC_type) == 0 || Q_stricmp("VKothos", ent->NPC_type) == 0)
	{
	...
	if (!Q_stricmp("emperor", ent->NPC_type) || !Q_stricmp("cultist_grip", ent->NPC_type) || !Q_stricmp("cultist_drain", ent->NPC_type) || !Q_stricmp("cultist_lightning", ent->NPC_type))
	{											//FIXME: extern this into NPC.cfg?
	...
	//==================
	if (ent->client->ps.saber[0].type != SABER_NONE && (!(ent->NPC->aiFlags & NPCAI_MATCHPLAYERWEAPON) || !ent->weaponModel[0]))
	...
	if (ent->client->NPC_class == CLASS_DESANN || ent->client->NPC_class == CLASS_TAVION || ent->client->NPC_class == CLASS_LUKE || ent->client->NPC_class == CLASS_KYLE || Q_stricmp("tavion_scepter", ent->NPC_type) == 0 || Q_stricmp("alora_dual", ent->NPC_type) == 0)
	{
		ent->NPC->aiFlags |= NPCAI_BOSS_CHARACTER;
	}
	else if (Q_stricmp("alora", ent->NPC_type) == 0 || Q_stricmp("rosh_dark", ent->NPC_type) == 0)
	{
	...
	case TEAM_PLAYER:
		...
		else if (ent->client->NPC_class == CLASS_JEDI || ent->client->NPC_class == CLASS_KYLE || ent->client->NPC_class == CLASS_LUKE)
		{ //good jedi
		...
		{
			if (ent->client->ps.weapon != WP_NONE && ent->client->ps.weapon != WP_SABER		 //sabers done above
		...
		if (ent->client->NPC_class == CLASS_PLAYER || ent->client->NPC_class == CLASS_VEHICLE || (ent->spawnflags & SFB_CINEMATIC))
		{
	...
	default:
		ent->NPC->defaultBehavior = BS_DEFAULT;
		if (ent->client->ps.weapon != WP_NONE && ent->client->ps.weapon != WP_MELEE && ent->client->ps.weapon != WP_SABER //sabers done above
			&& (!(ent->NPC->aiFlags & NPCAI_MATCHPLAYERWEAPON) || !ent->weaponModel[0]))								  //they do this themselves
		{
			G_CreateG2AttachedWeaponModel(ent, weaponData[ent->client->ps.weapon].weaponMdl, ent->handRBolt, 0);
		}
	...
	if (ent->client->ps.weapon == WP_NOGHRI_STICK && ent->weaponModel[0])
	{
		ent->genericBolt1 = gi.G2API_AddBolt(&ent->ghoul2[ent->weaponModel[0]], "*flash");
	}
...
int NPC_WeaponsForTeam(team_t team, int spawnflags, const char *NPC_type)
{
	...
	case TEAM_PLAYER:
		...
		if (Q_stricmpn("prisoner", NPC_type, 8) == 0 || Q_stricmpn("elder", NPC_type, 5) == 0)
		{
			return WP_NONE;
		}
...
void NPC_Begin(gentity_t *ent)
{
	...
	else if (ent->NPC->stats.health) // Was health supplied in NPC.cfg?
	{
		...
		{ // up everyone except jedi
			if (!Q_stricmp("tavion_sith_sword", ent->NPC_type) || !Q_stricmp("tavion_scepter", ent->NPC_type) || !Q_stricmp("kyle_boss", ent->NPC_type) || !Q_stricmp("alora_dual", ent->NPC_type) || !Q_stricmp("alora_boss", ent->NPC_type))
...
void NPC_Kill_f(void)
{
	...
	for (n = 1; n < ENTITYNUM_MAX_NORMAL; n++)
	{
		...
		{ //a spawner
			if ((player->targetname && Q_stricmp(name, player->targetname) == 0) || Q_stricmp(name, "all") == 0)
			{

...
void RT_FireDecide(void)
{
	...
	if (NPC->client->ps.groundEntityNum == ENTITYNUM_NONE && NPC->client->ps.forceJumpZStart && !PM_FlippingAnim(NPC->client->ps.legsAnim) && !Q_irand(0, 10))
	{ //take off
		RT_FlyStart(NPC);
	}
	...
	if (TIMER_Done(NPC, "nextAttackDelay") && TIMER_Done(NPC, "flameTime"))
	{
		if (NPC_ClearLOS(NPC->enemy))
		{
			...
			{																																																																		   //can we shoot our target?
				if ((NPC->client->ps.weapon == WP_ROCKET_LAUNCHER || (NPC->client->ps.weapon == WP_CONCUSSION && !(NPCInfo->scriptFlags & SCF_ALT_FIRE)) || (NPC->client->ps.weapon == WP_FLECHETTE && (NPCInfo->scriptFlags & SCF_ALT_FIRE))) && enemyDist < MIN_ROCKET_DIST_SQUARED) //128*128
				{
					enemyCS = qfalse; //not true, but should stop us from firing
					hitAlly = qtrue;  //us!
		...
		{ //try to shoot if it's time
			if (TIMER_Done(NPC, "nextAttackDelay"))
			{
				...
				{
					if ((ucmd.buttons & BUTTON_ATTACK) && !Q_irand(0, altChance))
				...
				{
					if ((ucmd.buttons & BUTTON_ATTACK) && Q_irand(0, altChance * 5))
					{ //fire the beam shot
						ucmd.buttons &= ~BUTTON_ATTACK;
						ucmd.buttons |= BUTTON_ALT_ATTACK;
...
{ //switch to seeker AI for a while
	if (TIMER_Done(self, "jetRecharge") && !RT_Flying(self))
	{
...
void RT_Flying_MaintainHeight(void)
{
	...
	*/
	...
	if (NPC->enemy && (!Q3_TaskIDPending(NPC, TID_MOVE_NAV) || !NPCInfo->goalEntity))
	{
		if (TIMER_Done(NPC, "heightChange"))
		{
			...
			if (NPC->enemy->client && NPC->enemy->client->ps.groundEntityNum == ENTITYNUM_NONE && (NPC->enemy->client->ps.forcePowersActive & (1 << FP_LEVITATION)))
...
{
	if (Q3_TaskIDPending(NPC, TID_MOVE_NAV) && UpdateGoal())

...
qboolean Jedi_CultistDestroyer(gentity_t *self)
{
	...
	if (self->client->NPC_class == CLASS_REBORN && self->s.weapon == WP_MELEE && Q_stricmp("cultist_destroyer", self->NPC_type) == 0)
	{
		return qtrue;
	}
	return qfalse;
...
{
	if (!NPC->ghoul2.size() || NPC->weaponModel[1] <= 0)
	{
		return;
	}
	...
	if (NPC->genericBolt1 != -1)
	{
		...
		for (int time = curTime - 25; time <= curTime + 25 && !hit; time += 25)
		{
			...
			{ //hit something
				...
				if (traceEnt->takedamage && trace.entityNum != lastHit && (!traceEnt->client || traceEnt == NPC->enemy || traceEnt->client->NPC_class != NPC->client->NPC_class))
				{														  //smack
...
void Tavion_ScepterSlam(void)
{
	...
	if (boltIndex != -1)
	{
		...
		for (i = 0; i < numEnts; i++)
		{
			...
			if ((radiusEnts[i]->client->ps.eFlags & EF_HELD_BY_RANCOR) || (radiusEnts[i]->client->ps.eFlags & EF_HELD_BY_WAMPA))
			...
			if (dist <= radius)
			{
				...
				if (radiusEnts[i]->client && radiusEnts[i]->client->NPC_class != CLASS_RANCOR && radiusEnts[i]->client->NPC_class != CLASS_ATST)
				{
					...
					{
						if (dist < halfRad || radiusEnts[i]->client->ps.groundEntityNum != ENTITYNUM_NONE)
...
void Tavion_SithSwordRecharge(void)
{
	if (NPC->client->ps.torsoAnim != BOTH_TAVION_SWORDPOWER && NPC->count && TIMER_Done(NPC, "rechargeDebounce") && NPC->weaponModel[0] != -1)
	{
...
{
	if (NPC && NPC->client && NPC->client->NPC_class == CLASS_SHADOWTROOPER && Q_stricmpn("shadowtrooper", NPC->NPC_type, 13) == 0)
	{
		...
		else if (NPC->health > 0 && !NPC->client->ps.saberInFlight && !(NPC->client->ps.eFlags & EF_FORCE_GRIPPED) && !(NPC->client->ps.eFlags & EF_FORCE_DRAINED) && NPC->painDebounceTime < level.time)
...
{
	if (TIMER_Done(NPC, "chatter") && !Q_irand(0, 3) && NPCInfo->blockedSpeechDebounceTime < level.time && jediSpeechDebounceTime[NPC->client->playerTeam] < level.time)
	{
		int event = -1;
		if (NPC->enemy && NPC->enemy->client && (NPC->enemy->client->NPC_class == CLASS_RANCOR || NPC->enemy->client->NPC_class == CLASS_WAMPA || NPC->enemy->client->NPC_class == CLASS_SAND_CREATURE))
		{	//never taunt these mindless creatures
			//NOTE: howlers?  tusken?  etc?  Only reborn?
		}
		else
		{
			if (NPC->client->playerTeam == TEAM_PLAYER && NPC->enemy && NPC->enemy->client && NPC->enemy->client->NPC_class == CLASS_JEDI)
...
static void Jedi_AdjustSaberAnimLevel(gentity_t *self, int newLevel)
{
	...
	if (self->client->playerTeam == TEAM_ENEMY)
	{
		...
		if (!Q_stricmp("cultist_saber_all", self->NPC_type) || !Q_stricmp("cultist_saber_all_throw", self->NPC_type))
		{ //use any, regardless of rank, etc.
		}
		else if (!Q_stricmp("cultist_saber", self->NPC_type) || !Q_stricmp("cultist_saber_throw", self->NPC_type))
		{ //fast only
			self->client->ps.saberAnimLevel = SS_FAST;
		}
		else if (!Q_stricmp("cultist_saber_med", self->NPC_type) || !Q_stricmp("cultist_saber_med_throw", self->NPC_type))
		{ //med only
			self->client->ps.saberAnimLevel = SS_MEDIUM;
		}
		else if (!Q_stricmp("cultist_saber_strong", self->NPC_type) || !Q_stricmp("cultist_saber_strong_throw", self->NPC_type))
		{ //strong only
...
qboolean Kyle_CanDoGrab(void)
{
	...
	{ //Boss Kyle
		...
		{ //have a valid enemy
			...
			{ //okay to grab again
				if (NPC->client->ps.groundEntityNum != ENTITYNUM_NONE && NPC->enemy->client->ps.groundEntityNum != ENTITYNUM_NONE)
				{ //me and enemy are on ground
					if (!PM_InOnGroundAnim(&NPC->enemy->client->ps))
					{
						if ((NPC->client->ps.weaponTime <= 200 || NPC->client->ps.torsoAnim == BOTH_KYLE_GRAB) && !NPC->client->ps.saberInFlight)
						{
...
{ //FIXME: for many of these checks, what we really want is horizontal distance to enemy
	...
	if (enemy_dist < 128 && NPC->enemy && NPC->enemy->client && (NPC->enemy->client->ps.torsoAnim == BOTH_SPINATTACK6 || NPC->enemy->client->ps.torsoAnim == BOTH_SPINATTACK7))
	{ //whoa, back off!!!
	...
	{ //see if we grabbed enemy
		...
		{
			if (Kyle_CanDoGrab() && NPC_EnemyRangeFromBolt(NPC->handRBolt) <= 72.0f)
			{ //grab him!
				Kyle_GrabEnemy();
				return;
			}
	...
	{ //maintain distance
		...
		if (NPC->client->ps.weapon == WP_SABER																			   //using saber
			...
			&& !(NPC->client->ps.forcePowersActive & (1 << FP_SPEED)) && !(NPC->client->ps.saberEventFlags & SEF_INWATER)) //saber not in water
		{																												   //hold it out there
	...
	else if (NPC->enemy->client && NPC->enemy->s.weapon == WP_SABER && NPC->enemy->client->ps.saberLockTime > level.time && NPC->client->ps.saberLockTime < level.time)
	...
	else if (NPC->enemy->s.weapon == WP_TURRET && !Q_stricmp("PAS", NPC->enemy->classname) && NPC->enemy->s.apos.trType == TR_STATIONARY)
	{
	...
	else if (enemy_dist <= 64 && ((NPCInfo->scriptFlags & SCF_DONT_FIRE) || (!Q_stricmp("Yoda", NPC->NPC_type) && !Q_irand(0, 10))))
	{ //can't use saber and they're in striking range
		if (!Q_irand(0, 5) && InFront(NPC->enemy->currentOrigin, NPC->currentOrigin, NPC->client->ps.viewangles, 0.2f))
		{
			...
			else
			{
				if (Jedi_DecideKick())
				{ //let's try a kick
					if (G_PickAutoMultiKick(NPC, qfalse, qtrue) != LS_NONE || (G_CanKickEntity(NPC, NPC->enemy) && G_PickAutoKick(NPC, NPC->enemy, qtrue) != LS_NONE))
					{ //kicked!
	...
	else if (enemy_dist <= 64 && NPC->max_health - NPC->health > NPC->max_health * 0.25f //lost over 1/4 of our health
			 && NPC->client->ps.forcePowersKnown & (1 << FP_DRAIN)						 //know how to drain
			 && WP_ForcePowerAvailable(NPC, FP_DRAIN, 20)								 //have enough power
			 && !Q_irand(0, 10) && InFront(NPC->enemy->currentOrigin, NPC->currentOrigin, NPC->client->ps.viewangles, 0.2f))
	{
		TIMER_Set(NPC, "draining", 3000);
		TIMER_Set(NPC, "attackDelay", 3000);
		Jedi_Advance();
	...
	{ //we're too damn close!
		...
		else if (NPC->client->ps.stats[STAT_WEAPONS] & (1 << WP_SCEPTER) && !Q_irand(0, 20))
		{
	...
	{ //we're way out of range
		...
		if (NPCInfo->stats.aggression < Q_irand(0, 20) && NPC->health < NPC->max_health * 0.75f && !Q_irand(0, 2))
		{
			if (NPC->enemy && NPC->enemy->s.number < MAX_CLIENTS && NPC->client->NPC_class != CLASS_KYLE && ((NPCInfo->aiFlags & NPCAI_BOSS_CHARACTER) || NPC->client->NPC_class == CLASS_SHADOWTROOPER) && Q_irand(0, 3 - g_spskill->integer))
			{ //hmm, bosses should do this less against the player
			}
			else if (NPC->client->ps.saber[0].type == SABER_SITH_SWORD && NPC->weaponModel[0] != -1)
			{
				Tavion_SithSwordRecharge();
				usedForce = qtrue;
			}
			else if ((NPC->client->ps.forcePowersKnown & (1 << FP_HEAL)) != 0 && (NPC->client->ps.forcePowersActive & (1 << FP_HEAL)) == 0 && Q_irand(0, 1))
			{
				ForceHeal(NPC);
				usedForce = qtrue;
				//FIXME: check level of heal and know not to move or attack when healing
			}
			else if ((NPC->client->ps.forcePowersKnown & (1 << FP_PROTECT)) != 0 && (NPC->client->ps.forcePowersActive & (1 << FP_PROTECT)) == 0 && Q_irand(0, 1))
			{
				ForceProtect(NPC);
				usedForce = qtrue;
			}
			else if ((NPC->client->ps.forcePowersKnown & (1 << FP_ABSORB)) != 0 && (NPC->client->ps.forcePowersActive & (1 << FP_ABSORB)) == 0 && Q_irand(0, 1))
			{
				ForceAbsorb(NPC);
				usedForce = qtrue;
			}
			else if ((NPC->client->ps.forcePowersKnown & (1 << FP_RAGE)) != 0 && (NPC->client->ps.forcePowersActive & (1 << FP_RAGE)) == 0 && Q_irand(0, 1))
			{
				Jedi_Rage();
				usedForce = qtrue;
			}
		...
		{ //approach enemy
			if (!usedForce)
			{
				if (NPC->enemy && NPC->enemy->client && (NPC->enemy->client->ps.torsoAnim == BOTH_SPINATTACK6 || NPC->enemy->client->ps.torsoAnim == BOTH_SPINATTACK7))
				{ //stay put!
				}
				else
				{
	...
	*/
	...
	{						  //we're out of striking range and we are allowed to attack
		...
		{ //They're being gripped, rush them!
			...
			if ((NPCInfo->rank >= RANK_LT_JG || WP_ForcePowerUsable(NPC, FP_SABERTHROW, 0)) && !Q_irand(0, 5) && !(NPC->client->ps.forcePowersActive & (1 << FP_SPEED)) && !(NPC->client->ps.saberEventFlags & SEF_INWATER)) //saber not in water
			{																																																				 //throw saber
				ucmd.buttons |= BUTTON_ALT_ATTACK;
			}
		}
		...
		else
		{
			...
			if (NPC->client->NPC_class == CLASS_KYLE && (NPC->spawnflags & 1) && (NPC->enemy && NPC->enemy->client && !NPC->enemy->client->ps.saberInFlight) && TIMER_Done(NPC, "kyleTakesSaber") && !Q_irand(0, 20))
			{
			...
			else
			{
				...
				else if (NPC->enemy && NPC->enemy->s.number < MAX_CLIENTS && ((NPCInfo->aiFlags & NPCAI_BOSS_CHARACTER) || NPC->client->NPC_class == CLASS_SHADOWTROOPER))
				{ //hmm, bosses do this less against player
					chanceScale = 8 - g_spskill->integer * 2;
				}
				else if (NPC->client->NPC_class == CLASS_DESANN || !Q_stricmp("Yoda", NPC->NPC_type))
				...
				if (chanceScale && (enemy_dist > Q_irand(100, 200) || (NPCInfo->scriptFlags & SCF_DONT_FIRE) || (!Q_stricmp("Yoda", NPC->NPC_type) && !Q_irand(0, 3))) && enemy_dist < 500 && (Q_irand(0, chanceScale * 10) < 5 || (NPC->enemy->client && NPC->enemy->client->ps.weapon != WP_SABER && !Q_irand(0, chanceScale))))
				{ //else, randomly try some kind of attack every now and then
						...
						(!Q_irand(0, 1) || NPC->s.weapon != WP_SABER))
					{
						...
						else if (NPC->health < NPC->max_health * 0.75f && Q_irand(FORCE_LEVEL_0, NPC->client->ps.forcePowerLevel[FP_DRAIN]) > FORCE_LEVEL_1 && WP_ForcePowerUsable(NPC, FP_DRAIN, 0) && ((NPCInfo->scriptFlags & SCF_DONT_FIRE) && Q_stricmp("cultist_drain", NPC->NPC_type) || Q_irand(0, 1)))
						{
							ForceDrain2(NPC);
							NPC->client->ps.weaponTime = Q_irand(1000, 3000 + (g_spskill->integer * 500));
						...
						else if (WP_ForcePowerUsable(NPC, FP_GRIP, 0) && NPC->enemy && InFOV(NPC->enemy->currentOrigin, NPC->currentOrigin, NPC->client->ps.viewangles, 20, 30))
						{
							//taunt
							if (TIMER_Done(NPC, "chatter") && jediSpeechDebounceTime[NPC->client->playerTeam] < level.time && NPCInfo->blockedSpeechDebounceTime < level.time)
							{
						...
						{
							if (WP_ForcePowerUsable(NPC, FP_SABERTHROW, 0) && !(NPC->client->ps.forcePowersActive & (1 << FP_SPEED)) && !(NPC->client->ps.saberEventFlags & SEF_INWATER)) //saber not in water
							{																																							  //throw saber
								ucmd.buttons |= BUTTON_ALT_ATTACK;
							}
						}
					...
					{
						if ((NPCInfo->rank >= RANK_LT_JG || WP_ForcePowerUsable(NPC, FP_SABERTHROW, 0)) && !(NPC->client->ps.forcePowersActive & (1 << FP_SPEED)) && !(NPC->client->ps.saberEventFlags & SEF_INWATER)) //saber not in water
						{																																															   //throw saber
							ucmd.buttons |= BUTTON_ALT_ATTACK;
						}
					}
	...
	if (NPCInfo->stats.aggression > Q_irand(5, 15) && NPC->health < NPC->max_health * 0.75f && !Q_irand(0, 2))
	{
		if ((NPC->client->ps.forcePowersKnown & (1 << FP_RAGE)) != 0 && (NPC->client->ps.forcePowersActive & (1 << FP_RAGE)) == 0)
		{
			Jedi_Rage();
		}
	}
...
qboolean Jedi_DodgeEvasion(gentity_t *self, gentity_t *shooter, trace_t *tr, int hitLoc)
{
	...
	case HL_WAIST:
		...
		else
		{
			...
			if (self->NPC && ((self->NPC->scriptFlags & SCF_NO_ACROBATICS) || PM_InKnockDown(&self->client->ps)))
			{
				return qfalse;
			}
			if (self->client && (self->client->ps.forceRageRecoveryTime > level.time || (self->client->ps.forcePowersActive & (1 << FP_RAGE))))
			...
			if (self->client->NPC_class == CLASS_BOBAFETT || (self->client->NPC_class == CLASS_REBORN && self->s.weapon != WP_SABER) || self->client->NPC_class == CLASS_ROCKETTROOPER)
			{
				self->client->ps.forceJumpCharge = 280; //FIXME: calc this intelligently?
			}
			else
	...
	if (dodgeAnim != -1)
	{
		...
		*/
		...
		{ //player
			if ((self->client->ps.forcePowersActive & (1 << FP_SPEED)))
			{ //in speed
				if (PM_DodgeAnim(self->client->ps.torsoAnim) && !PM_DodgeHoldAnim(self->client->ps.torsoAnim))
				{ //already in a dodge
...
evasionType_t Jedi_CheckFlipEvasions(gentity_t *self, float rightdot, float zdiff)
{
	...
	if (self->client)
	{
		...
		if (self->client->ps.forceRageRecoveryTime > level.time || (self->client->ps.forcePowersActive & (1 << FP_RAGE)))
	...
	{ //already running on a wall
		...
		{ //I'm running on a wall to my left and the attack is on the left
			if (animLength - self->client->ps.legsAnimTimer > 400 && self->client->ps.legsAnimTimer > 400)
		...
		{ //flip off the wall!
			...
			return EVASION_OTHER;
			 ...
			 && (self->NPC->rank == RANK_CREWMAN || self->NPC->rank >= RANK_LT) && Q_irand(0, 1) && !PM_InRoll(&self->client->ps) && !PM_InKnockDown(&self->client->ps) && !PM_SaberInSpecialAttack(self->client->ps.torsoAnim))
	{
		...
		{ //it's clear, let's do it
			...
			if (self->client->NPC_class == CLASS_BOBAFETT || (self->client->NPC_class == CLASS_REBORN && self->s.weapon != WP_SABER))
			{
				G_AddEvent(self, EV_JUMP, 0);
			}
			else
		...
		*/
		{
			...
			else
			{
				...
				*/
				...
				if (self->client->NPC_class == CLASS_ALORA || self->client->NPC_class == CLASS_SHADOWTROOPER || self->client->NPC_class == CLASS_TAVION)
				...
				{ //acrobats aren't so bad
					if (evasionType == EVASION_PARRY || evasionType == EVASION_DUCK_PARRY || evasionType == EVASION_JUMP_PARRY)
					{ //slower with parries
...
{
	if (self->client->ps.torsoAnimTimer > 300 && ((PM_SaberInAttack(self->client->ps.saberMove) && self->client->ps.saberAnimLevel == SS_STRONG) || PM_SpinningSaberAnim(self->client->ps.torsoAnim) || PM_SaberInSpecialAttack(self->client->ps.torsoAnim)
												  ...
												  || PM_FlippingAnim(self->client->ps.torsoAnim) || PM_RollingAnim(self->client->ps.torsoAnim)))
...
qboolean Jedi_InNoAIAnim(gentity_t *self)
{
	...
	if (PM_KickingAnim(NPC->client->ps.legsAnim) || PM_StabDownAnim(NPC->client->ps.legsAnim) || PM_InAirKickingAnim(NPC->client->ps.legsAnim) || PM_InRollIgnoreTimer(&NPC->client->ps) || PM_SaberInKata((saberMoveName_t)NPC->client->ps.saberMove) || PM_SuperBreakWinAnim(NPC->client->ps.torsoAnim) || PM_SuperBreakLoseAnim(NPC->client->ps.torsoAnim))
	{
		return qtrue;
	}
...
void Jedi_CheckJumpEvasionSafety(gentity_t *self, usercmd_t *cmd, evasionType_t evasionType)
{
	...
	{ //on terra firma right now
		if (NPC->client->ps.velocity[2] > 0 || NPC->client->ps.forceJumpCharge || cmd->upmove > 0)
		{ //going to jump
...
evasionType_t Jedi_SaberBlockGo(gentity_t *self, usercmd_t *cmd, vec3_t pHitloc, vec3_t phitDir, gentity_t *incoming, float dist = 0.0f)
{
	...
	if (PM_InSpecialJump(self->client->ps.legsAnim) && PM_SaberInSpecialAttack(self->client->ps.torsoAnim))
	{
		return EVASION_NONE;
	}
	...
	else
	{
		...
		if ((dist > 16 && (Q_irand(0, 2) || saberBusy)) || self->client->ps.saberInFlight || !self->client->ps.SaberActive() || (self->client->NPC_class == CLASS_REBORN && self->s.weapon != WP_SABER))
	...
	if (zdiff >= -5) //was 0
	{
		...
		{
			if (rightdot > 12 || (rightdot > 3 && zdiff < 5) || (!incoming && fabs(hitdir[2]) < 0.25f)) //was normalized, 0.3
			{																							//coming from right
				if (doDodge)
				{
					if (doRoll)
			...
			else if (rightdot < -12 || (rightdot < -3 && zdiff < 5) || (!incoming && fabs(hitdir[2]) < 0.25f)) //was normalized, -0.3
			{																								   //coming from left
				if (doDodge)
				{
					if (doRoll)
		...
		{																   //jump!
			...
			{ //gotta jump!
				...
				{ //superjump
					...
					if (self->NPC && !(self->NPC->scriptFlags & SCF_NO_ACROBATICS) && self->client->ps.forceRageRecoveryTime < level.time && !(self->client->ps.forcePowersActive & (1 << FP_RAGE)) && !PM_InKnockDown(&self->client->ps))
					{
						self->client->ps.forceJumpCharge = 320; //FIXME: calc this intelligently
				...
				{ //normal jump
					//FIXME: check the jump, if can't, then block
					if (self->NPC && !(self->NPC->scriptFlags & SCF_NO_ACROBATICS) && self->client->ps.forceRageRecoveryTime < level.time && !(self->client->ps.forcePowersActive & (1 << FP_RAGE)))
					{
						if ((self->client->NPC_class == CLASS_BOBAFETT || (self->client->NPC_class == CLASS_REBORN && self->s.weapon != WP_SABER)) && !Q_irand(0, 1))
						{ //flip!
							if (rightdot > 0)
							{
								TIMER_Start(self, "strafeLeft", Q_irand(500, 1500));
					...
					if (self->client->NPC_class == CLASS_ALORA || self->client->NPC_class == CLASS_SHADOWTROOPER || self->client->NPC_class == CLASS_TAVION)
					{
						if (!incoming && self->client->ps.groundEntityNum < ENTITYNUM_NONE && !Q_irand(0, 2))
						{
							if (!PM_SaberInAttack(self->client->ps.saberMove) && !PM_SaberInStart(self->client->ps.saberMove) && !PM_InRoll(&self->client->ps) && !PM_InKnockDown(&self->client->ps) && !PM_SaberInSpecialAttack(self->client->ps.torsoAnim))
							{ //do the butterfly!
								int butterflyAnim;
...
static evasionType_t Jedi_CheckEvadeSpecialAttacks(void)
{
	...
	if (!NPC->enemy || NPC->enemy->health <= 0 || !NPC->enemy->client)
	...
	if (NPC->enemy->client->ps.torsoAnim == BOTH_SPINATTACK6 || NPC->enemy->client->ps.torsoAnim == BOTH_SPINATTACK7)
	{ //back away from these
		if ((NPCInfo->aiFlags & NPCAI_BOSS_CHARACTER) || NPC->client->NPC_class == CLASS_SHADOWTROOPER || NPC->client->NPC_class == CLASS_ALORA || Q_irand(0, NPCInfo->rank) > RANK_LT_JG)
	...
	{ //check some other attacks?
		//check roll-stab
		if (NPC->enemy->client->ps.torsoAnim == BOTH_ROLL_STAB || (NPC->enemy->client->ps.torsoAnim == BOTH_ROLL_F && ((NPC->enemy->client->pers.lastCommand.buttons & BUTTON_ATTACK) || (NPC->enemy->client->ps.pm_flags & PMF_ATTACK_HELD))))
		{ //either already in a roll-stab or may go into one
			if ((NPCInfo->aiFlags & NPCAI_BOSS_CHARACTER) || NPC->client->NPC_class == CLASS_SHADOWTROOPER || NPC->client->NPC_class == CLASS_ALORA || Q_irand(-3, NPCInfo->rank) > RANK_LT_JG)
			{ //see if we should evade
				...
				{ //facing me
					...
					{																								//evade!
						...
						if ((NPCInfo->scriptFlags & SCF_NO_ACROBATICS) || !doJump)
						{ //roll?
						...
						//didn't roll, do jump
						if (NPC->s.weapon != WP_SABER || (NPCInfo->aiFlags & NPCAI_BOSS_CHARACTER) || NPC->client->NPC_class == CLASS_SHADOWTROOPER || NPC->client->NPC_class == CLASS_ALORA || Q_irand(-3, NPCInfo->rank) > RANK_CREWMAN)
						{										   //superjump
...
static qboolean Jedi_SaberBlock(void)
{
	...
	*/
	...
	for (saberNum = 0; saberNum < MAX_SABERS; saberNum++)
	{
		...
		{
			if (NPC->enemy->client->ps.saber[saberNum].type != SABER_NONE && NPC->enemy->client->ps.saber[saberNum].blade[bladeNum].length > 0)
	...
	{ //did some sort of evasion
		...
		{ //(not dodge)
			...
			{
				if (NPC->client->NPC_class == CLASS_TAVION || NPC->client->NPC_class == CLASS_SHADOWTROOPER || NPC->client->NPC_class == CLASS_ALORA)
				{
	...
	if (evasionType != EVASION_DUCK_PARRY && evasionType != EVASION_JUMP_PARRY && evasionType != EVASION_JUMP && evasionType != EVASION_DUCK && evasionType != EVASION_FJUMP)
	{
...
static void Jedi_EvasionSaber(vec3_t enemy_movedir, float enemy_dist, vec3_t enemy_dir)
{
	...
	else if (NPC->enemy->client && NPC->enemy->s.weapon == WP_SABER && NPC->enemy->client->ps.saberLockTime > level.time)
	...
	{
		if ((!NPC->client->ps.saberInFlight || (NPC->client->ps.dualSabers && NPC->client->ps.saber[1].Active())) && Jedi_SaberBlock())
		{
			return;
		}
	}
	...
	{ //check to see if he's coming at me
		...
		{ //coming at/facing me!
			...
			if (NPC->client->ps.weaponTime || NPC->client->ps.saberInFlight || NPC->client->NPC_class == CLASS_BOBAFETT || (NPC->client->NPC_class == CLASS_REBORN && NPC->s.weapon != WP_SABER) || NPC->client->NPC_class == CLASS_ROCKETTROOPER)
			...
			case 3:
				...
				if (Jedi_DecideKick() //let's try a kick
					&& (G_PickAutoMultiKick(NPC, qfalse, qtrue) != LS_NONE || (G_CanKickEntity(NPC, NPC->enemy) && G_PickAutoKick(NPC, NPC->enemy, qtrue) != LS_NONE)))
				{ //kicked
					TIMER_Set(NPC, "kickDebounce", Q_irand(3000, 10000));
				}
			...
			default:
				...
				{ //certain chance they will pick an alternative evasion
					...
					else if (shooting_lightning || throwing_saber || enemy_dist < 80)
					{
						...
						if (shooting_lightning || (!Q_irand(0, 2) && NPCInfo->stats.aggression < 4 && TIMER_Done(NPC, "parryTime")))
						{
							...
							else if ((NPCInfo->rank == RANK_CREWMAN || NPCInfo->rank > RANK_LT_JG) && !(NPCInfo->scriptFlags & SCF_NO_ACROBATICS) && NPC->client->ps.forceRageRecoveryTime < level.time && !(NPC->client->ps.forcePowersActive & (1 << FP_RAGE)) && !PM_InKnockDown(&NPC->client->ps))
				...
				{ //strafed
					...
					if (!(NPCInfo->scriptFlags & SCF_NO_ACROBATICS) && NPC->client->ps.forceRageRecoveryTime < level.time && !(NPC->client->ps.forcePowersActive & (1 << FP_RAGE)) && (NPCInfo->rank == RANK_CREWMAN || NPCInfo->rank > RANK_LT_JG) && !PM_InKnockDown(&NPC->client->ps) && !Q_irand(0, 5))
					{ //FIXME: make this a function call?
						//FIXME: check for clearance, safety of landing spot?
						if (NPC->client->NPC_class == CLASS_BOBAFETT || (NPC->client->NPC_class == CLASS_REBORN && NPC->s.weapon != WP_SABER) || NPC->client->NPC_class == CLASS_ROCKETTROOPER)
						{
...
static void Jedi_FaceEnemy(qboolean doPitch)
{
	...
	if (NPC->client->NPC_class == CLASS_BOBAFETT && TIMER_Done(NPC, "flameTime") && NPC->s.weapon != WP_NONE && NPC->s.weapon != WP_DISRUPTOR && (NPC->s.weapon != WP_ROCKET_LAUNCHER || !(NPCInfo->scriptFlags & SCF_ALT_FIRE)) && NPC->s.weapon != WP_THERMAL && NPC->s.weapon != WP_TRIP_MINE && NPC->s.weapon != WP_DET_PACK && NPC->s.weapon != WP_STUN_BATON && NPC->s.weapon != WP_MELEE)
	...
	//Find the desired angles
	if (!NPC->client->ps.saberInFlight && (NPC->client->ps.legsAnim == BOTH_A2_STABBACK1 || NPC->client->ps.legsAnim == BOTH_CROUCHATTACKBACK1 || NPC->client->ps.legsAnim == BOTH_ATTACK_BACK || NPC->client->ps.legsAnim == BOTH_A7_KICK_B))
	{ //point *away*
	...
	else if (NPC->client->ps.legsAnim == BOTH_A7_KICK_RL || NPC->client->ps.legsAnim == BOTH_A7_KICK_BF || NPC->client->ps.legsAnim == BOTH_A7_KICK_S)
	{ //???
	}
	else
	{ //point towards him
	...
	{ //some kind of saber combat event is still pending
		...
		{																					 //we hit our enemy last time we swung, drop our aggression
			if (!Q_irand(0, 1))																 //FIXME: dependant on rank/diff?
			{
				...
				if (!Q_irand(0, 3) && NPCInfo->blockedSpeechDebounceTime < level.time && jediSpeechDebounceTime[NPC->client->playerTeam] < level.time && NPC->painDebounceTime < level.time - 1000)
				{
...
static void Jedi_CombatIdle(int enemy_dist)
{
	...
	if (NPC->client->ps.forcePowersActive & (1 << FP_RAGE) || NPC->client->ps.forceRageRecoveryTime > level.time)
	...
	{ //FIXME: only do this if standing still?
		...
		if (Q_irand(2, chance) < NPCInfo->stats.aggression)
		{
			...
			{ //FIXME: add more taunt behaviors
				...
				if (enemy_dist > 200 && NPC->client->NPC_class != CLASS_BOBAFETT && (NPC->client->NPC_class != CLASS_REBORN || NPC->s.weapon == WP_SABER) && NPC->client->NPC_class != CLASS_ROCKETTROOPER && NPC->client->ps.SaberActive() && !Q_irand(0, 5))
...
static qboolean Jedi_AttackDecide(int enemy_dist)
{
	...
	if (NPC->enemy->client && NPC->enemy->s.weapon == WP_SABER && NPC->enemy->client->ps.saberLockTime > level.time && NPC->client->ps.saberLockTime < level.time)
	{ //enemy is in a saberLock and we are not
		return qfalse;
	}
	...
	{ //tavion, fencers, jedi trainer are all good at following up a parry with an attack
		if ((PM_SaberInParry(NPC->client->ps.saberMove) || PM_SaberInKnockaway(NPC->client->ps.saberMove)) && NPC->client->ps.saberBlocked != BLOCKED_PARRY_BROKEN)
	...
	if (!(ucmd.buttons & BUTTON_ATTACK) && !(ucmd.buttons & BUTTON_ALT_ATTACK) && !(ucmd.buttons & BUTTON_FORCE_FOCUS))
...
static void Jedi_CheckEnemyMovement(float enemy_dist)
{
	...
	if (!(NPCInfo->aiFlags & NPCAI_BOSS_CHARACTER))
	{
		if (PM_KickingAnim(NPC->enemy->client->ps.legsAnim) && NPC->client->ps.groundEntityNum != ENTITYNUM_NONE
			//FIXME: I'm relatively close to him
			&& (NPC->enemy->client->ps.legsAnim == BOTH_A7_KICK_RL || NPC->enemy->client->ps.legsAnim == BOTH_A7_KICK_BF || NPC->enemy->client->ps.legsAnim == BOTH_A7_KICK_S || (NPC->enemy->enemy && NPC->enemy->enemy == NPC)))
		{ //run into the kick!
			ucmd.forwardmove = ucmd.rightmove = ucmd.upmove = 0;
			VectorClear(NPC->client->ps.moveDir);
			Jedi_Advance();
		}
		else if (NPC->enemy->client->ps.torsoAnim == BOTH_A7_HILT && NPC->client->ps.groundEntityNum != ENTITYNUM_NONE)
		...
		else if ((NPC->enemy->client->ps.torsoAnim == BOTH_A6_FB || NPC->enemy->client->ps.torsoAnim == BOTH_A6_LR) && NPC->client->ps.groundEntityNum != ENTITYNUM_NONE)
		{ //run into the attack
		...
		{ //enemy is mad at *me*
			if (NPC->enemy->client->ps.legsAnim == BOTH_JUMPFLIPSLASHDOWN1 || NPC->enemy->client->ps.legsAnim == BOTH_JUMPFLIPSTABDOWN || NPC->enemy->client->ps.legsAnim == BOTH_FLIP_ATTACK7)
			...
			else if (NPC->enemy->client->ps.legsAnim == BOTH_WALL_FLIP_BACK1 || NPC->enemy->client->ps.legsAnim == BOTH_WALL_FLIP_RIGHT || NPC->enemy->client->ps.legsAnim == BOTH_WALL_FLIP_LEFT || NPC->enemy->client->ps.legsAnim == BOTH_WALL_RUN_LEFT_FLIP || NPC->enemy->client->ps.legsAnim == BOTH_WALL_RUN_RIGHT_FLIP)
...
void RT_CheckJump(void)
{
	...
	if (!NPCInfo->goalEntity)
	{
		...
		{
			//FIXME: debounce this?
			if (TIMER_Done(NPC, "roamTime") && Q_irand(0, 9))
	...
	{ //hunt him down
		if ((NPC_ClearLOS(NPC->enemy) || NPCInfo->enemyLastSeenTime > level.time - 500) && InFOV(jumpPos, NPC->currentOrigin, NPC->client->ps.viewangles, 20, 60))
		{
...
static void Jedi_Combat(void)
{
	...
	if (TIMER_Done(NPC, "allyJediDelay"))
	{
		if ((!NPC->client->ps.saberInFlight || (NPC->client->ps.saberAnimLevel == SS_DUAL && NPC->client->ps.saber[1].Active())) && (!(NPC->client->ps.forcePowersActive & (1 << FP_GRIP)) || NPC->client->ps.forcePowerLevel[FP_GRIP] < FORCE_LEVEL_2))
...
void NPC_Jedi_Pain(gentity_t *self, gentity_t *inflictor, gentity_t *other, const vec3_t point, int damage, int mod, int hitLoc)
{
	...
	{ //hit by a client
		...
		if (mod == MOD_FORCE_GRIP || mod == MOD_FORCE_LIGHTNING || mod == MOD_FORCE_DRAIN)
		{ //see if we should turn on absorb
			if ((self->client->ps.forcePowersKnown & (1 << FP_ABSORB)) != 0 && (self->client->ps.forcePowersActive & (1 << FP_ABSORB)) == 0)
		...
		{ //respectable amount of normal damage
			if ((self->client->ps.forcePowersKnown & (1 << FP_PROTECT)) != 0 && (self->client->ps.forcePowersActive & (1 << FP_PROTECT)) == 0)
			{											   //know protect and not already using it
					...
					|| Q_irand(0, g_spskill->integer + 1)) //enemy is player
				{
					if (Q_irand(0, self->NPC->rank) > RANK_ENSIGN)
					{
						if (!Q_irand(0, 1))
						{
							if (other->s.number < MAX_CLIENTS && ((self->NPC->aiFlags & NPCAI_BOSS_CHARACTER) || self->client->NPC_class == CLASS_SHADOWTROOPER) && Q_irand(0, 6 - g_spskill->integer))
							{
							}
							else
							{
...
qboolean Jedi_CheckDanger(void)
{
	...
	{ //run away!
		if (!level.alertEvents[alertEvent].owner || !level.alertEvents[alertEvent].owner->client || (level.alertEvents[alertEvent].owner != NPC && level.alertEvents[alertEvent].owner->client->playerTeam != NPC->client->playerTeam))
		{ //no owner
			return qfalse;
		}
...
qboolean Jedi_CanPullBackSaber(gentity_t *self)
{
	...
	if (self->client->NPC_class == CLASS_SHADOWTROOPER || self->client->NPC_class == CLASS_ALORA || (self->NPC && (self->NPC->aiFlags & NPCAI_BOSS_CHARACTER)))
	{
		return qtrue;
	}
...
qboolean Jedi_CheckKataAttack(void)
{
	...
	{ //only top-level guys and bosses do this
		...
		{ //attacking
			if ((g_saberNewControlScheme->integer && !(ucmd.buttons & BUTTON_FORCE_FOCUS)) || (!g_saberNewControlScheme->integer && !(ucmd.buttons & BUTTON_ALT_ATTACK)))
...
static void Jedi_Attack(void)
{
	...
	if (NPC->painDebounceTime > level.time)
	{
		...
		{ //see if we grabbed enemy
			if (NPC->client->ps.torsoAnimTimer <= 200)
			{
				if (Kyle_CanDoGrab() && NPC_EnemyRangeFromBolt(NPC->handRBolt) < 88.0f)
				{ //grab him!
					Kyle_GrabEnemy();
					return;
				}
	...
	if (NPC->client->ps.saberLockTime > level.time)
	{
		...
		if (NPC->client->ps.forcePowerLevel[FP_PUSH] > FORCE_LEVEL_2 && NPC->client->ps.saberLockTime < level.time + 5000 && !Q_irand(0, 10))
		{
			ForceThrow(NPC, qfalse);
		}
		...
		else
		{
			...
			else if (NPC->client->NPC_class == CLASS_TAVION || NPC->client->NPC_class == CLASS_SHADOWTROOPER || NPC->client->NPC_class == CLASS_ALORA || (NPC->client->NPC_class == CLASS_KYLE && (NPC->spawnflags & 1)))
			{
	...
	if (NPC->enemy)
	{
		if (NPC->enemy->health <= 0 && NPC->enemy->enemy == NPC && (NPC->client->playerTeam != TEAM_PLAYER || (NPC->client->NPC_class == CLASS_KYLE && (NPC->spawnflags & 1) && NPC->enemy == player))) //good guys don't gloat (unless it's Kyle having just killed his student
			...
			else
			{
				...
				{																																					 //keep walking
					...
					{ //got there
						...
						{
							if (NPC->client->ps.saber[0].type == SABER_SITH_SWORD && NPC->weaponModel[0] != -1)
							{
								Tavion_SithSwordRecharge();
							}
							else if ((NPC->client->ps.forcePowersKnown & (1 << FP_HEAL)) != 0 && (NPC->client->ps.forcePowersActive & (1 << FP_HEAL)) == 0)
							{
								ForceHeal(NPC);
							}
						}
	...
	if (NPC->enemy->NPC && NPC->enemy->NPC->charmedTime > level.time)
	...
	if (NPC->client->playerTeam == TEAM_ENEMY && NPC->client->enemyTeam == TEAM_PLAYER && NPC->enemy && NPC->enemy->client && NPC->enemy->client->playerTeam != NPC->client->enemyTeam && OnSameTeam(NPC, NPC->enemy) && !(NPC->svFlags & SVF_LOCKEDENEMY))
	...
	if (!(NPCInfo->scriptFlags & SCF_CHASE_ENEMIES) || ((NPC->client->ps.forcePowersActive & (1 << FP_HEAL)) && NPC->client->ps.forcePowerLevel[FP_HEAL] < FORCE_LEVEL_2))
	...
	if ((NPCInfo->scriptFlags & SCF_DONT_FIRE)																																										  //not allowed to attack
		|| ((NPC->client->ps.forcePowersActive & (1 << FP_HEAL)) && NPC->client->ps.forcePowerLevel[FP_HEAL] < FORCE_LEVEL_3) || ((NPC->client->ps.saberEventFlags & SEF_INWATER) && !NPC->client->ps.saberInFlight)) //saber in water
	{
	...
	{
		if (Q_irand(0, NPC->client->ps.saberAnimLevel) > 0 && Q_irand(0, NPC->max_health + 10) > NPC->health && !Q_irand(0, 3))
	...
	{ //check other special combat behavior
		if (NPC->client->NPC_class != CLASS_BOBAFETT && (NPC->client->NPC_class != CLASS_REBORN || NPC->s.weapon == WP_SABER) && NPC->client->NPC_class != CLASS_ROCKETTROOPER)
		{
			if (NPC->client->NPC_class == CLASS_TAVION || NPC->client->NPC_class == CLASS_SHADOWTROOPER || NPC->client->NPC_class == CLASS_ALORA || (g_spskill->integer && (NPC->client->NPC_class == CLASS_DESANN || NPCInfo->rank >= Q_irand(RANK_CREWMAN, RANK_CAPTAIN))))
			{ //Tavion will kick in force speed if the player does...
				if (NPC->enemy && !NPC->enemy->s.number && NPC->enemy->client && (NPC->enemy->client->ps.forcePowersActive & (1 << FP_SPEED)) && !(NPC->client->ps.forcePowersActive & (1 << FP_SPEED)))
				{
					int chance = 0;
					switch (g_spskill->integer)
					{
		...
		if (NPC->client->NPC_class == CLASS_ALORA)
		{
			...
			{ //chance of doing a special dual saber throw
				if (NPC->client->ps.saberAnimLevel == SS_DUAL && !NPC->client->ps.saberInFlight)
				{
			...
			else if (NPC->enemy && ucmd.forwardmove > 0 && fabs((float)ucmd.rightmove) < 32 && !(ucmd.buttons & BUTTON_WALKING) && !(ucmd.buttons & BUTTON_ATTACK) && NPC->client->ps.saberMove == LS_READY && NPC->client->ps.legsAnim == BOTH_RUN_DUAL)
			{ //running at us, not attacking
				if (Distance(NPC->enemy->currentOrigin, NPC->currentOrigin) > 80)
				{
					if (NPC->client->ps.legsAnim == BOTH_FLIP_F || NPC->client->ps.legsAnim == BOTH_ALORA_FLIP_1 || NPC->client->ps.legsAnim == BOTH_ALORA_FLIP_2 || NPC->client->ps.legsAnim == BOTH_ALORA_FLIP_3)
					{
	...
	if (VectorCompare(NPC->client->ps.moveDir, vec3_origin) && (ucmd.forwardmove || ucmd.rightmove))
...
qboolean Rosh_BeingHealed(gentity_t *self)
{
	if (self && self->NPC && self->client && (self->NPC->aiFlags & NPCAI_ROSH) && (self->flags & FL_UNDYING) && (self->health == 1											//need healing
...
qboolean Rosh_TwinPresent(gentity_t *self)
{
	...
	if (!foundTwin || foundTwin->health < 0)
	{
...
qboolean Rosh_TwinNearBy(gentity_t *self)
{
	...
	if (self->client && foundTwin->client)
	{
		if (Distance(self->currentOrigin, foundTwin->currentOrigin) <= 512.0f && G_ClearLineOfSight(self->client->renderInfo.eyePoint, foundTwin->client->renderInfo.eyePoint, foundTwin->s.number, MASK_OPAQUE))
		{
...
{
	if (NPC->client && NPC->client->leader && NPC->client->leader->client)
	{
		if (DistanceSquared(NPC->client->leader->currentOrigin, NPC->currentOrigin) <= (256 * 256) && G_ClearLineOfSight(NPC->client->leader->client->renderInfo.eyePoint, NPC->client->renderInfo.eyePoint, NPC->s.number, MASK_OPAQUE))
		{
			...
			{
				if (NPC->client->leader->client->ps.legsAnim == BOTH_FORCEHEAL_START && NPC->client->leader->health >= NPC->client->leader->max_health)
				{ //let him get up now
...
{
	if (NPC->client && NPC->client->leader)
	{
		if (Distance(NPC->client->leader->currentOrigin, NPC->currentOrigin) <= 512.0f && G_ClearLineOfSight(NPC->client->leader->client->renderInfo.eyePoint, NPC->client->renderInfo.eyePoint, NPC->s.number, MASK_OPAQUE))
		{
			NPC_FaceEntity(NPC->client->leader, qtrue);
...
{
	if (NPC->client->ps.torsoAnim == BOTH_KYLE_PA_1 || NPC->client->ps.torsoAnim == BOTH_KYLE_PA_2 || NPC->client->ps.torsoAnim == BOTH_KYLE_PA_3 || NPC->client->ps.torsoAnim == BOTH_PLAYER_PA_1 || NPC->client->ps.torsoAnim == BOTH_PLAYER_PA_2 || NPC->client->ps.torsoAnim == BOTH_PLAYER_PA_3 || NPC->client->ps.torsoAnim == BOTH_FORCE_DRAIN_GRAB_END || NPC->client->ps.torsoAnim == BOTH_FORCE_DRAIN_GRABBED)
	{
		NPC_UpdateAngles(qtrue, qtrue);
		return qtrue;
	}
	...
	*/
	...
	if (NPC->client->ps.torsoAnim == BOTH_FORCE_DRAIN_GRAB_START || NPC->client->ps.torsoAnim == BOTH_FORCE_DRAIN_GRAB_HOLD)
	{
		if (!TIMER_Done(NPC, "draining"))
		{ //FIXME: what do we do if we ran out of power?  NPC's can't?
			//FIXME: don't keep turning to face enemy or we'll end up spinning around
	...
	{
		if (NPC->client->ps.torsoAnimTimer <= 1200 && !NPC->count)
		{
			Tavion_ScepterSlam();
			NPC->count = 1;
		}
	...
	if (NPC->client->NPC_class == CLASS_REBORN)
	{
		if ((NPCInfo->aiFlags & NPCAI_HEAL_ROSH))
		{
			...
			if (NPC->client->leader)
			{
				...
				if (NPC->client->leader->client->ps.legsAnim == BOTH_FORCEHEAL_START && TIMER_Done(NPC, "healRoshDebounce"))
				{
					if (Kothos_HealRosh())
					{
						helpingRosh = qtrue;
				...
				*/
				...
				else if (NPC->enemy && DistanceSquared(NPC->enemy->currentOrigin, NPC->currentOrigin) < Twins_DangerDist())
				{
					if (NPC->enemy && Kothos_Retreat())
					{
						...
						{
							if (NPC->painDebounceTime > level.time || (NPC->health < 100 && Q_irand(-20, (g_spskill->integer + 1) * 10) > 0) || !Q_irand(0, 80 - (g_spskill->integer * 20)))
							{
								NPC->flags &= ~FL_LOCK_PLAYER_WEAPONS;
				...
				else if (!G_ClearLOS(NPC, NPC->client->leader) || DistanceSquared(NPC->currentOrigin, NPC->client->leader->currentOrigin) > (512 * 512))
			...
			{																 //Vil and/or Dasariah still around to heal me
				...
				{															 //FIXME: custom anims
					if (Rosh_TwinPresent(NPC))
					{
						if (!NPC->client->ps.weaponTime)
						{ //not attacking
							if (NPC->client->ps.legsAnim != BOTH_FORCEHEAL_START && NPC->client->ps.legsAnim != BOTH_FORCEHEAL_STOP)
...
void NPC_BSJedi_Default(void)
{
	...
	if (NPC->client->ps.saber[0].type == SABER_SITH_SWORD && NPC->weaponModel[0] != -1)
	{
		if (NPC->health < 100 && !Q_irand(0, 20))
		{
			Tavion_SithSwordRecharge();
		}
	}

...
void SP_info_player_deathmatch(gentity_t *ent)
{
	...
	else
	{
		...
		if (saber.brokenSaber1)
			G_ModelIndex(saber.brokenSaber1);
		if (saber.brokenSaber2)
			G_ModelIndex(saber.brokenSaber2);
		if (saber.skin)
...
static void G_SetSkin(gentity_t *ent)
{
	...
	//color tinting
	if (g_char_color_red->integer || g_char_color_green->integer || g_char_color_blue->integer)
	{
...
void G_NextTestAxes(void)
{
	...
	} while (testAxes[1] == testAxes[0] || (testAxes[1] - testAxes[0]) == 3 || (testAxes[0] - testAxes[1]) == 3 || testAxes[2] == testAxes[0] || (testAxes[2] - testAxes[0]) == 3 || (testAxes[0] - testAxes[2]) == 3 || testAxes[2] == testAxes[1] || (testAxes[2] - testAxes[1]) == 3 || (testAxes[1] - testAxes[2]) == 3);

	Com_Printf("Up: %s\nRight: %s\nForward: %s\n", AxesNames[testAxes[0]], AxesNames[testAxes[1]], AxesNames[testAxes[2]]);
	if (testAxes[0] == POSITIVE_X && testAxes[1] == POSITIVE_Z && testAxes[2] == POSITIVE_Y)
	{
		Com_Printf(S_COLOR_RED "WRAPPED\n");
	}
}
...
void G_BoneOrientationsForClass(int NPC_class, char *boneName, Eorientations *oUp, Eorientations *oRt, Eorientations *oFwd)
{
	...
	*oFwd = NEGATIVE_Z;
	...
	case CLASS_HAZARD_TROOPER:
		...
		break;
	case CLASS_SABER_DROID:
		if (Q_stricmp("pelvis", boneName) == 0 || Q_stricmp("thoracic", boneName) == 0)
		{
			*oUp = NEGATIVE_X;
			*oRt = NEGATIVE_Z;
			*oFwd = NEGATIVE_Y;
	...
	case CLASS_WAMPA:
		...
		break;
	case CLASS_ASSASSIN_DROID:
		if (Q_stricmp("pelvis", boneName) == 0 || Q_stricmp("lower_lumbar", boneName) == 0 || Q_stricmp("upper_lumbar", boneName) == 0)
...
qboolean G_SetG2PlayerModelInfo(gentity_t *ent, const char *modelName, const char *customSkin, const char *surfOff, const char *surfOn)
{
	...
	{ // we found the model ok
		...
		else
		{
			...
			else if (!Q_stricmp("gonk", modelName) || !Q_stricmp("seeker", modelName) || !Q_stricmp("remote", modelName) || !Q_stricmpn("r2d2", modelName, 4) || !Q_stricmpn("r5d2", modelName, 4))
			...
			else if (!Q_stricmp("rancor", modelName) || !Q_stricmp("mutant_rancor", modelName))
			{
				ent->handLBolt = gi.G2API_AddBolt(&ent->ghoul2[ent->playerModel], "*l_hand");
				ent->handRBolt = gi.G2API_AddBolt(&ent->ghoul2[ent->playerModel], "*r_hand");
				ent->headBolt = gi.G2API_AddBolt(&ent->ghoul2[ent->playerModel], "*head_eyes");
		...
		else if (!Q_stricmp("rockettrooper", modelName) || !Q_stricmp("hazardtrooper", modelName) || !Q_stricmp("saber_droid", modelName) || !Q_stricmp("assassin_droid", modelName))
		{
			Eorientations oUp, oRt, oFwd;
...
{
	if (g_saber->string && g_saber->string[0] && Q_stricmp("none", g_saber->string) && Q_stricmp("NULL", g_saber->string))
	...
	if (player && player->client && player->client->sess.mission_objectives[LIGHTSIDE_OBJ].status == 2 && g_saberDarkSideSaberColor->integer)
	{ //dark side!
		//always use red
	...
	if (g_saber2->string && g_saber2->string[0] && Q_stricmp("none", g_saber2->string) && Q_stricmp("NULL", g_saber2->string))
	{
...
{
	if (!player || !player->client || player->client->pers.teamState.state != TEAM_ACTIVE || level.time - player->client->pers.enterTime < 100)
...
qboolean ClientSpawn(gentity_t *ent, SavedGameJustLoaded_e eSavedGameJustLoaded)
{
	...
	if ((ent->client->ps.stats[STAT_WEAPONS] & (1 << WP_SABER)) && !ent->client->ps.saberStylesKnown)

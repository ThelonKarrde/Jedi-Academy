...
qboolean G_MoverPush(gentity_t *pusher, vec3_t move, vec3_t amove, gentity_t **obstacle)
{
	...
	*obstacle = NULL;
	...
	if (pusher->currentAngles[0] || pusher->currentAngles[1] || pusher->currentAngles[2] || amove[0] || amove[1] || amove[2])
	{
		float radius;
	...
	for (e = 0; e < listedEntities; e++)
	{
		...
		if (((pusher->spawnflags & 2) && !Q_stricmp("func_breakable", pusher->classname)) || ((pusher->spawnflags & 16) && !Q_stricmp("func_static", pusher->classname)))
...
qboolean G_EntIsUnlockedDoor(int entityNum)
{
	...
	if (G_EntIsDoor(entityNum))
	{
		...
		{ //find out what is targetting it
			...
			{
				if (!Q_stricmp("trigger_multiple", owner->classname) || !Q_stricmp("trigger_once", owner->classname)) //FIXME: other triggers okay too?
				{

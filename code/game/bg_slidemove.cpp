...
qboolean PM_SlideMove(float gravMod)
{
	...
	{																											  //a non-player client, not an NPC under player control
		if (pml.walking																							  //walking on the ground
			|| (pm->ps->groundEntityNum != ENTITYNUM_NONE														  //in air
				...
				&& !(pm->ps->pm_flags & PMF_TIME_KNOCKBACK) && pm->gent && pm->gent->forcePushTime < level.time)) //not being pushed
		{																										  //
	...
	for (bumpcount = 0; bumpcount < numbumps; bumpcount++)
	{
		...
		if ((trace.contents & CONTENTS_BOTCLIP) && (slideMoveContents & CONTENTS_BOTCLIP))
...
void PM_StepSlideMove(float gravMod)
{
	...
	else
	{
		...
		{
			if (pm->ps->clientNum < MAX_CLIENTS && trace.plane.normal[2] < MIN_WALK_NORMAL)
		...
		{
			if (pm->ps->clientNum && isGiant && g_entities[trace.entityNum].client && pm->gent && pm->gent->client && pm->gent->client->NPC_class == CLASS_RANCOR)
			...
			else if (pm->ps->clientNum && isGiant && g_entities[trace.entityNum].client && g_entities[trace.entityNum].client->playerTeam == pm->gent->client->playerTeam)

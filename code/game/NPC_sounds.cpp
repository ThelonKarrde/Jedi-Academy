...
void G_AddVoiceEvent(gentity_t *self, int event, int speakDebounceTime)
{
	...
	{
		if (self->client->ps.powerups[PW_CLOAKED] || self->client->ps.powerups[PW_UNCLOAKING] > level.time)
		{ //I'm cloaked (or still decloaking), so don't talk and give away my position...
			...
			if ((event >= EV_ANGER1 && event <= EV_VICTORY3) || (event >= EV_CHASE1 && event <= EV_SUSPICIOUS5))
			{
				return;
			}

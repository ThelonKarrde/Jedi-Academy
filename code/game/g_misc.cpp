...
void ammo_power_converter_use(gentity_t *self, gentity_t *other, gentity_t *activator)
{
	...
	if (self->setTime < level.time)
	{
		...
		else if (ps->ammo[AMMO_BLASTER] >= ammoData[AMMO_BLASTER].max && ps->ammo[AMMO_POWERCELL] >= ammoData[AMMO_POWERCELL].max && ps->ammo[AMMO_METAL_BOLTS] >= ammoData[AMMO_METAL_BOLTS].max)
		{
			// play full sound
...
void misc_atst_use(gentity_t *self, gentity_t *other, gentity_t *activator)
{
	...
	{ //get out of ATST
		...
		if (legsAnim != BOTH_STAND1 && !PM_InSlopeAnim(legsAnim) && legsAnim != BOTH_TURN_RIGHT1 && legsAnim != BOTH_TURN_LEFT1)

...
qboolean NPC_UpdateAngles(qboolean doPitch, qboolean doYaw)
{
	...
	{
		if (NPC->client->NPC_class == CLASS_ROCKETTROOPER && !NPC->enemy)
	...
	if (!NPC_IsTrooper(NPC) && NPC->enemy && !G_IsRidingVehicle(NPC) && NPC->client->NPC_class != CLASS_VEHICLE)
	{
...
{
	if (self && self->client && self->client->playerTeam == TEAM_PLAYER && self->NPC && self->NPC->charmedTime && (self->NPC->charmedTime < level.time || self->health <= 0))

...
static qboolean NPC_Howler_Move(int randomJumpChance = 0)
{
	...
	if ((!randomJumpChance || Q_irand(0, randomJumpChance)) && NPC_MoveToGoal(qtrue))
	{
		if (VectorCompare(NPC->client->ps.moveDir, vec3_origin) || !NPC->client->ps.speed)
		{ //uh.... wtf?  Got there?
...
static void Howler_TryDamage(int damage, qboolean tongue, qboolean knockdown)
{
	...
	{ //hit *something*
		...
		if (!victim->client || victim->client->NPC_class != CLASS_HOWLER)
		{ //not another howler
...
static void Howler_Howl(void)
{
	...
	for (i = 0; i < numEnts; i++)
	{
		...
		if (distSq <= radiusSquared)
		{
			...
			if (radiusEnts[i]->health > 0 && radiusEnts[i]->client && radiusEnts[i]->client->NPC_class != CLASS_RANCOR && radiusEnts[i]->client->NPC_class != CLASS_ATST && !PM_InKnockDown(&radiusEnts[i]->client->ps))
			{
				if (PM_HasAnimation(radiusEnts[i], BOTH_SONICPAIN_START))
				{
					if (radiusEnts[i]->client->ps.torsoAnim != BOTH_SONICPAIN_START && radiusEnts[i]->client->ps.torsoAnim != BOTH_SONICPAIN_HOLD)
					{
...
static void Howler_Attack(float enemyDist, qboolean howl)
{
	...
	if (!TIMER_Exists(NPC, "attacking"))
	{
		...
		if (NPC->enemy && NPC->enemy->client && PM_InKnockDown(&NPC->enemy->client->ps) && enemyDist <= MIN_DISTANCE)
		{
...
static void Howler_Combat(void)
{
	...
	if (NPC->client->ps.groundEntityNum == ENTITYNUM_NONE)
	{ //not on the ground
		if (NPC->client->ps.legsAnim == BOTH_JUMP1 || NPC->client->ps.legsAnim == BOTH_INAIR1)
...
void NPC_BSHowler_Default(void)
{
	...
	if (NPC->enemy)
	{
		...
		if (!TIMER_Done(NPC, "flee") && NPC_BSFlee()) //this can clear ENEMY
		...
		{ //not standing around
			if (!(NPCInfo->last_ucmd.forwardmove) && !(NPCInfo->last_ucmd.rightmove))
			{ //stood last frame
				if (TIMER_Done(NPC, "walking") && TIMER_Done(NPC, "running"))
		...
		{
			if (!TIMER_Done(NPC, "standing") || !TIMER_Done(NPC, "retreating"))
			{
				G_SoundOnEnt(NPC, CHAN_VOICE, va("sound/chars/howler/idle_hiss%d.mp3", Q_irand(1, 2)));
			}
			else if (!TIMER_Done(NPC, "walking") || NPCInfo->localState == LSTATE_FLEE)
			{
			...
			if (NPCInfo->localState == LSTATE_BERZERK || NPCInfo->localState == LSTATE_FLEE)
			{

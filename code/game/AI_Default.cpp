...
void NPC_BSDefault(void)
{
	...
	{ //still don't have an enemy
		...
		{ //check for alert events
			...
			{					 //heard/saw something
				...
				{ //was a big event
					if (level.alertEvents[alertEvent].owner && level.alertEvents[alertEvent].owner != NPC && level.alertEvents[alertEvent].owner->client && level.alertEvents[alertEvent].owner->health >= 0 && level.alertEvents[alertEvent].owner->client->playerTeam == NPC->client->enemyTeam)
					{ //an enemy
	...
	if (NPC->enemy && !(NPCInfo->scriptFlags & SCF_FORCED_MARCH))
	{
		...
		if (NPC->client->leader && NPCInfo->goalEntity == NPC->client->leader && !Q3_TaskIDPending(NPC, TID_MOVE_NAV))
		{
			NPC_ClearGoal();
		}
		NPC_BSST_Attack();
	...
	if (UpdateGoal())
	{ //have a goal
		if (!NPC->enemy && NPC->client->leader && NPCInfo->goalEntity == NPC->client->leader && !Q3_TaskIDPending(NPC, TID_MOVE_NAV))
		{
			NPC_BSFollowLeader();
		}
		else

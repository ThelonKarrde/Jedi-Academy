...
qboolean G_CheckSaberAllyAttackDelay(gentity_t *self, gentity_t *enemy)
{
	...
	if (self->NPC && self->client->leader == player && self->enemy && self->enemy->s.weapon != WP_SABER && self->s.weapon == WP_SABER)
...
void G_SetEnemy(gentity_t *self, gentity_t *enemy)
{
	...
	if (self->enemy == NULL)
	{
		...
		if (self->client->playerTeam == TEAM_PLAYER && enemy->s.number == 0 && enemy->client && enemy->client->playerTeam == TEAM_PLAYER)
		...
		else if (self->client && self->client->NPC_class == CLASS_KYLE && self->client->leader == player && !TIMER_Done(self, "kyleAngerSoundDebounce"))
		...
		else if (self->client && enemy->client && self->client->playerTeam != enemy->client->playerTeam)
		{
			...
			if (self->forcePushTime < level.time) // not currently being pushed
			{
				...
				{ //team did not have an enemy previously
					if (self->NPC && self->client->playerTeam == TEAM_PLAYER && enemy->s.number < MAX_CLIENTS && self->client->clientInfo.customBasicSoundDir && self->client->clientInfo.customBasicSoundDir[0] && Q_stricmp("jedi2", self->client->clientInfo.customBasicSoundDir) == 0)
					{
						switch (Q_irand(0, 2))
						{
						case 0:
			...
			if (event)
			{ //yell
				if (self->client && self->client->NPC_class == CLASS_KYLE && self->client->leader == player)
		...
		if (self->s.weapon == WP_BLASTER || self->s.weapon == WP_REPEATER ||
			self->s.weapon == WP_THERMAL || self->s.weapon == WP_BLASTER_PISTOL || self->s.weapon == WP_BOWCASTER)
	...
	*/
	...
	for (entNum = 0; entNum < globals.num_entities; entNum++)
	{
		...
		if (newenemy != NPC && (newenemy->client || newenemy->svFlags & SVF_NONNPC_ENEMY) && !(newenemy->flags & FL_NOTARGET) && !(newenemy->s.eFlags & EF_NODRAW))
		{
			if (newenemy->health > 0)
			{
				if ((newenemy->client && NPC_ValidEnemy(newenemy)) || (!newenemy->client && newenemy->noDamageTeam == enemyTeam))
				{ //FIXME:  check for range and FOV or vis?
...
gentity_t *NPC_CheckEnemy(qboolean findNew, qboolean tooFarOk, qboolean setEnemy)
{
	...
	{
		if (NPC->enemy->client->playerTeam && NPC->enemy->client->playerTeam != TEAM_FREE)
		{
			//			assert( NPC->client->playerTeam != NPC->enemy->client->playerTeam);
			if (NPC->client->playerTeam != NPC->enemy->client->playerTeam && NPC->client->enemyTeam != TEAM_FREE && NPC->client->enemyTeam != NPC->enemy->client->playerTeam)
			{
...
{
	if (NPC->client && !G_CanPickUpWeapons(NPC))
	{ //this NPC can't pick up weapons...
		return;
	}
	if (NPC->s.weapon == WP_NONE && NPC->enemy)
	{ //if running away because dropped weapon...
		if (NPCInfo->goalEntity && NPCInfo->goalEntity == NPCInfo->tempGoal && NPCInfo->goalEntity->enemy && !NPCInfo->goalEntity->enemy->inuse)

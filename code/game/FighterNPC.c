...
{
	if (gParent && gParent->client && gParent->client->inSpaceIndex && gParent->client->inSpaceIndex < ENTITYNUM_WORLD)
	{
		return qtrue;
	}
	return qfalse;
...
{
	if (FighterOverValidLandingSurface(pVeh) && !parentPS->speed) //stopped
	{
		return qtrue;
	}
	return qfalse;
...
//a predicted event or only doing it game-side. -rich
	...
	*/
	...
	if (pVeh->m_iRemovedSurfaces || parentPS->electrifyTime >= curTime)
	{ //go out of control
	...
	else if (!pVeh->m_pVehicleInfo->Inhabited(pVeh) && parentPS->speed > 0)
	{ //pilot jumped out while we were moving forward (not landing or landed) so just keep the throttle locked
		...
		pVeh->m_ucmd.forwardmove = 127;
			 ...
			 pVeh->m_LandTrace.fraction >= 0.05f)
	{
	...
	if (pVeh->m_pVehicleInfo->strafePerc
		...
		&& pVeh->m_pVehicleInfo->Inhabited(pVeh) //has to have a driver in order to be capable of landing
#endif
		&& !pVeh->m_iRemovedSurfaces && parentPS->electrifyTime < curTime && (pVeh->m_LandTrace.fraction >= 1.0f					   //no grounf
...
static void ProcessOrientCommands(Vehicle_t *pVeh)
{
	...
	if (isLandingOrLanded												   //going slow enough to start landing
		&& !pVeh->m_iRemovedSurfaces && parentPS->electrifyTime < curTime) //not spiraling out of control
	{
		...
		if (pVeh->m_LandTrace.fraction > 0.1f || pVeh->m_LandTrace.plane.normal[2] < MIN_LANDING_SLOPE)
	...
	else if (pVeh->m_pPilot && pVeh->m_pPilot->s.number < MAX_CLIENTS && parentPS->speed > 0.0f) //&& !( pVeh->m_ucmd.forwardmove > 0 && pVeh->m_LandTrace.fraction != 1.0f ) )
	{
		...
		else
		{
			...
			*/
			...
			if (!FighterOverValidLandingSurface(pVeh) || parentPS->speed > MIN_LANDING_SPEED)
	...
	{ //only if capable of landing
		if (!isDead && parentPS->electrifyTime < curTime && (!pVeh->m_pVehicleInfo->surfDestruction || !pVeh->m_iRemovedSurfaces))
...
*/
	...
	if (!pVeh->m_pVehicleInfo->Inhabited(pVeh) && pVeh->m_LandTrace.fraction >= groundFraction && !FighterIsInSpace((gentity_t *)parent) && !FighterSuspended(pVeh, parentPS))
	{
		pVeh->m_ucmd.upmove = 0;
		//pVeh->m_ucmd.forwardmove = 0;
		pVeh->m_vOrientation[PITCH] += pVeh->m_fTimeModifier;
	...
	{																								  //add in strafing roll
		...
		if (!BG_UnrestrainedPitchRoll(riderPS, pVeh))
		{ //cap it reasonably
			if (pVeh->m_pVehicleInfo->rollLimit != -1 && !pVeh->m_iRemovedSurfaces && parentPS->electrifyTime < curTime)
			{
...
static void AnimateVehicle(Vehicle_t *pVeh)
{
...
#endif
	{
		...
		{
			if ((pVeh->m_ucmd.forwardmove < 0 || pVeh->m_ucmd.upmove < 0 || isLanded) && pVeh->m_LandTrace.fraction <= 0.4f && pVeh->m_LandTrace.plane.normal[2] >= MIN_LANDING_SLOPE)

...
{ //FIXME: pick up on bodies of dead buddies?
	...
	else if (!NPC->client->ps.weaponTime && TIMER_Done(NPC, "attackDelay") && TIMER_Done(NPC, "inactiveDelay"))
	{
...
void NPC_BSSaberDroid_Attack(void)
{
	...
	*/
	...
	if (NPC->client->ps.legsAnimTimer && NPC->client->ps.legsAnim != BOTH_A3__L__R) //this one is a running attack
...
void NPC_BSSD_Default(void)
{
	...
	{	 //have an enemy
		if (!NPC->client->ps.SaberActive())
		{
			...
			if (NPC->client->ps.legsAnim == BOTH_TURNOFF || NPC->client->ps.legsAnim == BOTH_STAND1)
			{

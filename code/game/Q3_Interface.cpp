...
static void Q3_SetObjective(const char *ObjEnum, int status)
{
	...
	if (objectiveID == LIGHTSIDE_OBJ && status == SET_OBJ_FAILED)
...
static qboolean Q3_SetNavGoal(int entID, const char *name)
{
	...
	if (Q_stricmp("null", name) == 0 || Q_stricmp("NULL", name) == 0)
	{
...
int CQuake3GameInterface::PlaySound(int taskID, int entID, const char *name, const char *channel)
{
	...
	//
	if (!in_camera ||
		(!g_skippingcin || !g_skippingcin->integer)) // paranoia towards project end <g>
	{
		// Text on

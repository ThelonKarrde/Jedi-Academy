...
qboolean Rancor_CheckAhead(vec3_t end)
{
	...
	if (trace.entityNum < ENTITYNUM_WORLD && G_EntIsBreakable(trace.entityNum, NPC))
		...
		{ //can't macro-nav, just head right for him
			...
			{ //all else fails, look at him
				...
				{ //nothing to destroy?  just go straight at goal dest
					...
					{
						if (TIMER_Done(NPC, "attacking") && TIMER_Done(NPC, "frustrationAttack"))
						{
							float enemyDist = Distance(dest, NPC->currentOrigin);
							if ((!horzClose || !Q_irand(0, 5)) && Q_irand(0, 1))
							{
		...
		if (DistanceSquared(radiusEnts[i]->currentOrigin, boltOrg) <= radiusSquared)
		{
			...
			*/
			...
			{ //smack
				...
				*/
				if ((NPC->spawnflags & SPF_RANCOR_FASTKILL) && radiusEnts[i]->s.number >= MAX_CLIENTS)
				{
				...
				if (radiusEnts[i]->client->NPC_class != CLASS_RANCOR && radiusEnts[i]->client->NPC_class != CLASS_ATST && !(radiusEnts[i]->flags & FL_NO_KNOCKBACK))
				{
...
void Rancor_Smash(void)
{
	...
	for (i = 0; i < numEnts; i++)
	{
		...
		if (distSq <= radiusSquared)
		{
			...
			if (radiusEnts[i]->health > 0 && radiusEnts[i]->client && radiusEnts[i]->client->NPC_class != CLASS_RANCOR && radiusEnts[i]->client->NPC_class != CLASS_ATST)
			{
				if (distSq < halfRadSquared || radiusEnts[i]->client->ps.groundEntityNum != ENTITYNUM_NONE)
...
void Rancor_Attack(float distance, qboolean doCharge, qboolean aimAtBlockedEntity)
{
	if (!TIMER_Exists(NPC, "attacking") && TIMER_Done(NPC, "attackDebounce"))
	{
		...
		{ //holding enemy
			if ((!(NPC->spawnflags & SPF_RANCOR_FASTKILL) || NPC->activator->s.number < MAX_CLIENTS) && NPC->activator->health > 0 && Q_irand(0, 1))
			{ //quick bite
		...
		else if ((NPC->spawnflags & SPF_RANCOR_MUTANT) || distance >= NPC->maxs[0] + (MIN_DISTANCE * NPC->s.modelScale[0]) - 64.0f)
		{ //try to grab
...
void Rancor_Combat(void)
{
	...
	{ //holding my enemy
		...
		else if ((NPC->spawnflags & SPF_RANCOR_FASTKILL) && NPC->activator && NPC->activator->s.number >= MAX_CLIENTS)
		{
			Rancor_Attack(0, qfalse, qfalse);
		}
		else if (NPC->useDebounceTime >= level.time && NPC->activator)
		...
		{
			if (!NPC->useDebounceTime && NPC->activator && NPC->activator->s.number < MAX_CLIENTS)
	...
	if (advance)
	{ //have to get closer
		if ((NPC->spawnflags & SPF_RANCOR_MUTANT) && (!NPC->enemy || !NPC->enemy->client))
		{ //don't do breath attack vs. bbrushes
		}
		else
		{
			vec3_t yawOnlyAngles = {0, NPC->currentAngles[YAW], 0};
			if (NPC->enemy->health > 0 && fabs(distance - (250.0f * NPC->s.modelScale[0])) <= (80.0f * NPC->s.modelScale[0]) && InFOV(NPC->enemy->currentOrigin, NPC->currentOrigin, yawOnlyAngles, 30, 30))
			{
				int chance = 9;
				if ((NPC->spawnflags & SPF_RANCOR_MUTANT))
...
void NPC_Rancor_Pain(gentity_t *self, gentity_t *inflictor, gentity_t *other, const vec3_t point, int damage, int mod, int hitLoc)
{
	...
	if (other && other->inuse && other != self->enemy && !(other->flags & FL_NOTARGET))
	{
		if (!self->count)
		{
			if ((!other->s.number && !Q_irand(0, 3)) || !self->enemy || self->enemy->health == 0 || (self->enemy->client && self->enemy->client->NPC_class == CLASS_RANCOR) || (!Q_irand(0, 4) && DistanceSquared(other->currentOrigin, self->currentOrigin) < DistanceSquared(self->enemy->currentOrigin, self->currentOrigin)))
	...
	if ((hitByRancor || (self->count == 1 && self->activator && !Q_irand(0, 4)) || Q_irand(0, 200) < damage) //hit by rancor, hit while holding live victim, or took a lot of damage
		&& self->client->ps.legsAnim != BOTH_STAND1TO2 && TIMER_Done(self, "takingPain"))
	{
		if (!Rancor_CheckRoar(self))
		{
			if (self->client->ps.legsAnim != BOTH_MELEE1 && self->client->ps.legsAnim != BOTH_MELEE2 && self->client->ps.legsAnim != BOTH_ATTACK2 && self->client->ps.legsAnim != BOTH_ATTACK10 && self->client->ps.legsAnim != BOTH_ATTACK11)
...
void Rancor_FireBreathAttack(void)
{
	...
	if (tr.entityNum < ENTITYNUM_WORLD && traceEnt->takedamage && traceEnt->client)
...
{
	if (NPC->client->ps.legsAnim == BOTH_ATTACK2 || NPC->client->ps.legsAnim == BOTH_ATTACK10 || NPC->client->ps.legsAnim == BOTH_ATTACK11)
	{
...
void NPC_BSRancor_Default(void)
{
	...
	else if (NPC->client->ps.legsAnim == BOTH_ATTACK4 || NPC->client->ps.legsAnim == BOTH_ATTACK6 || NPC->client->ps.legsAnim == BOTH_ATTACK7)
	{
	...
	else if ((NPC->client->ps.legsAnim == BOTH_PAIN2 || NPC->client->ps.legsAnim == BOTH_HOLD_DROP) && NPC->count == 1 && NPC->activator)
	{
	...
	if (NPCInfo->localState == LSTATE_WAITING && TIMER_Done2(NPC, "takingPain", qtrue))
	...
	if (NPC->enemy)
	{
		...
		else if (!NPC->count)
		{
			...
			{ //something in our way
				...
				else
				{
					...
					{ //if it's a client and in our way, get mad at it!
						if (NPCInfo->blockedEntity != NPC->enemy && NPCInfo->blockedEntity->client && NPC_ValidEnemy(NPCInfo->blockedEntity) && !Q_irand(0, 9))
						{
			...
			if (NPC_ValidEnemy(NPC->enemy) == qfalse)
			{
				...
				if (!NPC->enemy->inuse || level.time - NPC->enemy->s.time > Q_irand(10000, 15000) || (NPC->spawnflags & SPF_RANCOR_FASTKILL)) //don't linger on dead bodies
				{																															  //it's been a while since the enemy died, or enemy is completely gone, get bored with him
					if ((NPC->spawnflags & SPF_RANCOR_MUTANT) && player && player->health >= 0)
		...
		Rancor_Combat();
		if (TIMER_Done(NPC, "attacking") && TIMER_Done(NPC, "takingpain") && TIMER_Done(NPC, "confusionDebounce") && NPCInfo->localState == LSTATE_CLEAR && !NPC->count)
		{ //not busy
			if (!ucmd.forwardmove && !ucmd.rightmove && VectorCompare(NPC->client->ps.moveDir, vec3_origin))
			{ //not moving

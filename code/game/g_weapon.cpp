...
bool WP_MissileTargetHint(gentity_t *shooter, vec3_t start, vec3_t out)
{
	...
	if ((!cg.snap) || !cv_autoAim)
		return false;
	if (shooter->s.clientNum != 0)
		return false; // assuming shooter must be client, using 0 for cg_entities[0] a few lines down if you change this
					  //	if (cg_crossHairStatus != 1 || cg_crosshairEntNum < 0 || cg_crosshairEntNum >= ENTITYNUM_WORLD) return false;
	if (cg_crossHairStatus != 1 || g_crosshairEntNum < 0 || g_crosshairEntNum >= ENTITYNUM_WORLD)
		return false;
...
//---------------------------------------------------------
{
	...
	{ //need to loop this in case we hit a Jedi who dodges the shot
		...
		if (traceEnt && (traceEnt->s.weapon == WP_SABER || (traceEnt->client && (traceEnt->client->NPC_class == CLASS_BOBAFETT || traceEnt->client->NPC_class == CLASS_REBORN))))
...
//---------------------------------------------------------
{
	...
	for (int i = 0; i < traces; i++)
	{
		...
		if (!hitDodged)
		{
			if (render_impact)
			{
				if ((tr.entityNum < ENTITYNUM_WORLD && traceEnt->takedamage) || !Q_stricmp(traceEnt->classname, "misc_model_breakable") || traceEnt->s.eType == ET_MOVER)
				{
					// Create a simple impact type mark that doesn't last long in the world
	...
	if (alt_fire)
	{
		...
		if ((lockEntNum > 0 || ent->NPC && lockEntNum >= 0) && lockEntNum < ENTITYNUM_WORLD && lockTime > 0)
		{
			...
			if (dif == 8 || random() * dif > 2 || random() > 0.97f)
			{
				...
				{
					if (missile->enemy->client && (missile->enemy->client->ps.forcePowersKnown & (1 << FP_PUSH)) && missile->enemy->client->ps.forcePowerLevel[FP_PUSH] > FORCE_LEVEL_0)
...
//---------------------------------------------------------
{
	...
	{ //blow once creature is underground (done with anim)
		...
		if (!ent->activator || !ent->activator->client || !ent->activator->client->ps.legsAnimTimer)
	...
	else if (ent->delay > level.time)
	{
		...
		if (ent->has_bounced)
		{
			...
			for (int i = 0; i < count; i++)
			{
				...
				else if (ent_list[i]->client && ent_list[i]->client->NPC_class != CLASS_SAND_CREATURE //ignore sand creatures
						 && ent_list[i]->health > 0)
				{
...
//---------------------------------------------------------
{
	...
	if (ent->health > 0)
	{
		...
		if ((ent->NPC || (ent->s.number && thisIsAShooter)) && ent->enemy)
...
//---------------------------------------------------------
{
	...
	if (ent->s.number == 0 && alert > 0)
	{
		if (ent->client->ps.groundEntityNum == ENTITYNUM_WORLD //FIXME: check for sand contents type?
			&& ent->s.weapon != WP_STUN_BATON && ent->s.weapon != WP_MELEE && ent->s.weapon != WP_TUSKEN_STAFF && ent->s.weapon != WP_THERMAL && ent->s.weapon != WP_TRIP_MINE && ent->s.weapon != WP_DET_PACK)

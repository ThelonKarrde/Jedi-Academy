...
bool Board(Vehicle_t *pVeh, bgEntity_t *pEnt)
{
	...
	{		   //switch to vehicle weapon
		...
		if (ent->client->ps.weapon != WP_SABER && ent->client->ps.weapon != WP_BLASTER)
...
static bool Update(Vehicle_t *pVeh, const usercmd_t *pUmcd)
{
	...
	if (pVeh->m_pVehicleInfo->shieldRechargeMS && parentPS->stats[STAT_ARMOR] > 0			  //still have some shields left
...
maintainSelfDuringBoarding:
	...
	*/
		...
		((nextSpeed > prevSpeed && nextSpeed > halfMaxSpeed && prevSpeed < halfMaxSpeed) || (nextSpeed > halfMaxSpeed && !Q_irand(0, 1000))))
	{
		...
		{
		case 1:
			shiftSound = pVeh->m_pVehicleInfo->soundShift1;
			break;
		case 2:
			shiftSound = pVeh->m_pVehicleInfo->soundShift2;
			break;
		case 3:
			shiftSound = pVeh->m_pVehicleInfo->soundShift3;
			break;
		case 4:
			shiftSound = pVeh->m_pVehicleInfo->soundShift4;
			break;
		}
		if (shiftSound)
		{
...
static bool UpdateRider(Vehicle_t *pVeh, bgEntity_t *pRider, usercmd_t *pUmcd)
{
	...
	if (rider && rider->client && parent && parent->client)
...
static void AttachRiders(Vehicle_t *pVeh)
{
	...
	//attach droid
	if (pVeh->m_pDroidUnit && pVeh->m_iDroidUnitTag != -1)
	{
		mdxaBone_t boltMatrix;
...
int G_ShipSurfaceForSurfName(const char *surfaceName)
{
	...
	if (!Q_strncmp("nose", surfaceName, 4) || !Q_strncmp("f_gear", surfaceName, 6) || !Q_strncmp("glass", surfaceName, 5))
	{
		return SHIPSURF_FRONT;
	}
	...
	if (!Q_strncmp("r_wing1", surfaceName, 7) || !Q_strncmp("r_wing2", surfaceName, 7) || !Q_strncmp("r_gear", surfaceName, 6))
	{
		return SHIPSURF_RIGHT;
	}
	if (!Q_strncmp("l_wing1", surfaceName, 7) || !Q_strncmp("l_wing2", surfaceName, 7) || !Q_strncmp("l_gear", surfaceName, 6))
	{
		return SHIPSURF_LEFT;
	}
	return -1;
...
void G_SetVehDamageFlags(gentity_t *veh, int shipSurf, int damageLevel)
{
	...
	case 3: //destroyed
		...
		{ //destroy the droid if we have one
			if (veh->m_pVehicle && veh->m_pVehicle->m_pDroidUnit)
			{ //we have one
				gentity_t *droidEnt = (gentity_t *)veh->m_pVehicle->m_pDroidUnit;
				if (droidEnt && ((droidEnt->flags & FL_UNDYING) || droidEnt->health > 0))
				{ //boom
					//make it vulnerable
					droidEnt->flags &= ~FL_UNDYING;
					//blow it up
	...
	case 2: //heavy only
		...
		{ //make the droid vulnerable if we have one
			...
			{ //we have one
				gentity_t *droidEnt = (gentity_t *)veh->m_pVehicle->m_pDroidUnit;
				if (droidEnt && (droidEnt->flags & FL_UNDYING))
				{ //make it vulnerab;e
					droidEnt->flags &= ~FL_UNDYING;
				}
			}
...
void G_VehicleSetDamageLocFlags(gentity_t *veh, int impactDir, int deathPoint)
{
	...
	else
	{
		...
		if (veh->m_pVehicle && veh->m_pVehicle->m_pVehicleInfo && veh->m_pVehicle->m_pVehicleInfo->malfunctionArmorLevel && veh->m_pVehicle->m_pVehicleInfo->armor)
		{
...
void G_VehUpdateShields(gentity_t *targ)
{
	if (!targ || !targ->client || !targ->m_pVehicle || !targ->m_pVehicle->m_pVehicleInfo)
	{
		return;
	}

...
int Z_Validate(void)
{
	...
	while (pMemory)
	{
		...
		if (pMemory->eTag != TAG_IMAGE_T && pMemory->eTag != TAG_MODEL_MD3 && pMemory->eTag != TAG_MODEL_GLM && pMemory->eTag != TAG_MODEL_GLA) //don't bother with disk caches as they've already been hit or will be thrown out next
		{
...
#ifdef DEBUG_ZONE_ALLOCS
#define DEF_STATIC(_char) \
	{ZONE_MAGIC, TAG_STATIC, 2, NULL, NULL, "<static>", 0, "", 0}, _char, '\0', { ZONE_MAGIC }
#else
#define DEF_STATIC(_char) \
	{ZONE_MAGIC, TAG_STATIC, 2, NULL, NULL}, _char, '\0', { ZONE_MAGIC }
#endif
...
static void Z_TagDebug_f(void)
{
	...
	while (pMemory)
	{
		if ((pMemory->eTag == eTag || eTag == TAG_ALL) && (!bSnapShotTestActive || (pMemory->iSnapshotNumber == giZoneSnaphotNum && AllTagBlockLabels_Local[psTagStrings[pMemory->eTag]][pMemory->sOptionalLabel] < 0)))
		{

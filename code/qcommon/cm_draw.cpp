...
//OUT:	none
{
	...
	if (DeltaY > DeltaX)
	{
		...
		unsigned short ErrorAdj = unsigned short(((unsigned long)DeltaX << 16) / (unsigned long)DeltaY);
	...
	unsigned short ErrorAdj = unsigned short(((unsigned long)DeltaY << 16) / (unsigned long)DeltaX);
	// Draw all pixels other than the first and last
	while (--DeltaX)
	{
		unsigned short ErrorAccTemp = ErrorAcc; // remember currrent accumulated error

...
long FS_HashFileName(const char *fname, int hashSize)
{
	...
	while (fname[i] != '\0')
	{
		...
		if (letter == PATH_SEP)
			letter = '/'; // damn path names	//mac and unix are different
...
int FS_FOpenFileRead(const char *filename, fileHandle_t *file, qboolean uniqueFILE)
{
	...
	*/
			...
			else if (search->dir)
			{
				...
				//
				if (fs_copyfiles->integer == 2 && fs_cdpath->string[0] && !Q_stricmp(dir->path, fs_basepath->string) && FS_FileCacheable(filename))
				{
...
int FS_ReadFile(const char *qpath, void **buffer)
{
	...
	*buffer = buf;
	...
	// PRECACE CHECKER!
#ifndef FINAL_BUILD
	if (com_sv_running && com_sv_running->integer && cls.state >= CA_ACTIVE)
	{ //com_cl_running

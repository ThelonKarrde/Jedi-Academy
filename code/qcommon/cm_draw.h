...
{
	CPixel32 t;
	t.r = (byte)(((int)x.r + (int)y.r) >> 1);
	t.g = (byte)(((int)x.g + (int)y.g) >> 1);
	t.b = (byte)(((int)x.b + (int)y.b) >> 1);
	t.a = (byte)(((int)x.a + (int)y.a) >> 1);
	return t;
}
...
{
	CPixel32 t;
	t.r = (byte)((x.r * alpha + y.r * inv_alpha) >> 8);
...
public:
	...
	{
		if (ClipLine(x1, y1, x2, y2))
			DrawLineAveNC(x1, y1, x2, y2, color);
	}
	...
	{
		if (ClipLine(x1, y1, x2, y2))
			DrawLineAANC(x1, y1, x2, y2, color);
	}

...
void Netchan_Transmit(netchan_t *chan, int length, const byte *data)
{
	...
	if (length >= FRAGMENT_SIZE)
	{
		...
		do
		{
			...
			{
				Com_Printf("%s send %4i : s=%i ack=%i fragment=%i,%i\n", netsrcString[chan->sock], send.cursize, chan->outgoingSequence - 1, chan->incomingSequence, fragmentStart, fragmentLength);
			}
...
qboolean Netchan_Process(netchan_t *chan, msg_t *msg)
{
	...
	if (showpackets->integer)
	{
		if (fragmented)
		{
			Com_Printf("%s recv %4i : s=%i ack=%i fragment=%i,%i\n", netsrcString[chan->sock], msg->cursize, sequence, sequence_ack, fragmentStart, fragmentLength);
		}
		else
		{
			Com_Printf("%s recv %4i : s=%i ack=%i\n", netsrcString[chan->sock], msg->cursize, sequence, sequence_ack);
		}
	}

...
jinit_c_main_controller(j_compress_ptr cinfo, boolean need_full_buffer)
{
  ...
  jmain = (my_main_ptr)(*cinfo->mem->alloc_small)((j_common_ptr)cinfo, JPOOL_IMAGE,
  ...
  if (need_full_buffer)
  {
    ...
    for (ci = 0, compptr = cinfo->comp_info; ci < cinfo->num_components;
         ci++, compptr++)
    {
      jmain->whole_image[ci] = (*cinfo->mem->request_virt_sarray)((j_common_ptr)cinfo, JPOOL_IMAGE, FALSE,
  ...
  else
  {
    ...
    for (ci = 0, compptr = cinfo->comp_info; ci < cinfo->num_components;
         ci++, compptr++)
    {
      jmain->buffer[ci] = (*cinfo->mem->alloc_sarray)((j_common_ptr)cinfo, JPOOL_IMAGE,
                                                      compptr->width_in_blocks * DCTSIZE,
                                                      (JDIMENSION)(compptr->v_samp_factor * DCTSIZE));
    }
  }

...
static GOBVoid FreeCache(GOBVoid)
{
	...
	if (CacheBlocks)
	{
		for (i = 0; i < NumCacheBlocks; ++i)
		{
			if (CacheBlocks[i].data)
				MemFuncs.free(CacheBlocks[i].data);
...
static GOBError CommitFileTable(GOBVoid)
{
	...
	if (!FSFuncs.write(handle, &num, sizeof(num)))
		return GOBERR_FILE_WRITE;
	...
	for (num = 0; num < ArchiveNumBlocks; ++num)
	{
		...
		if (!FSFuncs.write(handle, &block, sizeof(block)))
			return GOBERR_FILE_WRITE;
	}
	...
	for (num = 0; num < ArchiveNumFiles; ++num)
	{
		...
		if (!FSFuncs.write(handle, &basic, sizeof(basic)))
			return GOBERR_FILE_WRITE;
	}
	...
	for (num = 0; num < ArchiveNumFiles; ++num)
	{
		...
		if (!FSFuncs.write(handle, &ext, sizeof(ext)))
			return GOBERR_FILE_WRITE;
	}
...
{
	if (!LibraryInit)
		return GOBERR_NOT_INIT;

	// if we have an open archive, close it
	if (!InvalidHandle(ArchiveHandle))
		GOBArchiveClose();
						...
						GOBBool extended, GOBBool safe)
{
	...
	if (!FSFuncs.read(handle, &magic, sizeof(magic)))
		return GOBERR_FILE_READ;
	if (SwapBytes(magic) != GOB_MAGIC_IDENTIFIER)
		return GOBERR_NOT_GOB_FILE;

	// read the GOB archive size
	if (!FSFuncs.read(handle, &ArchiveSize, sizeof(ArchiveSize)))
		return GOBERR_FILE_READ;
	ArchiveSize = SwapBytes(ArchiveSize);

	// read the number of blocks
	if (!FSFuncs.read(handle, &ArchiveNumBlocks, sizeof(ArchiveNumBlocks)))
		return GOBERR_FILE_READ;
	ArchiveNumBlocks = SwapBytes(ArchiveNumBlocks);

	// read the number of files
	if (!FSFuncs.read(handle, &ArchiveNumFiles, sizeof(ArchiveNumFiles)))
		return GOBERR_FILE_READ;
...
GOBUInt32 GOBGetSlack(GOBUInt32 x)
{
	...
	if (align)
		return GOB_BLOCK_ALIGNMENT - align;
	return 0;
}
...
GOBError GOBOpen(GOBChar *file, GOBHandle *handle)
{
	...
	{
		if (!OpenFiles[*handle].valid)
			break;
	}

	if (*handle >= GOB_MAX_OPEN_FILES)
		return GOBERR_TOO_MANY_OPEN;
...
static GOBUInt32 RawRead(GOBVoid *buffer, GOBUInt32 size, GOBUInt32 pos)
{
	...
	// seek
	if (FSFuncs.seek(ArchiveHandle, pos, GOBSEEK_START))
		return 0;

	if (CurrentArchivePos != pos)
		++ReadStats.totalSeeks;
...
static GOBUInt32 FindFreeBlock(GOBVoid)
{
	...
	{
		if (BlockTable[i].next == GOB_INVALID_BLOCK)
			return i;
	}
...
GOBError GOBWrite(GOBVoid *buffer, GOBUInt32 size, GOBUInt32 mtime, const GOBChar *file, GOBUInt32 codec_mask)
{
	...
	{
		if (FileTableBasic[handle].block == GOB_INVALID_BLOCK)
			break;
	}

	if (handle >= GOB_MAX_FILES)
		return GOBERR_TOO_MANY_FILES;
	if (handle >= (GOBInt32)ArchiveNumFiles)
		ArchiveNumFiles = handle + 1;
	...
	for (pos = 0; pos < size; pos += GOB_BLOCK_SIZE)
	{
		...
		// get a free block
		block = FindFreeBlock();
		if (block >= GOB_MAX_BLOCKS)
			return GOBERR_TOO_MANY_BLOCKS;
		if (block >= ArchiveNumBlocks)
			ArchiveNumBlocks = block + 1;

		// if this is not the first block, mark next block for the last block
		// else assign the first block in file table
		if (pos != 0)
			BlockTable[last_block].next = block;
		else
			FileTableBasic[handle].block = block;
		...
		in_size = size - pos;
		if (in_size > GOB_BLOCK_SIZE)
			in_size = GOB_BLOCK_SIZE;
		...
		if (codec_index == CodecFuncs.codecs)
			return GOBERR_NO_SUITABLE_CODEC;

...
*/
	...
	else
	{
		...
		if (child)
		{
			for (j = 0; j < 12; j++)
			{
				((float *)&tbone[2])[j] = backlerp * ((float *)&tbone[0])[j] + (1.0 - backlerp) * ((float *)&tbone[1])[j];
			}
		}
		else
		{
			for (j = 0; j < 12; j++)
			{
				((float *)&bonePtr[child])[j] = backlerp * ((float *)&tbone[0])[j] + (1.0 - backlerp) * ((float *)&tbone[1])[j];
			}
		}
	}
...
*/
	...
	//
	// ModView: ITU? Oh, well...
	if ((ent->e.iFrame_Primary >= aHeader->numFrames) || (ent->e.iFrame_Primary < 0) || (ent->e.iOldFrame_Primary >= aHeader->numFrames) || (ent->e.iOldFrame_Primary < 0))
	{
#ifdef _DEBUG
	...
	if ((ent->e.iFrame_Secondary >= aHeader->numFrames) || (ent->e.iFrame_Secondary < 0) || (ent->e.iOldFrame_Secondary >= aHeader->numFrames) || (ent->e.iOldFrame_Secondary < 0))
	{
#ifdef _DEBUG
		ri.Printf(PRINT_ALL, "R_AddGhoulSurfaces: no such frame %d to %d for '%s'\n",
#else

...
static bool drag_actual(mkey_enum keyFlags, int x, int y)
{
	...
	if (keyFlags != 0)
	{
			...
			if ((x != m_x) || (y != m_y))
			{
				...
				else
				{
					...
					if (s & 0x8000)
					{
						...
						if (AppVars.rotAngleZ > 360.0f)
							AppVars.rotAngleZ = AppVars.rotAngleZ - 360.0f;
						if (AppVars.rotAngleZ < -360.0f)
							AppVars.rotAngleZ = AppVars.rotAngleZ + 360.0f;
					}
					else
		...
		else if (keyFlags & KEY_RBUTTON)
		{
			if (y != m_y)
			{
				AppVars.zPos += ((float)(y - m_y) / 10.f) * MOUSE_ZPOS_SCALE;

				if (AppVars.zPos < -1000.f)
					AppVars.zPos = -1000.f;
				if (AppVars.zPos > 1000.f)
					AppVars.zPos = 1000.f;
...
bool drag(mkey_enum keyFlags, int x, int y)
{
	...
	{
#define BLAHBLAH(arg)                            \
	AppVars.arg##_SCROLL += (AppVars.arg - arg); \
	AppVars.arg = arg;
